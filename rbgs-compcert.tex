...
the behavior of which can more or less be characterized by
the possible sequences of system calls performed.
Several steps are involved to go
from a source program to an executing process,
which are modeled by CompCert language semantics
...
some of these aspects in simplified ways.
The lowest-level objects ever considered
are CompCert Asm programs.
The operation of the linker is approximated by
merging programs seen as sets of global definitions.
The remainder of the process is encoded
as part of the definition of language semantics:
first,
a symbol table is created laying out
the program's code and static data
into the runtime address space;
then,
an initial state is prepared
to emulate the usual invocation of a C program's
\kw{main} function at the end of the loading mechanism.
