...
say $\sigma_1 \sqsubseteq \llbracket p_1 \rrbracket$,
...
$\sigma \sqsubseteq \llbracket p_1 + p_2 \rrbracket$,
...
\cbstart
This can be done by establishing a Galois connection
$(\mathbb{D}^\sharp, {\sqsubseteq})
 \galois{\gamma}{\alpha}
 (\mathbb{D}^\natural, {\sqsubseteq})$
between the abstract domain $\mathbb{D}^\sharp$
and a more concrete one $\mathbb{D}^\natural$.
...
\cbstart
In that context,
a Galois connection between the source and target semantic domains
can be used to formalize the compiler's \emph{calling convention},
and the concretization function $\gamma$ will explain
how a C behavior can be realized at the assembly level.
...
[Condense and define $\rightarrow$ directly]
...
Existing work on the denotational semantics of
...
to game semantics \cite{gsnondet,the-sheaves-thing},
we believe that this work [does not recognize the importance of
distinguishing between angelic and demonic nondeterminism
...
of functional programming \cite{morris-and-tyrel}
to incorporate dual nondeterminism to our game semantics.
[This makes the theory in \S{}n really nice and uniform,
with dual nondet subsuming abstraction etc.]
...
$\mathbb{R} \circ \mathbb{S} : A_1 \Leftrightarrow A_3$
...
[Heavy rewrite needed but,
basically argue that reasoning with backward simulations is a PITA
but we define a nice abstract semantics
where everything embeds nicely
and which has very agreeable algebraic properties]
