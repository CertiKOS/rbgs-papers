...
such as CompCert,
its introduction represented a milestone
...
for instance
building verification tools with formal correctness proofs
extending CompCert \cite{vst,verasco} and
...
as a common mathematical framework for high-level
system description and reasoning.
On one hand, game semantics (\S\ref{sec:gamesem})
provides a highly flexible and expressive approach
to compositional semantics,
making it a plausible setting
to tackle the modeling of
complex heterogenous systems.
On the other hand,
verification often revolves around some notion of refinement
...
Given the central role compilers play
in the construction of modern computer systems,
any approach purporting to provide
a general-purpose framework for end-to-end verification
must be able to account for compiler use.
In particular,
it should be possible to integrate a compiler such as CompCert
and make it possible to exploit its correctness theorem
in this compositional, heterogenous setting.
[Add more details about the kind of things
we'd like to be doing?]

Since the introduction of CompCert
there has been a tremendous amount of work
aimed at refining its correctness theorem
...
Among those,
only Compositional CompCert \cite{compcompcert}
states both the semantics of open programs
and the overall compiler correctness theorem
in terms that characterize the interactions of
individual components directly.
Unfortunately,
this was only possible at the expense
of major changes and a significant increase in complexity
to the existing proofs of CompCert.
This has prevented the work
from being integrated to official releases of CompCert
or updated to function with recent versions.

%}}}

\subsection{Contributions} %{{{

This paper describes CompCertO,
a new extension of CompCert
which addresses these limitations.
In particular,
building on existing work:
\begin{itemize}
\item We generalize CompCert's labelled transition systems
  to describe cross-component interactions.
  Rather than relying on a fixed model,
  we use \emph{language interfaces}
  to describe the form of these interactions
  (\S\ref{sec:sem:open}).
\item Simulations are then expressed relative to a
  \emph{simulation convention}
  which describes the correspondance between
  the language interfaces respectively used
  by source and target semantics (\S\ref{sec:sim}).
\item Reading of extensions and injections
  as \emph{logical relations};
  we make this insight formal by defining CKLR (\S\ref{sec:cklr}).
\item As demonstrated in \S\ref{sec:passes},
  the expressivity and flexibility afforded
  by simulation conventions
  allows us to update the existing passes of CompCert
  with minimal effort.
\item As in CompCompCert,
  the intended behavior of a multi-component program
  is given by a \emph{semantic linking} / \emph{horizontal composition}
  operator (\S\ref{sec:sem:linker}).
  Going one step further,
  we relate this semantic operation to the
  syntactic linking of assembly programs
  introduced by SepCompCert,
  allowing compositional verification
  to be used to characterize the final linked assembly program
  (\S\ref{sec:cver}).
\item \ldots
\end{itemize}
...
This concludes our exposition of the results obtained in CompCertO,
our adaptation of CompCert following the principles laid out
in \S\ref{sec:principles}.

%}}}

\section{Evaluation} %{{{

We evaluate our results along two dimensions:
first,
we describe how they can be used to carry out
compositional compilation and verification;
then,
we discuss the complexity of our development
and compare it with similar approaches.
...
\begin{table} % tbl:slocs {{{
  \begin{tabular}{lr@{\:}r}
    ...
    \hline
  \end{tabular}
  \caption{Significant lines of code added in CompCertO}
  \label{tbl:slocs}
\end{table}
%}}}
...
our methodology comes with a very modest increase
in the complexity of most passes.

[Comment more and compare with others]
...
TODO:
\begin{itemize}
\item open simulations vs. our notion
\item RUSC vs. simulation algebra
\item their vs. ours handling of
  the assembly language interface
\item demonstrate compositional verification
  and verification of a runtime function.
  (whereas we don't other than showing proof principles)
\end{itemize}
