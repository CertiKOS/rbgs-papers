...
not only expect external calls to preserve that injection,
but also restrict changes to the source and target memory states
to regions that are within the injection's footprint:
addresses that are
related by the injection mapping,
and are granted non-empty permissions
in both memory states.
This is depicted in Fig.~\ref{fig:injp}.
...
source memory blocks which correspond to local variables
whose address is not taken
are removed from the target memory state.
