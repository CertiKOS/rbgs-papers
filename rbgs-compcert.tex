...
\begin{gather*}
    ...
    \\[1ex]
    \AxiomC{$\iota \Vdash \vec{v}_1 \hookrightarrow \vec{v}_2$}
    \AxiomC{$\iota \Vdash m_1 \hookrightarrow m_2$}
    \BinaryInfC{$\iota \Vdash
      f[\mathit{sg}](\vec{v}_1)@m_1
      \hookrightarrow
      f[\mathit{sg}](\vec{v}_2)@m_2$}
    \DisplayProof
    \\[1ex]
    \AxiomC{$\iota \subseteq \iota'$}
    \AxiomC{$\iota' \Vdash v_1' \hookrightarrow v_2'$}
    \AxiomC{$\iota' \Vdash m_1' \hookrightarrow m_2'$}
    \TrinaryInfC{$\iota \Vdash v_1'@m_1' \hookrightarrow v_2'@m_2'$}
    \DisplayProof
\end{gather*}
...
but unfortunately it insufficient for outgoing calls.
For instance,
in the \kw{SimplLocals} pass,
local variables whose address is not taken
...
The \kw{SimplLocals} proof depends on the fact that
external calls do not modify the source memory blocks
corresponding to such variables
in order to reestablish this correspondance
after external calls return.
...
\begin{figure} % fig:injp {{{
  ...
  \caption{External calls and memory injections.
    ...
    depicted respectively at the top and bottom
    of the figure. Arrows describe the mapping of
    source memory blocks into target ones.
    Memory blocks to the right of the dashed line
    have been allocated during the call,
    which adds a new entry to the injection mapping.
    The shaded areas must not be modified.
  }
  \label{fig:injp}
\end{figure}
%}}}
...
to leave regions that are within the injection's footprint untouched.
...
can be formalized along the following lines:%
\begin{gather*}
  \kw{injp} :=
    \langle
  ...
  \\[1ex]
  \small
  \AxiomC{$\kw{P}(\iota, m_1, m_2, \iota', m_1', m_2')$}
  \AxiomC{$\iota' \Vdash v_1' \hookrightarrow v_2'$}
  \AxiomC{$\iota' \Vdash m_1' \hookrightarrow m_2'$}
  \TrinaryInfC{$(\iota, m_1, m_2) \Vdash
    v_1'@m_1'
    \mathrel{R_\kw{injp}^\kw{A}}
    v_2'@m_2'$}
  \DisplayProof
\end{gather*}
where $\kw{P}(\iota, m_1, m_2, \iota', m_1', m_2')$
expresses the constraints we have outlined.%
...
The \kw{Stacking} pass constructs
the stack frames used by the final program
by mapping the abstract stack locations used by \kw{Linear}
...
the various new regions of stack frames
...
\kw{Stacking} translates reads from argument locations
by using \kw{Mach}'s special instruction \kw{getparam},
which accesses the caller's stack frame.
Therefore,
the \kw{stacking} simulation convention
asserts that the values of argument locations
are indeed store into corresponding stack slots
within the target memory.

In previous work,
where the $\mathcal{C}$ interface is uniformly used
to describe the semantics of all languages,
this has been a source of considerable challenges.
[EXPAND]

By contrast,
in our typed, relational model,
the argument-passing mechanism used by \kw{Stacking}
is entirely straightforward to express:
the \kw{stacking} simulation convention simply
...
This prevents us from having to modify the \kw{Stacking} proof
in similarly extensive ways.
\kw{Stacking} is the pass for which
we modify the proof most extensively
(largely to maintain the incoming invariants).
Yet,
while Compositional CompCert
more than doubles the size of the \kw{Stacking} proof,
and CompCertX [CHECK],
our changes only increase it by $\approx 15\%$.

[Keep some of that for an evaluation / related work section?]
...
The memory operations are used as the building blocks
of more complex constructions,
which in turn satisfy similar relational properties.
Ultimately,
this applies to complete language semantics
and translates to properties such as the ones
summarized in Thm.~\ref{thm:lprops}.

\subsection{Definition} %{{{

We formalize this idea by defining
a notion of Kripke logical relations over the CompCert memory model,
which admits memory extensions and injections
...
\begin{theorem}[Simulation conventions and CKLRs]
\[
