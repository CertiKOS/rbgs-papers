...
those typically used to describe the semantics of functional languages.
Nevertheless,
...
The composite strategy $\tau \circ \sigma : A \Rightarrow C$
is obtained by letting $\sigma$ and $\tau$ interact
with each other over their common game $B$,
and with the environment over the games $A$ and $C$.
Note that because $\tau$ may invoke $\sigma$ several times,
we need to first promote $\sigma$ to a strategy $\sigma^\dagger$,
capable of answering several independent questions.
The traditional formulation of $\tau \circ \sigma$
...
\]
where $s \restriction X,Y$ is the subsequence of $s$
retaining only the moves played in the games $X$ or $Y$.
...
silent divergence is identified with the empty interaction $\varepsilon$,
...
maintaining a set of diverging plays.
