...
Our family of games is built from
a class of \emph{elementary games},
which have an extremely simple structure.
...
Elementary games proceeds as follows:
...
When considering alternating, well-bracketed strategies,
...
\begin{itemize}
  ...
  \item The system may then ask a question from $M_A^\kw{Q}$,
    which will be answered by the environment by a move in $M_A^\kw{A}$.
...
the valid alternating, well-bracketed plays of $A \Rightarrow B$
...
first-order types
makes our model very straightforward
compared to the game models used to describe
the semantics of high-order functional languages.
Nevertheless,
...
remain relevant and useful in our context.
...
we will simply leave out odd-length plays.
...
\[
  \mathcal{S}_{A \Rightarrow B} \: \approx \:
   \big\{ \sigma \subseteq
      ...
      \big( M^\kw{Q}_A \cup M^\kw{A}_B \big) \mid
...
$\tau : \mathcal{S}_{B \Rightarrow C}$ \ldots

[Describe the traditional construction
$\{ (t \restriction {A,C}) \mid (t \restriction {A,B}) \in \sigma^\dagger \wedge
        (t \restriction {B,C}) \in \tau \}$]

Note that this traditional formulation
relies on identifying silent divergence with
the absence of any behavior,
since for any play $s$ consisting of an interaction
between $\sigma$ and $\tau$ over the common game $B$,
which will become silent in the composition,
we will have $s \restriction A,C = \varepsilon$.

%}}}

\subsubsection{Nondeterminism} %{{{

Notice that we do not impose
...
We will use this potential multiplicity
to represent specifications permitting a range of possible
concrete behaviors,
with trace containement as the corresponding notion of refinement.
...
is inappropriate:
in particular,
this would mean a diverging program
would refine any specification.
Instead,
...
a set of plays after which the system may silently diverge.
Instead,
we extend our model by allowing plays to contain
the special move $\Delta$ as the last move of the system.
This approach is more convenient given our treatment of divergence
as a standard behavior from the point of view of the refinement order,
but is otherwise equivalent to that found in \cite{gsnondet}.
...
we treat divergence by
[extending strategy representation with explicit divergence info];
however from the point of view of refinement
divergence is largely a behavior on par with others.
The semantic domain obtained follows
