...
which duplicates the behavior or $\sigma$ to answer several independent questions.
...
Consider $R, S, T$ arbitraty CompCert Kripke logical relations.
The composition of CKLRs satisfies the followinf properties.
\begin{gather*}
  (R ; S) ; T \equiv R ; (S ; T) \\
  \kw{inj} ; \kw{inj} \equiv
    \kw{ext} ; \kw{inj} \equiv
...
the simulations further be iterated
...
\begin{table*} % tbl:passes Passes of Composable Compcert %{{{
  ...
  \begin{tabular}{lllp{.55\textwidth}}
    ...
    -- & $\mathcal{C}[\kw{injp}]^*$ & $\mathcal{C}[\kw{injp}]^*$ &
    ...
    -- & $\mathcal{C}[\kw{inj}]$ & $\mathcal{C}[\kw{inj}]$ & \emph{RTL properties} \\
