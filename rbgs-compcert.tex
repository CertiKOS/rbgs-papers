...
where bootstrap code sets up the process state
...
\item[Initial states]
  ...
  there exist $i, s_2$ such that $q_2 \mathrel{I_2} s_2$ and
  $w_B \Vdash s_1 \mathrel{R_i} s_2$.
\item[Simulation]
  For all related states $w_B \Vdash s_1 \mathrel{R_i} s_2$
  and for any transition $s_1 \stackrel{t}{\rightarrow} s_1'$,
  there exists $i' \in I$ and $s_2' \in S_2$
...
\item[External calls]
  For all related states $w_B \Vdash s_1 \mathrel{R_i} s_2$
  and for any question $m_1 \in M_{A_1}^\kw{Q}$
  such that $s_1 \mathrel{X_1} m_1$,
  there exist $w_A \in W_A$ and a question $m_2 \in M_{A_2}^\kw{Q}$
  such that $w_A \Vdash m_1 \mathrel{\mathbb{R}_A^\kw{Q}} m_2$.
  Furthermore, for a corresponding pair of related answers
  $w_A \Vdash n_1 \mathrel{\mathbb{R}_A^\kw{A}} n_2$
  and a state $s_1' \in S_1$ such that $(s_1, n_1, s_1') \in Y_1$,
  there exists a state $s_2' \in S_2$ such that
  $(s_2, n_2, s_2') \in Y_2$.
\item[Final states]
  For all related states $w_B \Vdash s_1 \mathrel{R_i} s_2$
  and for any answer $r_1 \in M_{B_1}^\kw{A}$
  such that $s_1 \mathrel{F_1} r_1$,
  there exists an answer $r_2 \in M_{B_2}^\kw{A}$ such that
...
The horizontal composition operator and its properties below
allows us to decompose the verification of a complex program
into the verification of its individual parts.
In particular,
...
on the memory states and returned values.
...
but unfortunately it is insufficient for outgoing calls.
For instance,
in the \kw{SimplLocals} pass,
local variables whose addresses are not taken
...
uses memory extension;
in addition,
it is the first pass to modify the interface of function calls.
