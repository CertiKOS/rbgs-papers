...
of open programs \emph{directly} in terms of their
interaction with the environment.
By extending the semantic model of CompCert
in a way that embraces abstraction and
the relational aspect of CompCert's correctness proofs,
we are able to achieve this
without the prohibitive increase in complexity
that has plagued previous attempts in this direction
such as Compositional CompCert.
\end{abstract}
%}}}
...
such as CompCert,
its introduction represents a milestone
...
On the other hand,
verification often revolves notions of refinement
...
should satisfying the following requirements:
\begin{itemize}
\item The semantics of the source and target languages
  should characterize the behavior of \emph{open} components
  in terms of their interactions with the rest of the program.
\item The correctness theorem
  should go beyond contextual refinement and characterize
  these interactions directly.
  (This is because in that context there is no definitive
  notion of context ...)
\item {} [Account for abstraction --- if we're going to connect
  assembly code to hardware stuff and verify OS we need close-to-the
  machine stuff modelled.]
\item Certified linking of program components.
\end{itemize}
There has been a tremendous amount of work
aimed at refining the correctness theorem of CompCert
to make it more composisional and
model real-world compiler use
...
Our requirements have all been tackled
by one or more of these extensions,
but none has addressed them simultaneously.
