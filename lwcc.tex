...
[Our use of the word \emph{domain} and
of the semantic brackets $\llbracket - \rrbracket$
should not be taken to imply
that are semantics must be given in a particular style,
and at this stage
we impose no particular structure on the set $\mathbb{D}$.]

As a running example /
To illustrate this formalism,
we define two languages.
The programs in these two languages
are interpreted as defining single-argument integer functions.

we define a high-level language for computing integer functions,
whose semantics may be defined in the denotational style,
and a low-level language which might serve 
...
\[
  e_1 = ((x \times (3 + x)) + 2)
\]
An expression $e$ in \kw{AlgExp} can naturally be assigned a meaning
$\llbracket e \rrbracket : \mathbb{Z} \rightarrow \mathbb{Z}$
in the set of single-argument integer functions,
so that the meaning assigned to $e_1$ is:
\[
  \llbracket e_1 \rrbracket (n) = n^2 + 3n + 2
\]
\end{example}

\begin{example}[$\kw{RegOps}$]
...
\begin{align*}
  ...
  p \in \kw{RegOps} &::=
    ...
    r := r \times r; \, p
\end{align*}
Here $\bullet$ denotes the empty string.
An example program is:
\[
  p_1 = \quad
  \kw{r2} := \kw{r1} + 1; \,  %XXX constant not allowed
  \kw{r3} := \kw{r2} \times \kw{r2}; \,
  \kw{r1} := \kw{r2} + \kw{r3};
\]
To interpret programs as integer functions,
the machine first stores the argument in $\kw{r1}$,
executes the program's assignments in sequence,
the reads out the answer from the new contents of $\kw{r1}$.
In the program $p_1$,
the first assignment computes $x + 1$,
the second assignment computes $(x+1)^2$,
and the third assignement computes $(x+1)^2 + (x+1)$.
Accordingly,
the behavior $\llbracket p_1 \rrbracket$ associated to that program
is the same as that of $e_1$.
\end{example}

Compared to $\kw{AlgExp}$ and $\kw{RegOps}$,
the syntax and semantics of real-world languages
are much more intricate.
In Compcert,
programs are essentially modelled as
finite maps from identifiers to definitions of functions and variables.
C function definitions may carry complex abstract syntax trees
for the function's body,
whereas assembly function definition consist in a list of instructions
taken from a large collection.
The semantics of Compcert languages
are given in terms of state transition system,
and ultimately in terms of sets of finite and infinite traces.

Nevertheless,
these simplistic examples are sufficient to illustrate
a number of interesting phenomena.
In the rest of this section,
we will use them as a way to make the formal aspects
of these phenomena more accessible,
while we discuss their real-world counterparts
...
Given the definition of languages
...
\begin{align*}
  C(e_1 * e_2) = \quad  % XXX overloading e_1
    &\kw{r2} := \kw{r1}; \, % XXX copy not allowed
    {\uparrow} C(e_1); \\
    &\kw{r3} := \kw{r1}; \,
