...
are built up in a systematic way from elementary relations
...
\item $U^\kw{v}_R : \mathbb{B}$
  indicates whether $R$ matches undefined values on the left
  to arbitrary non-pointer values on the right;
\item $U^\kw{b}_R : W_R \rightarrow \mathcal{P}(\kw{block})$
  indicates for each possible world $w$
  whether $R$ matches undefined values on the left
  to pointers into a given block $b$ on the right;
\item $f : W_R \rightarrow \kw{meminj}$
  gives for each possible world $w$
  a memory injection which specifies how pointers should be related;
\item $R^\kw{mem} : W_R \rightarrow \mathcal{R}(\kw{mem})$
  indicates for each possible world $w$
  how memory states should be related.
\end{itemize}
The components of $R$ must additionally satisfy
