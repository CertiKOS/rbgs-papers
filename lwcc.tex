...
The log is also maintained,
and external calls are performed
outside of the original small-step semantics,
so that we use it with an empty $\kw{external\_call}$ predicate
and do not need to instrument Compcert memory states.
...
\begin{align*}
  [s \Rightarrow s'] &\Leftrightarrow
    \kw{Clight}_\varnothing(p) : s \rightarrow^* s' \\
...
and allows strategies with hidden states
and complex control behavior.
...
$\lhd_F : {!\EC} \multimap {!\EC} \multimap \EC$ is the strategy defined by
...
The linked strategy will be obtain by taking a fixed point,
...
\[
  \tilde{\sigma} =
    \Lambda
      ...
      \sigma \circ
      \lhd_F^\dagger \circ
      (\kw{eval}_{{!\EC} \otimes {!\EC}, {!\EC}} \otimes \kw{id}_{!\EC}) \circ
...
\begin{center}
  \includegraphics[scale=0.5]{linking-element}
\end{center}
