...
We consider the language of polynomials in one variable
over the field of integers.
These polynomials are algebraic expressions
build of integer constants, $+$, $\times$,
and a single variable $x$.
Formally, the language $P$ is given by the following grammar:
\[
  p \in P ::= c \:|\: x \:|\: (p \times p) \:|\: (p + p) \,,
\]
where $c \in \mathbb{Z}$ denotes an integer constant,
and $x$ is a single terminal symbol.
One ``program'' in this language is the following expression:
\[
  ((5 \times (x \times (2 + x))) + 1)
\]

If such expressions are to be interpreted as polynomials
in the usual sense,
then a natural semantic domain for the language $P$
is the set $\mathbb{F} = \mathbb{Z} \rightarrow \mathbb{Z}$
of single-argument functions over integers.
It is straightforward to define a denotational semantics
for this language:
\begin{align*}
  \llbracket c \rrbracket(v) &= c &
  \llbracket (p_1 + p_2) \rrbracket(v) &=
    \llbracket p_1 \rrbracket(v) + \llbracket p_2 \rrbracket(v) \\
  \llbracket x \rrbracket(v) &= v &
  \llbracket (p_1 \times p_2) \rrbracket(v) &=
    \llbracket p_1 \rrbracket(v) \cdot \llbracket p_2 \rrbracket(v)
\end{align*}
Then the meaning of the expression above is given by the function:
\[
  \llbracket ((5 \times (x \times (2 + x))) + 1) \rrbracket (v) =
    5v^2 + 10v + 1
\]
\end{example}

To illustrate a slightly lower-level and more operational language
with similar capabilities, here is another example.

\begin{example}[Assignment sequences]
The language $A$ models a machine
with an infinite number of registers,
which can perform a sequence of simple algebraic operations:
\begin{align*}
  r \in R &::= \texttt{r1} \:|\: \texttt{r2} \:|\: \cdots \\
  p \in A &::= \bullet \:|\: r := c; p \:|\: r := r + r; p \:|\: r := r \times r; p
\end{align*}
An example program is:
\begin{align*}
  \texttt{r2} &:= 10; \\
  \texttt{r2} &:= \texttt{r2} \times \texttt{r1}; & & (\texttt{r2} \mbox{ contains } 10x) \\
  \texttt{r1} &:= \texttt{r1} \times \texttt{r1}; & & (\texttt{r1} \mbox{ contains } x^2) \\
  \texttt{r3} &:= 5; \\
  \texttt{r3} &:= \texttt{r3} \times \texttt{r1}; & & (\texttt{r3} \mbox{ contains } 5x^2) \\
  \texttt{r1} &:= 1; \\
  \texttt{r1} &:= \texttt{r1} + \texttt{r2}; \\
  \texttt{r1} &:= \texttt{r1} + \texttt{r3}; \bullet & & (\texttt{r1} \mbox{ contains } 5x^2 + 10x + 1)
\end{align*}
