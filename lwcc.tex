...
An \emph{arena} is a tuple $\langle M, \lambda, \vdash \rangle$
where $M$ is a set of \emph{possible moves},
$\lambda : M_A \rightarrow \{\kw{O},\kw{P}\} \times \{\kw{Q},\kw{A}\}$
labels moves as \emph{opponent moves} or \emph{proponent moves}
and as \emph{questions} or \emph{answers}, and
$\vdash$ is a relation between $M + \{\star\}$ and $M$,
called the arena's \emph{enabling relation}.
...
[If necessary, add some more constraints on $\vdash$ vs $\lambda$:
answers don't justify anything and players alternate.
Let's see how the proofs go.]

\begin{definition}[Justified sequence]
A \emph{justified sequence} on an arena $\langle M, \lambda, \vdash \rangle$
is a finite sequence
$s = (m_1, p_1) (m_2, p_2) \cdots (m_n, p_n) : (M \times \mathbb{N})^*$.
With the convention $m_0 = \star$, we require that
$m_{p_i} \vdash m_i$ for all $1 \le i \le n$.
The $m_i$'s must alternate between opponent moves and proponent moves.
\end{definition}

Each $(m_i, p_i)$ is called a \emph{move in $s$}.
The component $p_i$ is called the move's \emph{pointer}
and is the index of
an earlier move called its \emph{target}.
When writing out particular sequences
...
it must be the case that $a, c, e$ be opponent moves,
$b, d$ proponent moves (as indicated by underlining),
and that:
\[
\star \vdash a, \quad
a \vdash b, \quad
...
A game is a tuple $A = \langle M_A, \lambda_A, \vdash_A, P_A \rangle$
where $\langle M_A, \lambda_A, \vdash_A \rangle$ is an arena
and $P_A$ is a prefix-closed set of justified sequences over that arena.
...
\begin{gather*}
  ...
  \vdash_A
      ...
      \right\}
    \vdash_B \\
  P_A \ifr{\Box (R^\kw{jsq} \rightarrow \Diamond R^\kw{jmv} \rightarrow (\Rightarrow))} P_B
\end{gather*}
...
(which will presumable return immediately),
then answers the initial function call
