...
of its operation.
For example,
when using the code in Fig.~\ref{fig:code},
we can think of the state of the queue
...
the client code does not directly access
...
\[
  ...
  \,.
\]
The transition system $L \mathbin@ U$ obliviously passes along
...
and updated with the corresponding answer.
When $L$ terminates,
...
\begin{equation} \label{eqn:overview:sepalg:frame}
  ...
  \infer1{
   {\begin{array}{r@{\:}l}
    L \:\vDash\: q@(m_0 \bullet \hat{m}_0) \rightarrowtail
      q_1@(m_1 \bullet \hat{m}_0) &\leadsto r_1@(m_1' \bullet \hat{m}_1) \rightarrowtail
      \cdots \\ \cdots \rightarrowtail
      q_n@(m_n \bullet \hat{m}_{n-1}) &\leadsto r_n@(m_n' \bullet \hat{m}_n) \rightarrowtail
      r@(m' \bullet \hat{m}_n)
   \end{array}}}
  \end{prooftree}
\end{equation}
...
\begin{example}
abstraction relation as
\[
  (\kw{mem} \times R) \mathbin; [\bullet]
\]
\end{example}
