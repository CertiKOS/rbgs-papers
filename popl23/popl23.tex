...
are likewise fine-tuned to a particular setting.
...
use models which are convenient for the task at hand.
At the same time,
the resulting specifications and proofs
could be embedded in more general ones,
where they could interfaced
...
%CompCertO pretends to do better.
%It deals with abstraction levels and compositionality,
%but it can't hide state
%which is a central feature/need for CAL.
...
\begin{example}[The $\mathcal{C}$ language interface] %{{{
...
are given as:
\[
  \kw{Clight}(-) : \mathcal{C}@\kw{mem} \twoheadrightarrow \mathcal{C}@\kw{mem}
  \qquad \qquad
  \kw{Asm}(-) : \mathcal{A}@\kw{mem} \twoheadrightarrow \mathcal{A}@\kw{mem}
  \,,
\]
so that in particular the behavior of assembly programs
to be modeled more precisely.

This approach also underscores the importance,
