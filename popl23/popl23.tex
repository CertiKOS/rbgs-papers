...
We can then rely on these services
without concern for the data representation
and implementation details used by lower-level code;
instead,
a programmer writing client code
will understand and reason about
this \emph{underlay} interface
in terms of a more abstract mental picture
of its operation.

Certified abstraction layers
are a formalization of this methodology
which was used to verify the operating system kernel
CertiKOS \cite{popl15}.
Modeling layers required a modification of CompCert's
closed semantics
to augment them with abstract state and
...
which initiates the execution of the $\kw{main}()$ function in $M$.
When $M$ invokes an external function,
the behavior of that function is directly obtained from the parameter $\chi$.
In the CertiKOS proof,
abstraction layers are formalized by using
a modified version of CompCert,
...
in the context of the underlay interface $L^\flat$,
...
the code $M$ itself does not directly access
the abstract memory component in $D$,
and can only affect it by invoking
the primitives of the underlay~$L^\flat$.
In the compositional framework of CompCertO,
a transition system
...
\[
  ...
  \,.
\]
which simply ``threads through'' a value of type $U$
as the underlying transition system $L$ executes.
The value is initially received
from the incoming question in $B \mathbin@ U$.
...
the behavior of the compiled program $M'$
is simulated by the behavior of $M$.
The transitivity of simulations allows
...
\[
  ...
  \,.
\]
This raises the question of the correspondence
between the source-level interactions in $\mathcal{C} \mathbin@ \kw{mem}$
and their matching target-level interactions in $\mathcal{A} \mathbin@ \kw{mem}$.
In other words,
...
\[
  ...
  \end{tikzcd}
  \qquad \qquad
  \vcenter{ \hbox{\includegraphics[scale=0.095]{draftfig/simsd}} }
\]
...
and call $\mathbf{SC}$ the category of
...
As depicted in Fig.~\ref{fig:overview:simcomp},
