...
[TODO: condense (move some content to overview?),
rework contributions once we have everything]
...
Then a transition system of type $A \twoheadrightarrow B$
describes a behavior with the following shape:
\[
  \includegraphics[scale=0.15]{draftfig/interaction}
\]
The component is activated by an incoming call,
in the form of a question $q \in B^\que$
...
and receiving the corresponding answers
$r_1, \ldots, r_n \in A^\ans$.
Eventually a final state is reached,
triggering the top-level answer $r \in B^\ans$.
...
the identity transition system $\kw{id}_A : A \twoheadrightarrow A$,
which echoes any incoming question as an outgoing one
and propagates the answer back to the caller:
\[
  \includegraphics[scale=0.08]{draftfig/identity}
\]
...
Compiler correctness only makes sense
...
String diagrams provide a more canonical and economical
representation of such morphisms,
...
String diagrams provide an intuitive representation
of composite morphisms

String diagrams are 

This means in particular that composite simulation conventions
can be represented as canonical \emph{string diagrams}
...
%
%Contrast low-level transition systems with
%coherence spaces and object-based semantics.
%
%Explain the CompCertO model in terms of high-level constructions.
