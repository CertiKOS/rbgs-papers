...
we follow \citet{freshlook}
and define a ternary \emph{join} relation
$J \subseteq (\kw{mem} \times \kw{mem}) \times \kw{mem}$,
where $J(m_1, m_2, m)$ will be written as:
\[
  m_1 \bullet m_2 \equiv m
  \,.
\]
This relationship
asserts that the memory states $m_1$ and $m_2$
can be merged into $m$.
Properties similar to that of a separation algebra
will ultimately ensure we are able to derive
a version of the \emph{frame rule}:
if a program can successfully execute on $m_1$ alone
to yield a new fragment $m_1'$,
...
Below we explain how such a relation can be defined
for the CompCert memory model.

\subsubsection{Memory Contents}

A CompCert memory state is essentially a partial map
of type \[ \kw{ptr} \rightharpoonup \kw{perm} \times \kw{memval} \,, \]
define on all addresses which carry non-empty permissions.
Hence, so far as the memory contents are concerned,
standard separation algebra constructions can be used
and we can define the join relation
as the disjoint union of partial maps.

Note that under this approach,
the memory value $\kw{undef}$ does not play any particular role.
We also ignore any values for addresses with empty permissions;
this is benign because no memory operation could depend on
or access these values in any way.
On the other hand,
defining $\bullet$ along these lines
makes it possible to support fine-grained separation,
where different addresses within the same block
may be assigned to different memory fragments.
...
However,
they will each carry their own $\kw{nextblock}$ counter.
If they were to independently allocate new blocks,
they may then each allocate new blocks
whose identifiers would clash.
