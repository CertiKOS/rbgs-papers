...
of any modern computing environment.
...
to real-world use:
are the source and target language semantics realistic?
Does the correctness property relating them
indeed correspond to the way the compiler is used,
...
Empirical evaluation seems to confirm that this works,
...
Here,
the correctness proof of the compiler
will become one important component within
...
when the target code itself is only part of a larger system?
...
Finally,
can these structures be subsumed within
...
has established ways to make the correctness theorem of CompCert
easier to use as an ingredient in
...
\item
  ...
  In conjunction with simulation relations and their own compositional structure,
...
\section{Basic framework} \label{sec:base} %{{{
...
which represent memory locations in $\kw{Clight}$,
