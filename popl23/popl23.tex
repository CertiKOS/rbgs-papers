...
In addition,
stateful components support an operation
\[
  \Sigma : A \rightarrow B \mathbin@ U
  \quad \vdash \quad
  \kw{fbk}_{u \in U}(\Sigma) : A \rightarrow B
  \,,
\]
which incorporates a $U$ component into the private states of $\Sigma$
and can be depicted as:
\[
  \text{[fig along the lines of:]}
  \quad
  \vcenter{\hbox{ \includegraphics[scale=0.16]{draftfig/encap} }}
\]
This operation interacts with simulations in the following way:
\[
  \begin{prooftree}
    \hypo{\Sigma^\sharp
      \le_{\mathbf{R}_A \twoheadrightarrow \mathbf{R}_B \mathbin@ R}
      \Sigma^\flat}
    \hypo{u \mathrel{R} v}
    \infer2{
      \kw{fbk}_{u \in U}(\Sigma^\sharp)
      \le_{\mathbf{R}_A \rightarrow \mathbf{R}_B}
      \kw{fbk}_{v \in V}(\Sigma^\flat)}
  \end{prooftree}
\]

\begin{example}
ClightP with encapsulated private environment?
\end{example}
...
After presenting CompCertO's notion of transition system
(\S\ref{sec:base:ts}),
we define layered composition
(\S\ref{sec:base:lcomp})
and present our extended notions of simulation and simulation convention
(\S\ref{sec:base:sim}).
As outlined in \S\ref{sec:overview:doublecat},
these constructions equip CompCertO semantics
with the structure of a double category,
giving a formal underpinning to the usual notions of
horizontal and vertical composition
found in existing work on compositional certified compilers.

Moreover,
the monoidal structures described in \S\ref{sec:overview:mon}
add one more dimension to the compositional structure of the model,
...
\begin{definition}[Identity transition system]
...
\subsection{Double Category} \label{sec:base:double} %{{{
...
in terms of $\odot$,
we can compose it with the compiler's correctness theorem
...
\subsection{Adjoining Explicit State} \label{sec:base:state} %{{{
