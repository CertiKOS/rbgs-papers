...
such that:
\begin{itemize}
  \item the initial states are related by $R$;
  \item the transition systems satisfy
    $L_1 \le_{A@R \twoheadrightarrow B@R} L_2$.
\end{itemize}
...
which parameterize simulations by
...
\begin{itemize}
  ...
  \item two accessibility relations
    ...
    corresponding to \emph{external} and \emph{internal} transitions;
...
where $w'$ is a successor world of $w$ such that $w \mapsto w'$.
The world $w'$ then becomes the simulation convention's new state,
and the questions for any subsequent activation
must be related at a world $w''$ such that $w' \leadsto w''$.

\begin{definition}[Simulation]
Suppose the components $\Sigma_1, \Sigma_2$
and the simulation conventions $\mathbb{R}_A, \mathbb{R}_B$
have the following types:
\[
\begin{tikzcd}
  A_1 \ar[r, "\Sigma_1"] \ar[d, Leftrightarrow, "\mathbb{R}_A"'] &
  B_1 \ar[d, Leftrightarrow, "\mathbb{R}_B"] \\
  A_2 \ar[r, "\Sigma_2"] & B_2
\end{tikzcd}
\]
There is a simulation
$\Sigma_1 \le_{\mathbb{R}_A \rightarrow \mathbb{R}_B} \Sigma_2$
when there exists a Kripke relation
$R : \mathcal{R}_{W_A \times W_B}(K_1, K_2)$
satisfying
$(w_0^A, w_0^B) \Vdash k_0^1 \mathrel{R} k_0^2$
and such that:
\[
...
\begin{lemma}
Correspondence between CompCertO
and stateful simulations.
\end{lemma}
