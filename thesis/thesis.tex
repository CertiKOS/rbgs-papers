...
\begin{figure} % fig:relators {{{
  ...
  \caption{A selection of relators.
    The relators $\times$, $+$, $\rightarrow$ are
    the standard relators for the corresponding type operators.
    The relator $\mathcal{P}^\le$ is an asymmetric
    relator for the powerset operator $\mathcal{P}$,
    which can be use to formulate simulations.
    }
  \label{fig:relators}
\end{figure}
%}}}
...
on nondeterminism in game semantics \citep{gsnondet}.
On the other hand,
the relaxation of (\ref{eqn:det-comp})
...
is the one most in line with out goals.
...
and imposes a proliferation of side-conditions to the user.
...
If this specification cannot be represented,
...
onto the usual construction of strategies,
