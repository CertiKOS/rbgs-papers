...
CertiKOS was developed in the Yale FLINT group.
I joined the group around the time the first effort
to verify CertiKOS started.
...
and guarantee that the source programs written by the user
...
as the borader problem of
modeling certified components.
...
\begin{figure}
  ...
  \begin{tabular}{c@{\hspace{4em}}c}
    ...
    \hspace{4em} \raisebox{0.5ex}{\includegraphics[scale=0.11]{demon}}
    \\
    (a) Angelic choice &
    (b) Demonic choice
  \end{tabular}
...
between questions and and their corresponding answers,
...
the operation $(m \mathop: N) \in E$ itself
...
\begin{itemize}
  ...
  \item
    ...
    sometimes written
    $v \mathop{\leftarrow} \tau \mathop{;} f(v)$.
\end{itemize}
...
\begin{align*}
  v \mathop\leftarrow \tau \mathop; \eta_X(v) \:&=\: \tau
  \\
  v \mathop\leftarrow \eta_X(x) \mathop; f(v) \:&=\: f(x)
  \\
  v \mathop\leftarrow
    (u \mathop\leftarrow \tau \mathop; f(u))
      \mathop; g(v) \:&=\:
  u \mathop\leftarrow \tau \mathop;
    v \mathop\leftarrow f(u) \mathop;
      g(v)
\end{align*}
...
its outermost operation $m \mathop: N$
...
\end{equation}
In fact,
every family of algebras which satisfies this property
...
\begin{equation} \label{eqn:substint}
  ...
  \qquad \qquad
  (t f)[\sigma] = v \mathop\leftarrow t[\sigma] \mathop; f(v)[\sigma]
\end{equation}
We will say that $(-)[\sigma]$ is a
\emph{monad homomorphisms} (see Definition~\ref{def:setmonhom} below).

\begin{remark} %{{{
In the context of universal algebra,
monads are usually understood to
...
\begin{align*}
  \eta^E_X(v) \, [\sigma] &= \eta^T_X(v)
    && v \in X
  \\
  (v \mathop\leftarrow t \mathop; f(v))[\sigma] &=
  v \mathop\leftarrow t[\sigma] \mathop; f(v)[\sigma]
...
to interpret each operation $(m \mathop: N) \in E$.
By induction on $t \in T_E(X)$:
\begin{align*}
  \ul{v}[\phi] &= \eta^T_X(v) &
  \ul{m}(t_n)_{n \in N} [\phi]
    &= n \mathop\leftarrow \phi_N(c^m) \mathop; \phi_X(t_n)
  \\
    &= \phi(\ul{v}) &
    &= \phi_X(n \mathop\leftarrow c^m \mathop; t_n)
  \\
    &&
...
\[
  ...
  \,.
\]
The environment opens with a question $(m \mathop: N) \in F$.
...
%\]
%where
%$(m_F \mathop: N_F) \in F$ is a question of the environment,
%$(m_E \mathop: N_E) \in E$ is a question of the system,
...
\begin{table} % tbl:cats {{{
  ...
  \caption[A selection of categories relevant to my work]%
   {A selection of categories relevant to my work.
    ...
    or $[\otimes]$ when they are monoidal closed with respect
...
defines the correspondence between $f$ and $g$
...
a correspondence between the single morphisms of type
$1 \rightarrow [X, Y]$
...
\[
  \begin{array}{c@{\qquad}c}
    \ul{m}(t_n)_{n \in N}[L] \: = \:
      (n \mathop\leftarrow m \mathop; t_n)[L] \: = \:
...
\[
  C_\kw{rot} :=
    v \mathop\leftarrow \kw{deq} \mathop; \kw{enq}[v] \mathop; \eta(v)
\]
...
\[
  \bigoplus_{i \in I} E_i :=
    \{ \iota_i(m) \mathop: N \mid i \in I, (m \mathop: N) \in E_i \}
\]
...
%  \big\langle
%    \kw{enq}[v] \mapsto
%      (i \mathop\leftarrow \kw{inc}_2 \mathop; \kw{set}[i, v]),
%    \:\:
%    \kw{deq} \mapsto
%      (i \mathop\leftarrow \kw{inc}_1 \mathop; \kw{get}[i])
%  \big\rangle
%\]
%\end{example}
...
For each effect $(m \mathop: N) \in E$,
the interaction specification monad
...
a family $(f^m)_{(m \mathop: N) \in F}$ of computations
...
where each occurrence of an effect $(m \mathop: N) \in F$
...
\[
  s \in \bar{P}_E(A) ::=
    ...
    \ul{m} n s \,,
\]
where $v \in A$, $(m \mathop: N) \in E$ and $n \in N$.
...
or it can trigger an effect $(m \mathop: N) \in E$ and
...
$(f^m)_{(m \mathop: N) \in F}$
providing an interpretation $f^m \in \mathcal{I}_E(N)$
of each effect $(m \mathop: N) \in F$
...
and a family $(f^m)_{(m \mathop: N) \in F}$
...
whenever $(f^m)_{(m \mathop: N) \in F}$ is a family of interactive computations
...
\begin{align*}
  \ul{m}(t_n)_{n \in N}[\mathbf{I}_E] &:=
    n \mathop\leftarrow \mathbf{I}_E \mathop; t_n[\mathbf{I}_E]
    \in \mathcal{I}_E(N) &
    (m \mathop: N) &\in E
\end{align*}
...
The operations $(m \mathop: N) \in \Gamma(u, v)$ are called
...
For each question $(m \mathop: N) \in \Gamma(u,v)$
...
as follows.
For each operation $(m \mathop: N) \in \Gamma(u, v)$
