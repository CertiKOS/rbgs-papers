...
Like other components considered by the DeepSpec project,
CompCert was written, specified and verified
in the Coq proof assistant.

The key to this achievement
was the decomposition of CompCert
into compilation passes,
which were then specified an verified individually.
...
These \emph{layer interfaces}
enumerate the primitives implemented by the layer
...
\begin{figure}
  ...
  \caption{Structure of the certified OS kernel CertiKOS.
    ...
    and horiontal lines represent layer interfaces.
    The high-level user program $C$ is transformed
    into the complete low-level program $C + \kw{CertiKOS}$
    by composing it with the successive certified abstraction layers
    of CertiKOS.
...
are formalized by bottom layer interface $\kw{PreInit}$.
...
which allows components to be connected
...
is provided by \emph{game semantics} \citep{cspgs},
...
Game semantics is a form of \emph{denotation semantics},
providing compositional models for programming languages.
However,
...
shares many characteristics with the \emph{trace semantics}
often used for example in the context of process algebra
or concurrent semantics.
...
In particular,
the typed aspect of the resulting models
...
The programmer can then forget the details of their implementation,
...
the memory address space must be managed explicitly,
...
representing a computation which proceeds inward
from the top-level operation
towards the leaves of the term.
