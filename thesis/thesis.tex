...
\[
    \kw{run} \cdot
    \underline{\kw{read}_\kw{x}} \cdot n \cdot
    \underline{\kw{write}_\kw{x}[2n]} \cdot \kw{ok} \cdot
...
A term of the form $\underline{m}(t_i)_{i \in N}$
...
On the other hand,
terms of the form $\underline{v} \in X$ correspond to a computation
...
yielding the term $t \sigma \in T_E(Y)$
defined by:
\begin{align*}
  \underline{v} \sigma &:= \sigma(v) & v &\in X \\
  \underline{m}(t_n)_{n \in N} \, \sigma &:=
    \underline{m}(t_n \sigma)_{n \in N} & (m:N) &\in E
\end{align*}
\end{definition}
...
the substitution $\sigma : \{x, y\} \rightarrow \varnothing$
defined as:
\begin{gather*}
  t := \underline{\kw{readbit}}(x, y)
  \\
  \sigma := \{
    x \mapsto \underline{\kw{print}}[\text{\rm ``Hello''}](
                \underline{\kw{done}}), \:
    y \mapsto \underline{\kw{print}}[\text{\rm ``World''}](
                \underline{\kw{done}})
  \}
  \,.
\end{gather*}
Then the term
$t\sigma \in T_{E_\kw{io}}(\varnothing)$
obtained applying $\sigma$ to $t$ is again:
\[
  t \sigma = w =
    \kw{readbit}(
...
Indeed, the abstract syntax tree of our example term $w$
...
a corresponding funtion $m^A : A^N \rightarrow A$.
When equational theories are used,
we need to make sure that the corresponding equations hold.
In our limited setting we can use the simpler definition below.
...
$m^A : A^N \rightarrow A$
for each operation $m : N$ in $E$.
We can then interpret
an arbitrary term $t \in T_E(A)$
as $t^A \in A$
using the recursive definition:
\begin{align*}
  \underline{v}^A &:= v & v &\in A \\
  \underline{m}(t_n)_{n \in N}^A &:= m^A(t_n^A)_{n \in N}
    & (m : N) &\in E
\end{align*}
\end{definition}
%}}}
...
The corresponding algebra is defined over the set
$S = \mathcal{P}(P)$,
with operations evaluated as:
\begin{align*}
  \kw{readbit}^S(\sigma_0, \sigma_1) &:=
...
This allows us to interpret terms with variables
from an arbitrary set
by assigning values in the set $A$ to each one:
given a term $t \in T_E(X)$, and
a variable assignment $\rho : X \rightarrow A$,
we can easily construct the substitution
$\sigma(x) := \underline{\rho(x)}$
and interpret $t$ as $(t\sigma)^A \in A$.

However,
to be able to interpret open terms in a more systematic way
and independently of any assignment,
...
\begin{itemize}
  ...
  \item For every ``semantic substitution''
    $\sigma : X \rightarrow T(Y)$,
    a map $\sigma^\dagger : T(X) \rightarrow T(Y)$
...
is given as above,
and must satisfy
for all $\sigma : X \rightarrow T(Y)$ and
$\rho : Y \rightarrow T(Z)$
the following properties:
\[
  \eta_Y^\dagger \circ \sigma \, = \,
  \sigma^\dagger \circ \eta_X \, = \,
  \sigma
  \qquad \qquad
  (\rho^\dagger \circ \sigma)^\dagger \, = \,
  \rho^\dagger \circ \sigma^\dagger
  \,.
\]
\end{definition}
...
Moreover,
if we specify
a computation $m^T \in T(N)$
for each operation $m \mathop{:} N$
in a signature~$E$,
we can derive an $E$-algebra on $T(X)$
for every set $X$ of variables:
\[
  m^{T(X)}(\tau_n)_{n \in N} \: := \:
    n \mathop{\leftarrow} m^T \mathop{;} \tau_n
  \,.
\]
This algebra is compatible with the monad structure,
in the sense that for all $\sigma : X \rightarrow T(Y)$,
the following property holds:
\[
  \sigma^\dagger \left( m^{T(X)}(\tau_n)_{n \in N} \right) =
    m^{T(Y)} \left( \sigma^\dagger(\tau_n) \right)_{n \in N}
\]
In fact,
a ``parametric interpretation'' of $m$
satisfying this property
uniquely determines $m^T$ as:
\[
  m^T = m^{T(N)}(\eta_N(n))_{n \in N}
  \,.
\]

\begin{example}[Free monad for a signature]
The set of terms $T_E(X)$
for a signature $E$ and a set of variables $X$
can be extended to a monad.
For $v \in X$, $t \in T_E(X)$ and $\sigma : X \rightarrow T_E(Y)$,
we define:
\[
  \eta_X(v) := \underline{v}
  \qquad \qquad
  \sigma^\dagger(t) := t \sigma
\]
For each operation $m : N$ in the signature $E$,
we define:
\[
  m^{T_E} := \underline{m}(\underline{n})_{n \in N}
  \,.
\]
...
For example,
in Chapter~\ref{sec:intspec}
I will construct a monad which adds
additional structure to signatures in a systematic way,
borrowing ideas from game semantics.
The presentation I have given above also
reveals clearly the connections between signatures
...
[comparing with Def.~\ref{def:setmonad}:]
The usual categorical definition can be recovered
by taking $\mu_X := \kw{id}_{T(X)}^\dagger$
and $T(f) := (\eta_Y \circ f)^\dagger$.
