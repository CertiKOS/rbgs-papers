...
%the angel and the demon \citep{refcal}.
%
...
% XXX discuss the free monad more
...
\]
Terms of the form $v$ correspond to a computation
which immediately terminates with the corresponding results.
Terms of the form $m(x_i)_{i \in N}$,
where $m : N$ is an operation in the signature $E$,
first invoke the primitive $m$.
The outcome $n \in N$ then resumes the computation
...
and a relation $R \subseteq S_2 \times S_2$,
...
\subsection{Layer interfaces with common state}
