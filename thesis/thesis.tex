...
As a complementary construction,
we can give a semantics for the effects of $E$
in a semantic domain $A$
by defining an interpretation
$\underline{m}^A : A^N \rightarrow A$
for each operation of the signature.
When equational theories are used,
we need to show that the corresponding equations hold.
...
We can then defined the interpretation
of a term $t \in T_E(A)$
as an element $t^A \in A$
defined by:
\begin{align*}
...
As outlined in \S\ref{sec:freemon},
these concepts admit an elegant categorical formulation.
$T_E$ is a \emph{free monad}
associated with the signature $E$,
where the substitution operation
corresponds to Kleisli composition.
The unit $\eta_X : X \rightarrow T_E(X)$
of the associated adjunction
corresponds to the identity substitution,
embedding the variables as the corresponding terms.
The counit $\varepsilon_A : T_E(A) \rightarrow A$
evaluates a term for a given interpretation.
