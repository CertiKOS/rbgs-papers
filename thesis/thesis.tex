...
\newcommand{\scref}{\succeq} % simulation convention refinement
\newcommand{\figsize}{}
...
\begin{figure}
  ...
  \begin{tabular}{c@{\hspace{4em}}c}
    ...
    \hspace{4em} \raisebox{0.66ex}{\includegraphics[scale=0.11]{demon}}
    \\
    (a) Angelic choice &
    (b) Demonic choice
  \end{tabular}
...
in the language $P = (\mathbbm{2} \cup \kw{string})^*$.
...
\[
  \hat{E}_\kw{io} X \: := \:
    X \times X \: + \:
    \left( \sum_{s \in \kw{string}} X \right) \: + \:
    1
\]
...
for all families $(x_{i,j})_{i \in I, j \in J_i}$
of elements:
\[
...
%whereas my own work goes further.
...
%While it may be possible to accommodate \#\ref{req:openabs},
%the parameterization of simulations
...
%In the context of the verification of heterogeneous systems
...
\begin{table} % tbl:compcerts {{{
  ...
  \caption[Taxonomy of CompCert extensions]%
   {Taxonomy of CompCert extensions
    ...
    %(5)~minimal changes.
    %The CompCertO model
    %is parameterized by the languages interfaces $A$, $B$.
  }
  \label{tbl:compcerts}
\end{table}
%}}}
...
The behavior of \textsf{A.s} and \textsf{B.s},
...
A possible execution of \textsf{B.s}
...
$\mathbb{C} : \mathcal{C} \Leftrightarrow \mathcal{A}$
for the whole compiler, %which is both compositional
...
in the granularity at which Kripke worlds are deployed.
In previous work,
...
%we interpret \kw{ret} as a top-level return to the environment,
...
%    The new bottom can be thought of as ``global failure'',
