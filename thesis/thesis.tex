...
can be represented internally as an object of $\mathbf{C}$.
In cartesian closed categories,
this correspondance is expressed with respect to products.

\begin{example}[Cartesian closure in $\mathbf{Set}$]
Given two sets $B, C \in \mathbf{Set}$,
the set of function $\mathbf{Set}(B, C) = C^B$
is itself an object of $\mathbf{Set}$.
This means there exists a one-to-one correspondence
between the morphisms of type $B \rightarrow C$
and the global elements of $C^B$:
\[
  \mathbf{Set}(B, C)
  \: \cong \:
  \mathbf{Set}(1, C^B)
...
between:
\begin{itemize}
  \item the functor ${-} \times B$,
    which maps an object $X$ to the product $X \times B$,
    and $f : X \rightarrow Y$ to the morphism
    $f \times B : X \times B \rightarrow Y \times B$
    defined by
    $
      (f \times B)(x, b) := (f(x), b)
    $;
  \item the functor ${-}^B$,
    which maps an object $X$ to the function space $X^B$
    and $f : X \rightarrow Y$ to the morphism
...
\[
  ...
  \qquad \qquad
  \begin{array}{r@{{} = {}}l}
    f & \varepsilon_C \circ (g \times B) \\
    g & f^B \circ \eta_A
  \end{array}
\]
The adjunction's counit $\varepsilon_C : C^B \times B \rightarrow C$
is an \emph{evaluation map},
which takes a function $\phi \in C^B$ and an argument $b \in B$
and returns the value $\phi(b)$.
The unit $\eta_A : A \rightarrow (A \times B)^B$
takes a value $a \in A$ and returns a function
which constructs a pair $(a, b)$ from its argument $b \in B$.
\end{example}

The same structure can be obtained
in any cartesian category where the functor
${-} \times B$,
which can be defined in general
by taking $f \times B$ to be
$f \times \kw{id}_B = \langle f \circ \pi_1, \: \pi_2 \rangle$,
has a right adjoint.

\begin{definition}[Cartesian closed category]
An object $B$ in a cartesian category $\mathbf{C}$
is called \emph{exponentiable}
if there is a functor $-^B$ and a natural tranformation
$\varepsilon : -^B \times B \rightarrow \kw{id}_\mathbf{C}$
such that for all $f : A \times B \rightarrow C$,
there exists a unique morphism $f^\dagger : A \rightarrow C^B$
such that:
\[
  \begin{tikzcd}[row sep=huge, column sep=large]
    A \ar[d, "f^\dagger"', dashed] &
    A \times B
      \ar[d, "f^\dagger \times \kw{id}_B"']
      \ar[dr, "f"] &
    \\
    C^B &
    C^B \times B \ar[r, "\varepsilon_C"'] &
    C
  \end{tikzcd}
  \qquad \qquad
  f = \varepsilon_C \circ (f^\dagger \times \kw{id}_B)
  \,.
\]
We call $\varepsilon_C$ an \emph{evaluation map}
and the object $C^B$ an \emph{exponential object}.
