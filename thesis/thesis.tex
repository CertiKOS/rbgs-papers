...
other kinds of certified systems,
...
CompCert supports an extensive subset of ISO C99,
and CompCert can emit assembly code
for PowerPC, ARM, RISC-V and x86 processors.
However,
in the context of verification,
CompCert is usually used as compiler from
...
or as a tool
we can use to build certified assembly programs.

\subsection{CompCert as a certified system}

Until now,
I have mostly presented
CompCert as a specific example of a certified system,
discussing the compositional structures used in
...
Their generalization can be understood as
an effort to model real-world compiler usage
more accurately,
...
will execute in a way that conforms to
the C standard.
...
to model the execution the kernel's code
...
\begin{itemize}
  \item
    ...
    which can model individual tranlation units,
    this would give us the ability to
    interface program components at the granular level
  ...
  \item
    ...
    a specified by the C calling convention in use.
\end{itemize}
