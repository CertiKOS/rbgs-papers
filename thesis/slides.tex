...
\begin{frame}{CompCert} %{{{
  The introduction of the cerified compiler \emph{CompCert}
  (10--15 years ago) \\
  ...
  \[
    \begin{prooftree}
      \infer0[$\kw{Clight}$]{\module{\kw{SimplLocals}} }
      \infer1{\module{\kw{Cshmgen}} }
      \infer1[$\kw{Csharpminor}$]{\vdots}
  ...
  \]
  can be obtained by composing similar theorems for each pass.
\end{frame}
%}}}

\begin{frame}{CertiKOS} %{{{
  Our certified kernel CertiKOS uses a similar approach, \\
  decomposing the code into \emph{certified abstraction layers}:
  \[
    \begin{prooftree}
      \hypo{C}
...
\begin{frame}{Capturing patterns in layers} %{{{
  In CertiKOS, individual layers follow common patterns (\S\ref{sec:cal}):
  \begin{center}
    \small
    \begin{tabular}{lc@{\qquad}c}
      ...
      {\begin{prooftree}
        \hypo{\jg{L_1}{R}{M}{L_2}}
        \infer1{\forall C \bdot C[L_2] \le_R C[M][L_1]}
      \end{prooftree}} &
      \tiny
      \hspace{-1.5em}
      $C[-] \sqsubseteq C[-]$
