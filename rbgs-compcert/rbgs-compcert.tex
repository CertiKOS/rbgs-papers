...
Instead,
reserch has focused on
compositional reasoning methods
based on contextual notions of equivalence and refinement,
...
This problem posed by this example is illustrated in Fig.~\ref{fig:hetsys}.
Under existing contextual approaches,
the NIC driver can only be specified in terms of
its interactions at the boundary of the C program code,
but abstracting away from these interactions
...
%such as network services, file systems, \ldots{}
%or to use them in specific ways.
Proper high-level specifications
for software components of this kind
must model these resources.
Expecting the verification framework
to fix \emph{a priori}
the dozens or hundreads
of kinds of resources and
...
Fortunately,
advances in the realm of compositional semantics
make them a realistic alternatives
to tackle problems of this kind.
In particular,
...
to model the advanced features
...
However,
to allow explorations in this direction,
a necessary first step
is to formulate a correctness theorem for CompCert
which can survive the use of these methods.
...
the first compositional extension of CompCert
satisfying the following requirements simultaneously:
\begin{enumerate}
...
\item \label{req:openabs}
  ...
  interactions is modelled explicitly.
\item \label{req:linking}
  A form of certified \emph{linking}
  is provided in addition to certified compilation.
...
modelled by our transition systems
must be taken into account by our notions of simulation.
In CompCertO,
...
The simulation convention $\mathbb{C}$ is defined in \S\ref{sec:callconv}.
