\documentclass[sigplan,10pt,review,anonymous,authordraft]{acmart}
...
\newcommand{\CA}{{\mathcal{C} \!\mathcal{A}}}
...
The introduction %a decade ago
of %the certified compiler
CompCert \cite{compcert}
was a breakthrough
in the scale and feasability of
certified software.
Since then,
researchers have formally verified %been able to formally verify
...
The most straightforward sense in which
a compiler can be correct is a
\emph{semantics preservation} property of the form
\[
  \llbracket p \rrbracket_\kw{S} \sim
  \llbracket \kw{C}(p) \rrbracket_\kw{T}
  \,,
\]
asserting the semantics $\llbracket p \rrbracket_\kw{S}$
of the source program $p$
to be related in some sense $\sim$
to the semantics $\llbracket \kw{C}(p) \rrbracket_\kw{T}$
of the target program $\kw{C}(p)$ emitted by the compiler $\kw{C}$.

For whole-program compilers,
semantics preservation is simple enough.
In CompCert,
...
However,
in practice compilation usually operates on program \emph{components}
rather that whole programs.
In this context,
formulating semantics preservation directly
is challenging.
The semantics itself must be compositional
and account for the intricacies
of linking and cross-component interactions.
Worse,
these interactions are often very different
between the source and target languages,
so the relation $\sim$ must bridge
a significant abstraction gap.
Given these difficulties,
common wisdom holds semantics preservation
to be a lost cause
for compositional compiler correctness.

Instead,
reserch in the area has mainly focused on
compositional reasoning methods
based on contextual equivalence and refinement,
formulated by reference to (whole-program)
operational semantics,
side-stepping the need for a compositional semantics
\cite{next700}.
This approach has been successfully applied
to CompCert, turning it
...
its horizon is a completed assembly program to be run as a user-level process.
Interactions between the completed program and its environment
are expressed through event traces,
but compositionality breaks at the boundary between
the completed program and the rest of the system.
This becomes a limitation
when we seek to use the resulting compiler
in the construction and verification
of complex heterogenous systems.
...
\begin{figure}
  ...
  \caption{A futuristic verification scenario.
    Contextual approaches to compositional reasoning
    impose a boundary between
    the completed program and its environment;
    compositionality operates only within this boundary.
    However,
    the verification of large-scale, heterogenous systems
    requires modelling components which cross this boundary
    in various ways and at various levels of abstraction.}
  \label{fig:hetsys}
\end{figure}
...
The example above is by no means contrived,
and the pattern is pervasive even in more mundane situations.
Indeed,
\emph{most} libraries available to contemporary programmers
are used to mediate access to external
network services, file systems, user interfaces, \ldots
or use them in specific ways.
Proper high-level specifications
of software components of this kind
must model the resources they access

To be sure,
given a specific situation
such as the one described in Example~\ref{ex:nicdriver},
existing techniques could be extended to
bring the NIC hardware model
within the scope of the ``whole program'' being considered,
and the exchange of Ethernet packets
as part of its observable behavior.
However, this approach does not scale.
[explain why:
there are hundreads of such situations involved in verifying
a large-scale heterogenous system;
this subordinates everything to the compiler correctness theoerm
when it should just be one component;
we seek to build an extensible framework once and for all;
what happens when the NIC manufacturer,
database engine vendor,
network service provider
all give us their own modified version of CompCertM]

Scaling up verification requires
giving an account of the operation
of such components
directly in terms of their interactions with
the resources they access.

[A satisfactory formal account of
the functionality of such libraries
\emph{cannot} be formulated in terms of the interaction
of a postulated ``completed program'' with its environment
(say, the operating system),
because this interaction is precisely the thing
that such libraries seek to abstract.]

[We need a compositional semantics there's no way around it]
...
to relate the stack within
assembly questions to that in the corresponding answers.
...
\[
  ...
  \AxiomC{$\vec{v} \sqsubseteq \mathit{rs}[\kw{ebx}, \kw{ecx}]$}
  \BinaryInfC{$
    x \vec{k} \Vdash
...
We can now present our result and some of its consequences.
...
The simulation convention $\mathbb{C}$ is defined in \S{}X.Y.
It formalizes the correspondance between C and assembly interactions.
By necessity,
it follows $\kw{Clight}$ and $\kw{Asm}$ in its use of
the CompCert memory model,
but is otherwise independent of the compiler
and in particular is not sensitive to the inclusion of optional optimization passes.

\subsection{Compositional compilation and verification} \label{sec:cver} %{{{

While the main focus of this paper is to
the present techniques we used to obtain this result,
we first outline some of the ways in which
it can be used to carry out compositional reasoning
in the presence of compilation.
...
Additionally,
suppose we wish to verify that the composite program
satisfies a specification $\Sigma$,
...
\paragraph{Granular use of $\mathbb{C}$}

In addition to the high-level principles outlined above,
Theorem~\ref{thm:compc} enables low-level reasoning
about the interaction between $\kw{Asm}$ code
and components implementing $\mathcal{C}$-style specifications
(including assembly modules compiler from $\kw{Clight}$ code),
and conversely between $\kw{Clight}$ code
and components implementing $\mathcal{A}$-style specifications,
(including hand-written assembly
which may offer some guarantees to a $\mathcal{C}$ caller, or none).
Indeed,
the simulation convention $\mathbb{C}$
serves as a formalization of the C calling convention
and precisely capture the guarantees that a C compiler must provide
at the granularity of individual calls.
...
$\Sigma_\mathcal{C}
 \le_{\mathbb{C} \twoheadrightarrow \mathbb{C}}
 \Sigma_\mathcal{A}$
related by the simulation convention $\mathbb{C}$.
We will first consider
what we can learn about the interaction
$\kw{Asm}(p) \oplus \Sigma_\mathcal{A}$
of an arbitrary assembly program $p$ with $\Sigma_\mathcal{A}$,
using only $\Sigma_\mathcal{C}$
and its relationship with $\Sigma_\mathcal{A}$.

To show that the combination admits a certain behavior
when $p$ calls into $\Sigma_\mathcal{A}$ with a query $m_\mathcal{A}$,
we may choose any world and related query
$w \Vdash m_\mathcal{C} \mathrel{\mathbb{C}^\que} m_\mathcal{A}$,
establishing that $p$ performed the call
in accordance with the C calling convention in use.
As we unroll the behavior of $\Sigma_\mathcal{C}$ on $m_\mathcal{C}$,
the simulation property ensures that
for any eventual answer $n_\mathcal{C}$ produced by $\Sigma_\mathcal{C}$,
there exists a related answer
$w \Vdash n_\mathcal{C} \mathrel{\mathbb{C}^\ans} n_\mathcal{A}$,
allowing us to deduce that the execution of $p$ eventually resumes.
The answer $n_\mathcal{A}$ is not completely determined,
since for instance $\Sigma_\mathcal{A}$ is free to allocate
new blocks in the target memory
and leave caller-save registers in an abitrary state,
however its relationship with $w$ and $n_\mathcal{C}$
captures all of the guarantees provided by the C calling convention.

Conversely,
consider the combination $\kw{Clight}(p) \oplus \Sigma_\mathcal{C}$
and what we can learn of the interaction between $p$ and $\Sigma_\mathcal{C}$
based on the latter's relationship with $\Sigma_\mathcal{A}$.
To show that the combination admits a certain behavior when
$p$ calls into $\Sigma_\mathcal{C}$ with a query $m_\mathcal{C}$,
we must show this behavior hold for \emph{all} possible choices of
$w$ and $m_\mathcal{A}$ such that
$w \Vdash m_\mathcal{C} \mathrel{\mathbb{C}^\que} m_\mathcal{A}$,
knowing that if $\Sigma_\mathcal{A}$ eventually returns with 
...
$\mathbb{C} : \mathcal{C} \Leftrightarrow \mathcal{A}$
used in the formulation of Theorem~\ref{thm:compc}.
...
\[
  \mathbb{C} :=
    ...
    \kw{injp}_{\!\mathcal{A}}
\]
The first component ensures
that the source is well-behaved
with respect to various memory relations,
namely:
\[
  \mathcal{R} := \kw{injp} + \kw{ext} + \kw{wt} + \kw{va}
\]
The component
$\kw{injp}_{\!\mathcal{A}}$
allows the assembly execution
to allocate stack frames
and guarantees the preservation by external calls
of their assembly-specific regions.
Finally,
The inner component
$\mathcal{C} \!\mathcal{A}$
...
The simulation convention
${\mathcal{C} \!\mathcal{A}} :
 \mathcal{C} \Leftrightarrow \mathcal{A}$
is defined as follows:
\begin{align*}
  {\mathcal{C} \!\mathcal{A}} &:=
    \langle
      W_{\mathcal{C} \!\mathcal{A}},
      {\mathcal{C} \!\mathcal{A}}^\que,
      {\mathcal{C} \!\mathcal{A}}^\ans
    \rangle
    \\
  W_{\mathcal{C} \!\mathcal{A}} &:=
    \kw{signature} \times
...
\[
  \AxiomC{$
   {\begin{array}{c}
      \kw{WB}(\mathit{sg}, \vec{v}, \bar{m}) \\
      \mathit{vf} = \mathit{rs}[\kw{pc}] \\
      \vec{v} = \kw{args}(\mathit{sg}, \mathit{rs}, m) \\
      \bar{m} = \kw{drop\_args}(\mathit{sg}, \mathit{rs}, m)
    \end{array}}
  $}
  \UnaryInfC{$
    (\mathit{sg}, \mathit{rs}) \Vdash
    \mathit{vf}[\mathit{sg}](\vec{v})@\bar{m}
    \mathrel{\CA^\que}
    \mathit{rs}@m
  $}
  \DisplayProof
\]
\[
  \AxiomC{$v' = \kw{retval}(\mathit{sg}, \mathit{rs}')$}
...
The predicate $\kw{WB}$
ensure that the arguments $\vec{v}$ are well-typed
with respect to the signature $\mathit{sg}$,
and that global constants have their prescribed values
in the source memory $\bar{m}$.
The function $\kw{args}$ extracts
the value of arguments from the assembly query.
Last but not least,
$\kw{drop\_args}$ ensures that
the regions of the memory which encode the arguments
at the assembly level
are inaccessible to the source execution.

\subsection{Incoming calls}

To establish that incoming calls
performed according to $\mathbb{C}$
are compatible with the various passes of CompCertO,
we rely on the following property.

\begin{lemma} \label{lm:incoming}
Any simulation convention $\mathbb{R} = \mathbb{R}_1 \cdots \mathbb{R}_n$
such that $\mathbb{R}_i \in \{ \kw{ext}, \kw{inj}, \kw{wt}, \kw{va} \}$
satisfies the following property:
\[
  \CA \cdot \kw{injp} 
  \scref
  \kw{injp} \cdot \mathbb{R} \cdot \kw{alloc} \cdot \kw{ext} \cdot \kw{stacking} \cdot \kw{asmgen} \cdot \kw{injp}
\]
\begin{proof}
For a memory injection $f$,
defined the injection $\kw{dom}(f)$
which maps every block $b$ in the domain of $f$
to itself with no offset shifting.
The following property is key to the proof:
\[
  f \Vdash m \hookrightarrow m' \quad \Rightarrow \quad
  \kw{dom}(f) \Vdash m \hookrightarrow m
\]
This allows us to expand $\CA$ by using the corresponding memory state
in both the source and target queries for all of CompCertO's passes.
For replies, the injection and extension properties provided by the passes
can be composed back into a single injection,
and the preservation properties required by the left-hand side $\kw{injp}$
can be obtained by the outer $\kw{injp}$ properties on the right-hand side.
For details,
see \texttt{driver/CallConv.v}.
\end{proof}
\end{lemma}

The passes of CompCertO can be extended with
self-simulation properties of \kw{Clight} and \kw{Asm}
to provide the $\mathcal{R}^*$ component of $\mathbb{C}$,
as well as the outermost $\kw{injp}$ components required by Lemma~\ref{lm:incoming}.

\subsection{Outgoing calls}

For most passes,
the requirements on external calls
can be satisfied by the $\mathcal{R}^*$ component of $\mathbb{C}$.
To bridge the gap for the remaining passes,
we use the following lemma.

\begin{lemma}
The following convention refinement holds:
\[
  \kw{alloc} \cdot \kw{ext} \cdot \kw{stacking} \cdot \kw{asmgen}
  \scref
  \kw{ext} \cdot \kw{ext} \cdot \kw{injp} \cdot \kw{ext} \cdot \CA
\]
\begin{proof}
The proof essentially works by converting
the intermediate $\mathcal{L}$ and $\mathcal{M}$ queries
into $\mathcal{C}$ queries by extracting function arguments
in the appropriate language-specific way.
The main difficulty is then to reconstruct
appropriate $\mathcal{L}$ and $\mathcal{M}$ replies
from the $\mathcal{C}$ replies obtained from the right-hand side;
in particular,
to reconstruct the location and register states,
we must use an aggregate of the original callee-save registers
and store the $\mathcal{C}$ return value in the appropriate locations,
leaving all others with the value $\kw{undef}$.
\end{proof}
\end{lemma}
...
and some of the consequences of ...
...
\begin{equation} \label{eqn:asmadeq}
  ...
  \,,
\end{equation}
similar in its role to Theorem~\ref{thm:asmlinking}.
...
rendering the proof of Theorem~\ref{thm:asmlinking} straightforward.
Secondly,
...
$\kw{injp} \twoheadrightarrow \kw{inj}$
for the passes $\kw{SimplLocals}$ and $\kw{Inlining}$
and to avoid the corresponding modifications
to the proofs found in CompCert.

%}}}

\subsection{Conclusions for CompCertO}

\subsection{Conclusions for CompCertM}
