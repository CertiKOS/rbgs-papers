...
\begin{figure} % fig:compc {{{
  ...
  \caption{Overall structure of the proof of \autoref{thm:compc}.
    ...
    denote parametricity properties (\autoref{thm:param}).
    The simulation proof for frontend passes,
    depicted as ``\ldots'' in the top part of the figure,
    can be constructed incrementally (see also \autoref{fig:incrcomp}).
    The other ellipsis denotes the passes
    $\kw{Linearize}$, $\kw{CleanupLabels}$ and $\kw{Debugvar}$,
    which use identity simulation conventions
    and can thus be composed without complication.
    }
  \label{fig:compc}
\end{figure}
%}}}
