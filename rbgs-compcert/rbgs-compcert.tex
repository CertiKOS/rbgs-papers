...
% Whether to show optional content
\newif\ifopt
\opttrue
...
and establish the following correcntess property.
...
invoking a function compiled by CompCertO
...
\subsection{External calls in injection passes} \label{sec:pass:inj} %{{{
...
if we show that the semantics of a given language
preserves a given invariant,
then the associated properties can be used to assist
in proving forward simulations
using the language as their source.
...
but since they are formulated in terms of
Def.~\ref{def:fsim},
the standard forward simultion techniques
defined by CompCert
...
\section{Specialized simulation conventions} \label{sec:passes} %{{{
...
This concludes the exposition of the techniques
used in CompCertO.
In this section,
we discuss the significance of our result.
To place them in context,
we first summarize some related work.
