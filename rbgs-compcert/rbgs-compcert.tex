...
%   \hline
    ...
    %\st{Unusedglob} \\
    \kw{Allocation} &
      $\kw{wt} \cdot \kw{ext}$ & $\kw{wt} \cdot \kw{ext}$ &
      +13 & (+0\%) \\
    \hline
    \textbf{LTL} & $\mathcal{C}$ & $\mathcal{C}$ & +15 & (+6\%) \\
    \kw{Tunneling} & $\kw{ext}$ & $\kw{ext}$ & +2 & (+0\%) \\
    \kw{Linearize} & \kw{id} & \kw{id} & -15 & (-3\%) \\
    \hline
    \textbf{Linear} & $\mathcal{C}$ & $\mathcal{C}$ & +16 & (+7\%) \\
    \kw{CleanupLabels} & \kw{id} & \kw{id} & -10 & (-4\%) \\
    \kw{Debugvar} & \kw{id} & \kw{id} & -12 & (-3\%) \\
    \kw{Stacking} & $\kw{wt} \cdot \kw{injp}$ & $\kw{wt} \cdot \mathcal{C\!M} \cdot \kw{inj}$ & +291 & (+11\%) \\
    \hline
    \textbf{Mach} & $\mathcal{C}$ & $\mathcal{M}$ & +100 & (+26\%) \\
    \kw{Asmgen} & $\mathcal{C\!A} \cdot \kw{ext}$ & $\mathcal{M\!A} \cdot \kw{ext}$ & +179 & (+6\%) \\
    \hline
  ...
  \caption{Passes of CompCertO
    grouped by their source language.
    ...
    self-simulation properties are inserted as identity passes (-).
...
and allows the caller to 
use various simulation conventions
in the set
$
...
\item
The components \kw{va} and \kw{wt}
do not transform the questions and answers,
but ensure that the arguments of the C-level call are well-typed
...
and guarantees the preservation
...
We first need to establish that incoming calls
performed according to $\mathbb{C}$
are compatible with the incoming convention
obtained from the sequence of passes
outlined in Table~\ref{tbl:passes}.
We rely on the following property
to expand the rightmost components of $\mathbb{C}$.
...
\[
  \kw{va} \cdot \kw{wt} \cdot \CA \cdot \kw{injp}
  \scref
  \kw{injp} \cdot \mathbb{R} \cdot \kw{wt} \cdot
  \mathcal{C\!M} \cdot \kw{inj} \cdot \mathcal{M\!A} \cdot \kw{ext} \cdot \kw{injp}
\]
\begin{proof}
...
\]
This allows us to expand the left-hand side
by using a single self-injecting memory state
...
For outgoing calls,
the simulation convention of all passes
up to \kw{Asmgen}
can be folded into the $(\sum \mathcal{R})^*$
component of $\mathbb{C}$.
The only remaining issue is the $\kw{ext}$
component of \kw{Asmgen}.
Fortunately,
we can use the following property:
\[
  \mathcal{C\!A} \cdot \kw{ext}
  \scref
  \kw{ext} \cdot \mathcal{C\!A}
\]
For questions,
we must remove the arguments region
of the intermediate memory state.
For replies,
we must reconstruct
an intermediate
register state
from a combination of:
1. the callee-save registers in the initial
intermediate register state and
2. the remaining registers
