...
(\S\ref{sec:sem:ref}).
To characterize CompCert's languages and simulation proofs,
...
to derive our main compiler correctness statement
...
A technical report \cite{compcerto21tr}
provides supplementary appendices.
...
We will write $L : A \twoheadrightarrow B$
when the transition system $L$ describes a strategy
for the game
...
We will use these notations extensively.
They are summarized in \autoref{tbl:notations}.
...
The register $\kw{eax}$ may contain an arbitrary value.
...
simplify the adaptation of the correctness proofs
of the various passes of CompCert.
...
\begin{figure} % fig:injp {{{
  ...
  \begin{tikzpicture}[scale=0.35,>=stealth]
    ...
    \draw[fill=lightgray] ( 3,4) rectangle +(2,1);
    \draw ( 6,4) rectangle +(3,1);
    \draw[fill=lightgray] (10,4) rectangle +(1,1);
    \draw[fill=lightgray] ( 0,0) rectangle +(8,1);
    \draw[fill=lightgray] ( 9,0) rectangle +(2,1);
