...
\title{%CompCertO:
  Compiling Certified Open C Components} %{{{
...
(network services, file systems, \ldots)
...
make them a realistic approach
...
are depicted in Fig.~\ref{fig:fsim}.
...
described in Fig.~\ref{fig:fsim}(c).
...
as depicted in Fig.~\ref{fig:simcomp}.
\end{theorem}
%}}}
...
formalizes the correspondance between C and assembly interactions.
By necessity,
...
and some further applications of Thm.~\ref{thm:compc}
...
%   \hline
  ...
  \caption{Languages and passes of CompCertO.
    ...
    In addition to compilation passes,
...
the properties given in Fig.~\ref{fig:cklr-def}.
Note that $R^\kw{mem}$
...
Note that in our approach to simulation conventions,
...
in our use of %the granularity at which we deploy
Kripke worlds.
In previous work,
...
This happens for example in \kw{SimplExpr}
when the source programs accesses temporified local variables.
Because the corresponding memory blocks
are allocated \emph{after} the function has been called,
they are not constrained by $\kw{injp}$
with reference to the original world.
...
\[
  \AxiomC{$
   {\begin{array}{c}
      \kw{WF}(\mathit{sg}, \vec{v}, \bar{m}) \qquad
      \vec{v} = \kw{args}(\mathit{sg}, \mathit{rs}, m) \\
      \mathit{vf} = \mathit{rs}[\kw{pc}] \qquad
...
are spelled out in Fig.~\ref{fig:hcomp}.
...
\section{Development and proofs}
...
\begin{table*}
  ...
  \hline
  \end{tabular}
  \caption{Proofs included in the Coq development.}
\end{table*}
