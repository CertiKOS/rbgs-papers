...
\begin{optional}
  \titlenote{%
    ...
    Yale University Technical Report YALEU/DCS/TR-1556
    \cite{compcerto21tr}
...
%to formalize language semantics of the various languages involved.
...
$\mathbb{R}_A : A_1 \Leftrightarrow A_2$ and
$\mathbb{R}_B : B_1 \Leftrightarrow B_2$.
If incoming questions %of $B_1$ and $B_2$
...
\begin{figure} % fig:injp {{{
  ...
  \begin{tikzpicture}[scale=0.35,>=stealth]
    ...
    \draw[fill=black\filltint] ( 3,4) rectangle +(2,1);
    \draw ( 6,4) rectangle +(3,1);
    \draw[fill=black\filltint] (10,4) rectangle +(1,1);
    \fill[black\filltint] (0,0) rectangle +(1,1);
    \fill[black\filltint] (3,0) rectangle +(1,1);
    \fill[black\filltint] (7,0) rectangle +(1,1);
    \draw (0,0) rectangle +(8,1);
    \draw[fill=black\filltint] ( 9,0) rectangle +(2,1);
    \draw[->] (0,4) -- (1,1); \draw[dotted] (2,4) -- (3,1);
    \draw (1,1) -- (1,0) (3,1) -- (3,0);
    \draw[->] (6,4) -- (4,1); \draw[dotted] (9,4) -- (7,1);
    \draw (4,1) -- (4,0) (7,1) -- (7,0);
    %
...
%a nondeterministically chosen number of
...
and the associated pass correctness proofs
...
However, as discussed in \S\ref{sec:ccinv},
we can define CKLRs which embed $\kw{va}$,
allowing it to propagate across language boundaries.
...
To express the simulation convention used by \kw{Allocation},
...
uses its world to remember %the initial location map and
...
\kw{LTL} and \kw{Linear} store in abstract stack locations
is consolidated by the \kw{Stacking} pass
...
\kw{Stacking} uses the CKLR \kw{injp}
for outgoing calls.
...
ensuring their protection by \kw{injp}.
By contrast,
...
Note that because of the complexity of the \kw{Stacking} pass,
...
\begin{remark}
Operations like \kw{free\_args} are used
...
The required \kw{Stacking} invariants can be established
from this side-condition,
however this
significantly increases the size and complexity of the proof.

In CompCertM,
a \kw{free\_args} operation is used prior to external calls
in the semantics of \kw{Mach} and \kw{Asm}:
