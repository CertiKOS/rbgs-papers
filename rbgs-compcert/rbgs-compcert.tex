...
either by developing a general notion of correctness
covering all passes
...
\[
  ...
  \AxiomC{$\vec{v} \text{ is a prefix of } \mathit{rs}[\kw{\%ebx}, \kw{\%ecx}]$}
  \BinaryInfC{$
    x \vec{k} \Vdash
    f(\vec{v}) \mathrel{\mathbb{L}^\que} \mathit{rs}@ x \vec{k}
  $}
  \DisplayProof
  \quad
  ...
  \BinaryInfC{$
    x \vec{k} \Vdash
    v' \mathrel{\mathbb{L}^\ans} \mathit{rs}@\vec{k}
  $}
  \DisplayProof
\]
...
Likewise,
the stack $x \vec{k}$ has no corresponding components in the C question,
...
\cite{compcompcert}.
Our framework
allows us to express this situation by using
...
\emph{horizontal} compositionality (Thm.~\ref{thm:fsim-hcomp}),
...
These properties are formulated as self-simulations
...
which states that a simulation convention
can be replaced by another in all simulation statements.
We construct a typed Kleene algebra \cite{tka}
involving this partial order and
the composition $\mathbb{R} \cdot \mathbb{S}$
of simulation conventions.
We then use this Kleene algebra to ensure that
our compiler correctness statement
uses a simulation convention which is both
compositional,
and insensitive to the inclusion of optional optimization passes.
...
\begin{table} % tbl:passes Passes of CompCertO %{{{
  ...
  \caption{Languages and passes of CompCertO.
    ...
    The framework used to express
    language semantics and pass correctness properties
    is explained in \S\ref{sec:sem}.
    ...
    (Thm.~\ref{thm:param}),
    allowing us to rewrite the compiler's overall simulation convention
...
The first step is to compose
the simulations of successive passes.
...
is more general than the convention $\mathbb{R}$,
so that any simulation accepting $\mathbb{S}$ as its
incoming convention will accept $\mathbb{R}$ as well.
...
which describes how a given invariant impacts the queries and replies
...
are families of predicates respectively on $M_A^\que, M_A^\ans$
indexed by $W$.
...
Then an invariant
$\mathbb{P} = \langle W, \mathbb{P}^\que, \mathbb{P}^\ans \rangle$
can be propoted to a simulation convention:
$\hat{\mathbb{P}} :=
 \langle W, \hat{\mathbb{P}}^\que, \hat{\mathbb{P}}^\ans \rangle$.
\end{definition}

As simulation conventions,
invariants satisfy a number of interesting properties.
For instance,
two invariants $P, Q$ always commute with each other,
so that $P \cdot Q \equiv Q \cdot P$.
Invariants can be duplicated
so that $P \sqsubseteq P \cdots P$.
Note that the converse relation does not hold
because the worlds used in the different copies of $P$
may be inconsistent.
In addition,
in some cases simulation conventions can propagate certain invariants.
For example, in the case of $\kw{wt}_\mathcal{C}$,
any simulation convention $\mathbb{R}$ consisting of
a non-empty sequence of CKLRs satsifies the property
$\mathbb{R} \cdot \kw{wt} \equiv
 \kw{wt} \cdot \mathbb{R} \cdot \kw{wt}$.
