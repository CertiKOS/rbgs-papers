...
\subsection{Simulation convention algebra} %{{{
...
[Need to give a transition and explain how this relates
to simulation convention algebra:
zooming in to the specific techniques that we use
to establish some of the properties we need,
especially self-simulation under
simulation conventions of interest,
compositional definitions of sim. conv. like
$\kw{injp}, \kw{ext}$
and how parametricity fits in the picture]
...
In this framework, (\ref{eqn:monoidrel}) can be reformulated as:
\[
...
\begin{figure} % fig:hcomp {{{
  ...
  \begin{tikzpicture}[xscale=0.5,yscale=0.7,
    ...
    \end{scope}
    \tiny
    \node[below right] at (I) {$\kw{in}$};
    \draw (L1) -| (-1,0);
    \draw (L2) -| (-1,0) -- (X);
    \draw[->] (X) -- node[below] {$\kw{out}$} (-3,0);
...
Roughly speaking,
whenever a new component instances are created
...
The $\kw{Selection}$, $\kw{Allocation}$, and $\kw{Stacking}$ passes
rely on typing invariants,
whereas the $\kw{Constprop}$, $\kw{CSE}$, $\kw{Deadcode}$
rely invariants tracked using CompCert's value analysis framework.
These invariants must be maintained by externall calls.
In CompCert,
...
\begin{tikzcd}[execute at end picture={
    ...
    \draw (node cs:name=I3,anchor=center) circle;
    \draw[dashed] (node cs:name=I4,anchor=center) circle;
    \end{scope}
  }]
%
  |[alias=I1]| q_1 \ar[r, "I"] &
