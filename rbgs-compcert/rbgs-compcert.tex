  ...
  \begin{tikzpicture}[xscale=1.75,yscale=0.9]
    ...
    \begin{pgfonlayer}{tint}
      \begin{scope}[every node/.style={black}]
        \footnotesize
        \fill[ACMYellow!40] (ULC) -- (Spec) -- (Def)
          to (NIC2) -| (ULC);
        \fill[ACMGreen!50]
...
which was designed specifically for the verification of CompCertO,
could adequately capture
the complexity of communication with hardware
or the concurrency inherent in composite systems
of the kind discussed in Example~\ref{ex:nicdriver}.
However,
