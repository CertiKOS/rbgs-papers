...
[XXX highlight more technical novelties /
ket point /
secret ingredients]
...
\begin{color}{blue}
\subsection{Discussion: horizontal composition and Kripke worlds} %{{{
...
the challenge is then to construct sets of worlds,
...
\[
  q\ph{qh} \cdot
    ...
    \pt{s1}s_2\ph{s2} \cdot
    \pt{s2}m}\ph{m} \cdot
    \pt{m}s_1'\ph{s1'} \cdot
    \pt{s1'}s_2'\ph{s2'} \cdot
    \pt{s2'}n\ph{n} \cdot
    \pt{n}\underline{s_3 \cdots
    s_k\ph{sn} \cdot
    \pt{sn}r}
\]
In our open simulations,
worlds are deployed in a way which
follows the structure of plays,
...
composite simulations work by
maintaining a stack of worlds for
all pending activations.
...
pseudo-registers used by the source program
...
in the target memory between
the injected source memory
and spilling locations.
