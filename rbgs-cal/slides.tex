...
\begin{frame}{Why this gap?} %{{{
  ...
  \pause
  Things we \emph{don't} need:
  \begin{itemize}
    \item Precise characterization of language features
    \item High-order computation
  \end{itemize}
\end{frame}  
%}}}
...
\begin{frame}{Refinement is correctness preservation} %{{{
  ...
  \pause
  Dual nondeterminism becomes a powerful \emph{specification} construct:
  \onslide<2->%
  \only<1-2>{
    \begin{gather*}
      \fbox{$C_1 \sqcup C_2$}
...
\begin{frame}{Game semantics and alternating refinement} %{{{
  Game semantics for nondeterminism can use the polarity of moves:
  \begin{gather*}
  ...
  \end{gather*}
  \pause
  The resulting refinement orderings are complicated to describe:
  \begin{align*}
    \{ 0 \mapsto 0 \}
    \: &\sqsubseteq \:
    \{ 0 \mapsto 0, \: 1 \mapsto 1, \: 1 \mapsto -1 \}
...
\begin{frame}{Game semantics and dual nondeterminism} %{{{
...
\pause
Strategies can be represented as sets of plays, \\
ordered by inclusion ($\subseteq$).

\pause
Strategy specifications can be represented as sets of strategies, \\
...
\pause
This construction can be used to add
dual nondeterminism to any poset.
\end{frame}
%}}}
...
%A simple game model is sufficient to describe low-level components. \\

  \begin{definition}[Games]
  We use signatures of the form:
  \[
  ...
  \begin{example}[Bounded queue]
    A queue with elements in $V$ has two operations:
    \[
      \kw{enq} : V \rightarrow \mathbbm{1} \qquad
...
\begin{example}[Queue rotation]
  The play:
  \[
    \underline{\kw{deq}} \cdot 5 \cdot
    \underline{\kw{enq}[5]} \cdot * \cdot
    \underline{5}
  \]
  can be depicted as:
  \[
...
\begin{frame}{Interaction specifications} %{{{
  ...
  \begin{example}[Queue rotation]
    We can express $\kw{rot}$ as:
    \[
      \kw{rot} \in \mathcal{I}_{E_\kw{bq}}(V) :=
        \bigsqcup_{v \in V}
          \, \kw{deq} \cdot v \cdot \kw{enq}[v] \cdot * \cdot v
    \]
  \end{example}
\end{frame}
%}}}
...
\begin{frame}{Two-sided strategies} %{{{
  ...
  This represents a two-sided strategy which:
  \begin{itemize}
    \item uses $E$ for \emph{outgoing} calls;
    \item uses $F$ for \emph{incoming} calls.
  \end{itemize}

  \includegraphics[scale=0.075]{mor}
\end{frame}
%}}}
  ...
  \[
    E@S :=
      \{ m@k : \kw{ar}(m) \times S \mid
         m \in E, k \in S \}
  \]
  These ingredients allow us to represent certified layers:
  \[
      L_1 \vdash M : L_2 \qquad \qquad \qquad
      L_2 \sqsubseteq R_*^{E_2} \circ M@S_1 \circ L_1
  \]
  \[
    \begin{prooftree}
      \hypo{C}
    ...
    \begin{tikzcd}[sep=tiny]
      E_2@S_2 & & E_2@S_1 \ar[ll, "R_*^{E_2}"'] & E_2 \\
      & \sqsubseteq & & \\
      \varnothing \ar[uu, "L_2"] \ar[rr, "L_1"'] & &
      E_1@S_1 \ar[uu, "M@S_1"', dashed] & E_1 \ar[uu, "M"']
    \end{tikzcd} 
  \]
