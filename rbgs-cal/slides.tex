...
\[
  0 \mapsto 0 \pause \:\sqsubseteq\:
  0 \mapsto 0 \:\sqcap\: 0 \mapsto 4 \pause \:\sqsubseteq\:
  \bigsqcap_{y \text{ even}} (0 \mapsto y) \:\sqsubseteq\: f
\]
\pause
\[
...
Dual nondeterminism is captured by the $\mathbf{FCD}$ monad.
...
Combine dual nondeterminism and game semantics:
\begin{itemize}
  \item Single play: ``if environment does $x$ then system does $y$''
  \item Strategy: angelic determinism adds environment choices
  \item Strategy specification:
    demonic nondeterminism adds system choices
\end{itemize}

Strategies are usually represented as sets of plays. \\
Strategy specifications can be represented as sets of stratgies.
\end{frame}
%}}}
