...
\]
The specification $(x \mapsto y)$ will be understood as:
\begin{quotation}
  Whenever the input is $x$, the output must be $y$
\end{quotation}

The function $f(n) := 2n$ statisfies the specifications:
\[
  \begin{array}{c@{\quad}c}
  (1 \mapsto 2) \sqcup (-1 \mapsto -2) &
  (0 \mapsto 0) \\[1em]
  (1 \mapsto 2) \sqcap (1 \mapsto -2) &
  \bigsqcup_{n \text{ odd}} \: \bigsqcap_{m \text{ even}} \: (n \mapsto m)
  \end{array}
\]
\end{frame}
%}}}
...
\begin{frame}[fragile]{Dual nondetermimism and data abstraction} %{{{
  ...
  as pairs of natural numbers $n = (n^+, n^-) \in \mathbb{N}^2$:
  \[
      x \mathrel{R} (n^+, n^-) \: \Leftrightarrow \:
      x = n^+ - n^-
  \]
  Then
  ...
  \[
    \begin{tikzcd}
      x \ar[r, "f"] \ar[d, "R"', dash] &
      y \ar[d, "R", dash, dashed] \\
      (n^+, n^-) \ar[r, "g"', dashed] &
      (m^+, m^-)
    \end{tikzcd}
  \]
  ...
  \only<1-2>{
  \[
    \forall n \in \mathbb{N}^2 \bdot
      \bigsqcup_{x \in R^\circ(n)}
      y \leftarrow f(x) \mathop{;}
      \bigsqcap_{m \in R(y)}
      m
      \:\sqsubseteq\:
      g(n)
  ...
  \only<3>{
  \[
    \forall x \in \mathbb{Z} \bdot f(x) \:\sqsubseteq\:
      \bigsqcap_{n \in R(x)}
      m \leftarrow g(n) \mathop{;}
      \bigsqcup_{y \in R^\circ(m)} y
  \]
  }
\end{frame}
%}}}
...
\begin{frame}{Plays} %{{{
More generally,
for a signature $E$ our game model will \\
use \textbf{odd-length} plays of the following form:
\[
  s \: \sqsubseteq_\kw{odd} \:
    ...
    \underline{v}
  \qquad
  (v \in A, m_i \in E, n_i \in \kw{ar}(m))
\]
