...
\begin{frame}{Scaling up certified software} %{{{
  ...
  \begin{itemize}
    \item General-purpose model to embed heterogenous components
    \item High-level algebra for composition and abstraction
  \end{itemize}
...
\begin{frame}{Mathematical frameworks} %{{{
  \textbf{Good news:}
...
\begin{frame}{Why this gap?} %{{{
  Challenges for adoption in certified software:
  \begin{itemize}
    \item Simplicity is key: broader adoption, mechanized proofs
    \item Techniques need to work together as a coherent whole
  \end{itemize}
\end{frame}  
%}}}
...
\begin{frame}{Refinement is correctness preservation} %{{{
...
Consider a function $f : \mathbb{Z} \rightarrow \mathbb{Z}$:
\[
...
\begin{quotation}
\vspace{1ex}
  Whenever the input is $x$, the output must be $y$
\end{quotation}
...
\begin{align*}
  (0 \mapsto 0)
  &\sqsubseteq (0 \mapsto 0) \sqcup (1 \mapsto 2 \:\sqcap\: 1 \mapsto 0) \\
  &\sqsubseteq (0 \mapsto 0) \sqcup (1 \mapsto 2) \sqsubseteq f
\end{align*}
\pause
\[
...
\begin{frame}{Dual nondeterminism as a monadic effect} %{{{
Dual nondeterminism corresponds to the $\mathbf{FCD}$ monad.

$\mathbf{FCD}(A)$ is the \emph{free completely distributive lattice}
generated by $A$.
Every element can be described as:
\[
  x \in \mathbf{FCD}(A) = \bigsqcap_{i \in I} \bigsqcup_{j \in J_i} a_{ij}
\]
\pause
The Kleisli extension of $f : A \rightarrow \mathbf{FCD}(B)$
can then be written:
\[
  f^*(x) = \bigsqcap_{i \in I} \bigsqcup_{j \in J_i} f(a_{ij})
\]

\pause
Our function specifications can be given in
$\mathbf{FCD}(\mathbb{Z} \times \mathbb{Z})$.
\end{frame}
%}}}
...
\begin{frame}[fragile]{Dual nondetermimism and data abstraction} %{{{
  ...
  \[
    \begin{tikzcd}
      ...
      y \ar[d, "R \qquad (\exists)", dash, dashed] \\
      n \ar[r, "g"'] &
      m
    \end{tikzcd}
  \]
...
\begin{frame}{Dual nondeterminism and game semantics} %{{{
  Game semantics for nondeterminism often \\
  conflate the two kinds of choices:
  \begin{gather*}
    \{ 0 \mapsto 0, \: 1 \mapsto 1, \: 1 \mapsto -1 \}
  \end{gather*}
  \pause
  The polarity of actions can still be used to construct \\
  an \emph{alternating} refinement ordering:
  \begin{align*}
    \{ 0 \mapsto 0 \}
    \: &\sqsubseteq \:
    \{ 0 \mapsto 0, \: 1 \mapsto 1, \: 1 \mapsto -1 \}
...
We will use \emph{sets of sets} of plays
to combine dual nondeterminism and game semantics:
\begin{itemize}
  \item A single play means ``if environment does $x$ then do $y$''
  \item Sets of plays use angelic determinism \\
    to range over environment behavior (strategies)
  \item Sets of strategies add demonic nondeterminism \\
    to range over possible system behaviors (strategy spec)
\end{itemize}
\end{frame}
%}}}
...
We use \emph{signatures} of the form:
\[
  E_\kw{bq} := \{
...
Client code can trigger an operation
...
\begin{frame}{Plays and interaction specifications} %{{{
More generally,
for a signature $E$ and a set $A$, our model uses
\textbf{odd-length} plays $s \in P_E(A)$ of the form:
\[
  s \: \sqsubseteq_\kw{odd} \:
    ...
    \underline{v}
  \qquad
  (m_i \in E, n_i \in \kw{ar}(m), v \in A)
\]
...
\]
\emph{Interaction specifications} can then be constructed as:
\[
...
\[
  v \in A \qquad
  x \in \mathcal{I}_E(A) \qquad
  f : A \rightarrow \mathcal{I}_E(B)
\]
\[
  \includegraphics[scale=0.06]{unit}
  \qquad
  \includegraphics[scale=0.08]{seq}
\]

This helps us define behaviors compositionally:
\[
  \kw{rot} \in \mathcal{I}_{E_\kw{bq}}(V) :=
    v \leftarrow \kw{deq} ;
    {\_} \leftarrow \kw{enq}[v] ;
    \eta(v)
\]
\[
  \includegraphics[scale=0.1]{rot}
...
\begin{itemize}
  \item Layer interfaces as
    $L_1 : 1 \rightarrow E_1@S_1$,
    $L_2 : 1 \rightarrow E_2@S_2$
  \item Layer implementations as $M : E_1 \rightarrow E_2$
  \item This can be promoted to $M@S_1 : E_1@S_1 \rightarrow E_2@S_2$
  \item $\llbracket M \rrbracket_{L_1} := M@S_1 \circ L_1 : 1 \rightarrow E_2@S_1$
  \item Abstraction relations as $R^*_{E_2} : E_2@S_2 \rightarrow E_2@S_1$
    or its adjoint
  \item Layer correctness as:
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}
    ...
    \item Relaxing all constraints and upgrading to dual nondeterminism
      yields a very expressive framework.
  \end{itemize}

  Our approach to nondeterminism in game semantics contains several innovations:
  \begin{itemize}
    \item Decoupling nondeterminism from the structure of plays
      makes the model much simpler
    \item Most proofs proceed by simple induction on plays;
      the nondeterminism monad does the rest
  \end{itemize}

  We these ideas will be a bridge ...
\end{frame}
