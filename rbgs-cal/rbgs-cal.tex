...
and define categories of effect signatures
and well-bracketed strategy specifications for games of the form
$E \multimap {!F}$ and ${!E} \multimap {!F}$.
The resulting model extends
...
of the certified compiler CompCert \cite{compcompcert,compcerto}
can be embedded into
our category of effect signatures and
strategy specifications for $E \multimap {!F}$,
and the correspondance between
their extension to ${!E} \multimap {!F}$
...
over a game signature $\Gamma$
are given for each player $u \in \Gamma$
...
\begin{definition}
A \emph{game signature homomorphism}
...
\]
In other words,
a game signature homomorphism
is a structure-preserving map on paths.
We write $\mathbf{GSP}$ for the category of
game signatures and their homomorphisms.
\end{definition}

When composing the strategy specifications
$\sigma \in \mathcal{S}([E, F])$ and
$\tau \in \mathcal{S}([F, G])$
to obtain $\tau \circ \sigma \in \mathcal{S}([E, F])$,
we will use the homomorphism
...
\end{align*}
defined similarly.
These game signature homomorphisms can then be extended to plays
...
We generalize this approach to arbitrary game signatures
...
\[
  ...
  \qquad
  \Gamma_p^+ := \bigotimes_{u \in \Gamma} \Gamma(p, u)
\]
For a question $m : u \rightarrow v$ in $\Gamma$,
we write its $\emph{incoming}$ and $\emph{outcoming traces}$
...
are defined by the game signature homomorphisms
...
is defined by the homomorphism
...
One major advantage of this representation of operators
it that their composition can be characterized
at the level of game signatures.
...
game signatures $(\Gamma_p)_{p \in \Gamma \setminus \kw{O}}$,
...
each player $p' \in \Gamma_p \setminus \{\kw{O}\}$.
For convenience,
we define $\Gamma_\kw{O} := [\Gamma_\kw{O}^-, \Gamma_\kw{O}^+]$
so that the players of $\Psi$ can be described as the set:
\[
...
are the same up to the specified isomorphism.
...
We generalize this approach in the following definition.
