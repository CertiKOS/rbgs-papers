...
is replaced by a subgraph associated with
...
The resulting game signature is as follows.
...
$\Gamma' = \Gamma[p := \Gamma_p]_{p \in \Gamma \setminus \kw{O}}$
...
so that the players of $\Gamma'$ can be described as the set:
\[
  \sum_{p \in \Gamma} \Gamma_p \setminus \{\kw{O}\} \,,
\]
with $(\kw{O}, \kw{P})$ corresponding to the
$\kw{O}$ player of $\Gamma'$.
A question in $\Gamma'((u, u'), (v, v'))$
...
\[ m'_{i-1} \cong m_i \cong m'_{i} \,, \]
so that all of the questions in the path
are isomorphic in the way specified by $\cdots$.
The questions of $\Gamma'((u, u'), (v, v'))$
