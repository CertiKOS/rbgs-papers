...
The possible evolutions of
an interaction over a game signature $\Gamma$
are characterized by the stack of pending questions.

\begin{definition}
For a game signature $\Gamma$ and a player $p \in \Gamma$,
a \emph{$p$-stack} over $\Gamma$ is a path $\kappa$:
\[
...
The game described by a signature $\Gamma$
is played on a graph as follows.
Vertices are pairs $(u, \kappa)$
with $u \in \Gamma$ and $\kappa$ is a $u$-stack.
...
in this graph,
where $\kappa : \kw{O} \twoheadrightarrow \kw{O}$
is a stack.
We will write
$P \, \Gamma$
...
the corresponding morphisms in $\gcat^{ib}$
...
partial plays of the form
$t : (\kw{O}, \kappa) \twoheadrightarrow (\kw{O}, \kappa')$,
and for a question $q \in F$,
the set $\bar{P}^{\kappa q}_\Gamma$ contains
partial plays of the form
$\bar{t} : (\kw{P}, \kappa q) \twoheadrightarrow (\kw{O}, \kappa')$.
We will define two operators:
\begin{gather*}
  \omega^\kappa :
...
\begin{align*}
  \omega^\kappa(t) &=
    t \: \sqcup \:
    \bigsqcup_{q \in F}
      q \, \bar{\omega}^{\kappa q}(\sigma^q, \omega^\kappa(t))
  \\
  \bar{\omega}^{\kappa q}(\underline v, t) &=
    v t
  \\
  \bar{\omega}^{\kappa q}(\underline m, t) &=
    m \, \omega^{\kappa q m}(\epsilon)
  \\
  \bar{\omega}^{\kappa q}(\underline m n \bar{s}, t) &=
    m \, \omega^{\kappa q m}(n \, \bar{\omega}^{\kappa q}(\bar{s}, t))
\end{align*}
That is,
$\omega^\kappa(t)$
first executes an arbitrary number of top-level questions
of $\sigma$ to completion,
including possible reentrant calls,
then continues using the play $t$.
This is expressed using
$\bar{\sigma}(\bar{s}, t)$, which embeds an interaction
$\bar{s} \in \bar{P}_E(\kw{ar}(q))$,
inserting reentrant calls as appropriate,
then continues using the play $t$.

To eliminate circular dependencies, we define
$\omega^\kappa_0(t) := t$
and construct the sequence of approximations:
\begin{align*}
  \omega^\kappa_{i+1}(t) &=
    t \: \sqcup \:
    \bigsqcup_{q \in F} \,
      q \, \bar{\omega}^{\kappa q}_i(\sigma^q, \omega^\kappa_i(t))
  \\
  ...
  \\
  \bar{\omega}^{\kappa q}_i(\underline m n \bar{s}, t) &=
    m \, \omega^{\kappa q m}_i(n \, \bar{\omega}^{\kappa q}_i(\bar{s}, t))
\end{align*}
Then the strategy associated to $\sigma$
is $W \sigma := \bigsqcup_{i \in \mathbb{N}} \omega_i(\epsilon)$.
