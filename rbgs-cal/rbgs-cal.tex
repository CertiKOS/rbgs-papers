...
a mechanized, machine-checked proof of correctness.
...
Then, using specialized tools,
we construct evidence that the program
conforms to the specification (the correctness proof),
which an independent third-party
should be able to verify and trust with minimal effort.

In order to connect the program and specifications
using a formal proof,
we need to formally define
the languages used to formulate them
by intepreting them into a mathematical domain.
The tools used to reason about them
are then shown to be sound with respect to this interpretation.
Ideally,
this will be mechanized in an established,
...
file system implementations \cite{fscq} and
processor designs \cite{...},
VST \cite{vst}
\ldots
...
The key idea behind the DeepSpec project
is to interpret specifications as \emph{interfaces}
between components.
Then once a component has been proved correct, \ldots
[client assumes, server proves etc.]
[internal specifications not part of the TCB]
...
is the uniformity of models underlying
...
interface specifications making it possible to connect them
in individual cases,
...
In the case of CompCert alone,
the semantic model used has evolved across time
to address new requirements and techniques introduced alongside
...
Then,
to connect artifacts verified using a variety of paradigms,
...
correctness proofs
of each one.
This model should be rich enough to offer
high-level composition and reasoning principles,
enabling the construction of large-scale certified systems,
yet simple enough
to be understood by a wide array of practitioners and
to be mechanized in a proof assistant.

\subsection{?}

[Summarize the appealing aspects of
refinement calculus,
...
(general and difficult <-> specific and easy)

More than one level of ``semantics''
Notions of ``syntax'' and ``semantics'' are relative concepts.

game semantics:
reconciles operational and denotational models,

[Grand challenge of
end-to-end, compositional verification of computer systems, 
building certified systems from
off-the-shelf certified components.]
