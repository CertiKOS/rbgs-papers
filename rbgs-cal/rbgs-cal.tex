...
$U(f_\phi^*)$ as the \emph{Kleisli extension} of $f$
for the monad $\mathbf{FCD}$.
...
This is because the $\mathbf{FCD}$ extension
...
For two event signatures $E$ and $F$,
the \emph{well-bracketed strategy specifications}
for the game ${!E} \multimap {!F}$
are given by $\mathbf{FCD}(P_{E,F})$.
The identity strategy is given as:
\[
  \kw{id}_E :=
    \bigsqcup_{m \in E, n \in \kw{ar}(m)}
      m \underline{m} n \underline{n}
\]
