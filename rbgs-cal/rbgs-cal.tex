...
However, because these efforts often use semantic
...
$c \leftarrow x; f(c)$ to denote $f_\phi^*(x)$,
writing $x; y$ when $f$ is a constant function,
and sometime implicitly extend the domain of monotonic functions
from a poset to its free distributive completion.
Likewise,
we will often use the embedding $\phi_f^*$ itself implicitly,
regading values in the poset as
the corresponding elements of the free distributive completion.
Lastly,
the \emph{assertion} $\{P\}$ of a proposition $P$
evalutates to the unit value $*$ when the proposition is true,
...
(it is angelic with respect to its choice of $k_1$).
On the other hand,
$R^*_E \circ \sigma$ is free to choose any underlay representation
for the outcome $k_1'$ produced by $\sigma$,
and the client must be ready to accept it
(it is demonic with respect to its choice of $k_2$).
...
with respect to $R$.
For an overlay state $k_1 \in S_1$,
$\tau$ may behave in various ways
depending on the corresponding underlay state $k_2 \in S_2$
it is invoked with,
and so the specification must allow them using demonic choice.
On the other hand,
when $\tau$ returns with a new underlay state $k_2'$,
the environment is free to choose
how to interpret it as an overlay state $k_1'$.
...
%
%Models featuring dual nondeterminism
%have been used in various forms.
%Beyond predicate transformers,
%the binary multirelations proposed in \cite{multirel}
%or the trace semantics used in \cite{altref}
%can be seen as variations on $\mathbf{FCD}$.
%The distinguishing feature of
%the free completely distributive lattice
%is that it starts from a poset rather than a set,
%allowing us to deploy it in the context of game semantics.

Despite the breath of the literature on game semantics,
there has been little focus on refinement.
The model of nondeterminism proposed in \cite{gsnondet}
has been recognized as problematic,
however more recent work addressing these issues
has remained fairly complex \cite{gsndsheaves}.
Work on adjascent topics such as
alternating-time temporal logic \cite{atl,altref} and
interface automata \cite{ia,itcd,gmos,gtf}
acknowedges the need to define refinement
...
The model of the process calculs CSP
proposed in \cite{cspdnd}
comes closest to our own approach.
%but its relevance to game semantics
...
Finally, variations on the free monad are numerous,
but the coinductive version known as
\emph{interaction trees} \cite{itree}
implemented in the Coq proof assistant
shares structures and goals with
