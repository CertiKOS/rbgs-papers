...
\begin{align*}
  C \sqsubseteq C_1 \:\wedge\: C \sqsubseteq C_2 &
    \:\Leftrightarrow\: C \sqsubseteq C_1 \sqcap C_2 \\
...
requires the system to refine
\emph{both} $C_1$ and $C_2$,
while $C_1 \sqcap C_2$
allows it to refine either one of them:
\begin{gather*}
  C_1 \sqsubseteq C \: \wedge \: C_2 \sqsubseteq C
    \: \Leftrightarrow \: C_1 \sqcup C_2 \sqsubseteq C \\
...
$E \uplus \{ v : \varnothing \mid v \in A \}$. \zhong{Should we use a more distinct syntax such as the retv($v$) event?}
...
Since the monad represents ongoing computations,
...
defined as follows.
Note that $\bot$ and $\{P\}$ are discussed at the end of
\S\ref{sec:refcal};
