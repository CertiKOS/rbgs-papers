...
in term of their behavior at the level of
the possible questions in a game signature.
...
in the three-player game signature $[A,B,C]$.
...
Define the internal projections as the game signature homomorphism:
\[
  \pi_i :
    \Gamma \rightarrow \left[
      \bigotimes_{p \in \Gamma} \Gamma(\kw{P}_i, p), \:
      \bigotimes_{p \in \Gamma} \Gamma(p, \kw{O})
    \right]
\]
which maps a question $m : u \rightarrow v$ to
$\check{\pi}(\kw{P}_i, m) \cdot \hat{\pi}(\kw{P}_i, m)$,
where:
\begin{gather*}
  \check{\pi}(p, m) = \begin{cases}
    m & \text{if } v = p \\
    \epsilon & \text{otherwise}
  \end{cases}
  \\
  \hat{\pi}(p, m) = \begin{cases}
    m & \text{if } u = p \\
    \epsilon & \text{otherwise}
  \end{cases}
\end{gather*}
and the external projection $\pi_X$
which maps $m : u \rightarrow v$
to $\hat{\pi}(\kw{O}, m) \cdot \check{\pi}(\kw{O}, m)$.
Then angelically choose a global play $s \in \Gamma$
matching the internal projections and take its external projection:
\begin{align*}
  &\Gamma[\kw{P}_1 := \sigma_1, \ldots, \kw{P}_n := \sigma_n] := \\
   & s_1 \leftarrow \sigma_1 ; \cdots ; s_n \leftarrow \sigma_n ; 
    \bigsqcup_{s \in P \Gamma}
    \{ \forall i \bdot \pi_i(s) \sqsubseteq s_i \} ; \pi_X(s)
\end{align*}
