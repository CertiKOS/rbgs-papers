...
\acmBooktitle{Proceedings of the 35th Annual ACM/IEEE Symposium on Logic in Computer Science (LICS '20), July 8--11, 2020, Saarbr\"ucken, Germany}
...
The Kleisli extension $f_\phi^*(x)$
replaces any outcome $a$ of the computation $x$
with the computation $f(a)$.
We will sometimes write
$f(x)$ or $a \leftarrow x ; f(a)$ for $f^*_\phi(x)$,
...
$
    x = \bigsqcap_{i \in I} \bigsqcup_{j \in J_i} a_{ij}
$
where
...
\[
    \bigsqcap_{i \in I} \bigsqcup_{j \in J_i} a_{i,j} =
    \bigsqcup_{f \in (\prod_i J_i)} \bigsqcap_{i \in I} a_{i, f_i}
\]
further allows angelic and demonic choices to commute,
and the status of $f^*$ as a complete homomorphism
