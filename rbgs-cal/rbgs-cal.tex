...
For example:
\[
    \htr{\text{$\kw{n}$ is odd}\,}{\kw{n := n * 2}}{\,\text{$\kw{n}$ is even}}
\]
...
The program $\kw{n := 2 * n} \: \sqcap \: \kw{n := 0}$
may be executed arbitrarily as $\kw{n := 2 * n}$ or $\kw{n := 0}$,
with no guarantee to the user as to which branch will be chosen.
...
\[
  ...
  \end{prooftree}
\]
The statement $\kw{n := n * 2} \: \sqcup \: \kw{n := 0}$
is more difficult to interpret than its demonic counterpart,
but can tought of as a program which magically behaves as
$\kw{n := n * 2}$ or $\kw{n := 0}$
depending on the needs of user.
...
In other words,
demonic choices %in the specification
...
the execution of $\kw{n := 2 * n}$
could have the form:
\[
    \kw{run} \cdot
    \underline{\kw{read}_\kw{n}} \cdot n \cdot
    \underline{\kw{write}_\kw{n}[2n]} \cdot \kw{ok} \cdot
...
the move $\kw{run}$.
The system move $\kw{read}_\kw{n}$ requests
the value of the variable $\kw{n}$,
communicated in response by the environment move $n$.
The system move $\kw{write}_\kw{n}(n)$ requests
storing the value $n$ in the variable $\kw{n}$,
...
\begin{align*}
  ...
  &
      \begin{array}{l@{\,}l}
        = \{ & \epsilon, \\ &
        \kw{run} \cdot
        \underline{\kw{read}_\kw{n}} ,
        \\ &
        \kw{run} \cdot
        \underline{\kw{read}_\kw{n}} \cdot n \cdot
        \underline{\kw{write}_\kw{n}[2n]} ,
        \\ &
        \kw{run} \cdot
        \underline{\kw{read}_\kw{n}} \cdot n \cdot
        \underline{\kw{write}_\kw{n}[2n]} \cdot \kw{ok} \cdot
...
\[
  \sigma \subseteq
    ...
    {\downarrow}(
      \kw{run} \cdot
      \underline{\kw{read}_\kw{n}} \cdot n \cdot
      \underline{\kw{write}_\kw{n}[2n + \delta]} \cdot \kw{ok} \cdot
      \underline{\kw{done}})
\]
...
This can be interpreted as choice of the environment
...
The model we have presented so far is insufficient
...
\begin{align*}
  ...
  & \, =
    \{ \epsilon,  \:
       \kw{run} \cdot \underline{\kw{read}_\kw{n}} \cdot n
       \mid n \in \mathbb{N} \} \,.
\end{align*}
...
\[
  s_{n,\delta} :=
        \kw{run} \cdot
        \underline{\kw{read}_\kw{n}} \cdot n \cdot
        \underline{\kw{write}_\kw{n}[2n + f_n]} \cdot \kw{ok} \cdot
        \underline{\kw{done}} \,,
\]
the more permissive \emph{strategy specification}
can be written as:
\[
  \sigma' :=
...
ordered by containement ($\supseteq$).
...
Note that somewhat counterintuitively,
the \emph{arguments} of operations correspond to
the possible \emph{outcomes} of the effect,
so that for instance the $\kw{readbit}$ operation takes two arguments.
Moreover,
