...
\end{align*}
then extend it to morphisms as $(f@S)^{m@k} := f^m\#k$.
Then in particular,
...
\subsection{Games} \label{sec:gamesem:games} %{{{
...
\subsection{Category} \label{sec:gamesem:cat} %{{{
...
\subsection{Embedding $\gcat^{ib}$} \label{sec:gamesem:emb} %{{{
...
\subsection{Hiding state} \label{sec:gamesem:pack} %{{{
...
but remember it in order to ajoin it to the next incoming move.
...
Although giving a comprehensive account
of the context of our work is challenging,
we discuss some related research.
...
for instance interface theories
...
but sensitivity to branching makes
the resulting refinement algebra less agreeable.
...
However, devising a notion of refinement insensitive to
branching would be more challenging. %, and may boil down to
%embedding interaction trees into a model similar to that of
%\S\ref{sec:intspec}.
%to compare them in that setting.
...
Game semantics has tremendous potential
for the formal verification of large and complex systems.
...
and to integrate general verification techniques such as
stepwise refinement and data abstraction
into the game semantics landscape.
...
the major distinguishing feature of game semantics
is the polarization of moves.
This establishes a boundary between
the system and the environment,
and is a key ingredient in the construction of
compositional models~\cite{cspgs}.
Likewise,
recognizing the distinction
and the duality between angelic and demonic nondeterminism
is crucial to a satisfactory treatment of refinement,
especially in the context of games.

It is interesting to note, however,
that refinement orderings
which bind too closely
the polarity of moves with that of nondeterministic choices
often end up requiring a fair amount of sophistication,
especially when they are constructed after-the-fact,
on top of (or by adapting) an existing model.
By contrast,
our approach embraces dual nondeterminism
to a maximal extent and from the ground up,
in a way that is only loosely coupled with
the structural details of plays.
While this carries an initial cost in the complexity
of the strategy model,
this is more than offset by the simplicity and uniformity
of the resulting refinement algebra.
