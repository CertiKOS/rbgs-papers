...
but also formally specify intended behavior
...
has been verified,
client components can rely on it
...
by their action on questions.

\begin{definition}
The \emph{template of arities} is the category ${\moo}_\kw{ar}$ 
a single object and sequences of sets as morphisms,
with concatenation as its composition.
In addition, we write $\mathbf{1}$ for the trivial category
with a single object and morphism.

A \emph{game signature} is a category $\Gamma \in \mathbf{Cat}$
together with the functors
$\kw{pt} : \mathbf{1} \rightarrow \Gamma$ and
$\kw{ar} : \Gamma \rightarrow {\moo}_\kw{ar}$:
\[
  \begin{tikzcd}
    \mathbf{1} \arrow[r, "\kw{pt}"] &
    \Gamma \arrow[r, "\kw{ar}"] &
    {\moo}_\kw{ar}
  \end{tikzcd}
\]
The functor $\kw{pt}$ picks out a distinguished object,
whereas $\kw{ar}$ assigns an arity to the morphisms of $\Gamma$
in a way that is compatible with composition.

A \emph{game signature homomorphism} between $\Gamma_1$ and $\Gamma_2$
is a functor $F : \Gamma_1 \rightarrow \Gamma_2$
which preserves the structure of game signatures:
\[
  \begin{tikzcd}
    & \Gamma \arrow[rd, "\kw{ar}"] \arrow[dd, "F"] & \\
    \mathbf{1}
      \arrow[ru, "\kw{pt}_1"]
      \arrow[rd, "\kw{pt}_2"] & &
      {\moo}_\kw{ar} \\
    & \Gamma_2 \arrow[ru, "\kw{ar}_2"] &
  \end{tikzcd}
\]


with a distinguished object $\mathrm{pt}$,
and which assigns an arity to each morphism,
given as a sequence of sets,
such that $\kw{ar}(f \circ g) = \kw{ar}(f) \cdot \kw{ar}(g)$.
In other words,
it is a span from $\mathbf{1}$ to $


We write ${\moo}_\kw{ar}$ for the \emph{template of arities},
with a single 
generated by the graph with a single vertex
and a loop 
and a


A \emph{game signature} is a category $\Gamma$,
with a distinguished object $\mathrm{pt}$
...
To define our model of games and strategies,
we adapt the basic setup introduced in \cite{ccss}.
Games are represented as \emph{categories},
whose objects denote \emph{positions} and
whose morphisms denote \emph{trajectories}
(fragments of plays).
To constrain a game $\Gamma \in \mathbf{Cat}$
to follow a certain structure,
we use a functor $\lambda : \Gamma \rightarrow {\moo}$
into a corresponding \emph{template category} ${\moo}$.
For example,
two-player alternating games correspond to
the template category ${\moo}_\kw{game}$
generated by paths in the graph:
\[
  {\moo}_\kw{game} :=
  \mathbf{Paths} \left(
  \begin{tikzcd}[column sep=large]
    \langle {\oplus}  \rangle \arrow[r, shift right, "\kw{P}"'] &
    \langle {\ominus} \rangle \arrow[l, shift right, "\kw{O}"']
  \end{tikzcd}
  \right)
\]
Therefore,
every position $p \in \Gamma$
in a game
$\Gamma \xrightarrow{\lambda} {\moo}_\kw{game}$
is labelled with a polarity
$\lambda(p) \in \{ \langle {\oplus} \rangle, \langle {\ominus} \rangle \}$,
and every trajectory $s : p_1 \rightarrow p_2 \in \Gamma$
is labelled with
an alternating sequence of players in $\{ \kw{O}, \kw{P} \}$
matching the polarities of $p_1$ and $p_2$,
each element corresponding to a move
within the trajectory.

As an illustration,
consider the game corresponding to
the families $(f^m \in \mathcal{I}_E(\kw{ar}(m)))_{m \in F}$
used to intepret effect handlers in \S\ref{sec:intspec}.
The opponent chooses a move $m \in F$,
the proponent asks a series of questions
$q_1, \ldots, q_k \in E$
which the opponent answers with
$r_1 \in \kw{ar}(q_1), \ldots, r_k \kw{ar}(q_k)$,
and finally the proponent
answers the question $m$ with a move $n \in \kw{ar}(m)$.


consider the plays used in the definition of $\mathcal{I}_E(A)$.
The corresponding game can be represented as the category
generated by the (infinite) graph:
\[
  \begin{tikzcd}
    N \arrow[r, shift right, "n \in N"'] &
    \kw{I} \arrow[l, shift right, "(m : N) \in 


This approach allows us to characterize
structures and operators on games
in terms of the templates,
so that they can be defined and resoned about
in terms of their essential structure.
For example,



capturing structures of interest
in the form of templates
allows us to define and reason about general constructions
