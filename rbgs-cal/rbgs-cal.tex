...
for each pair of players $u, v \Gamma$.
The operations $m \in \Gamma(u, v)$ are called
the \emph{questions} of $u$ to $v$.
Elements $n \in \kw{ar}(m)$ are called
the \emph{answers} to the question $m$.
\end{definition}
...
\[
  ...
  \end{tikzpicture} \,,
\]
and the game ${!E} \multimap {!F}$ will be generated from
the signature:
\[
  [E,F] =
...
\[
  [E_1,F_1] \vee [E_2,F_2] =
  \begin{tikzpicture}[baseline=(O.base),scale=1.5]
    \node[circle,draw] (P1) at (-1,0) {$\sigma_1$};
    \node[circle,draw,double] (O) at (0,0) {$\kw{O}$};
    \node[circle,draw] (P2) at (+1,0) {$\sigma_2$};
...
questions of $\kw{P}_{i-1}$ to $\kw{P}_i$ given by $E_i$,
and questions from $\kw{P}_n$ to $\kw{O}$ given by $F$:
\[
  [F, E_n, \ldots, E_1] :=
...
the families of sets
$(\hat{P}_\Gamma^u)_{u \in \Gamma}$ and
$(P_\Gamma^u)_{u \in \Gamma}$
...
\begin{gather*}
  ...
  {\begin{prooftree}
    \hypo{s \in \hat{P}_\Gamma^u}
    \hypo{m \in \Gamma(u, v)}
    \hypo{t \in P_\Gamma^v}
    \infer3{smt \in P_\Gamma^u}
  \end{prooftree}}
  \\[1ex]
  \epsilon \in \hat{P}_\Gamma^u
  \qquad
  \hat{P}_\Gamma^u \subseteq P_\Gamma^u
\end{gather*}
The elements of $\hat{P}_\Gamma^u$
are the \emph{balanced plays} of $\Gamma$
starting with player $u$.
The elements of $P_\Gamma^u$
are the \emph{well-bracketed plays} of $\Gamma$
starting with player $u$.
\end{definition}
...
starting at player $\kw{O}$.

\begin{definition}[Plays and strategies over a game signature]
The well-bracketed plays for a game signature $\Gamma$
...
\[
  ...
  \end{prooftree} \,.
\]
We will write $P_\Gamma := \langle P_\Gamma^\kw{O}, {\sqsubseteq} \rangle$
for poset of plays starting with $\kw{O}$,
and talk about the well-bracketed plays of $\Gamma$
...
\[
    \mathcal{S}_\Gamma := \mathbf{FCD}(P_\Gamma) \,.
\]
\end{definition}
...
directly at the level of strategy specifications.
To address this,
we introduce a systematic way of defining operators
in term of their action on the moves of game signatures.
To illustrate this method,
we use the categorical composition of strategies as an example.

The traditional definition of composition
in categories of games and strategies,
as given for example in \cite{gamesem99},
is as follows.
Given a strategy $\sigma : A \rightarrow B$,
which plays the game $B$ as the proponent
and the game $A$ as the opponent,
and a strategy $\tau : B \rightarrow C$,
which plays the game $C$ as the proponent
and the game $A$ as the opponent,
the strategy $\tau \circ \sigma : A \rightarrow C$
is obtained by first considering plays
of a game consisting of the three components $A, B, C$.
Given such a play $s \in P_{A,B,C}$,
we can project out
...
\]
In our setting,
$s$ will correspond to a play
in the two-player game signature $[A,B,C]$.
The projections can be defined as
operators of the following kind.

\begin{definition}
An \emph{operator} $\psi : \Gamma_1 \rightarrow \Gamma_2$
from the game signature $\Gamma_1$
to the game signature $\Gamma_2$ associates
to each player $u \in \Gamma_1$ a player $\psi(u) \in \Gamma_2$, and
to each question $m : u \rightarrow v$ in $\Gamma_1$
a path $\psi(m) : \psi(u) \twoheadrightarrow \psi(v)$
in $\Gamma_2[\kw{ar}(m)]$,
that is a sequence of questions
$\psi(u) = u_0 \xrightarrow{m_1} \cdots \xrightarrow{m_n} u_n = \psi(v)$
in $\Gamma_2$ such that
$\kw{ar}(m_1) = \cdots = \kw{ar}(m_n) = \kw{ar}(m)$.
\end{definition}

For instance,
when composing the strategy specification
$\sigma \in \mathcal{S}_{[E, F]}$ and
$\tau \in \mathcal{S}_{[F, G]}$
to obtain $\tau \circ \sigma \in \mathcal{S}_{[E, F]}$,
we will use the following operator
$\psi_X : [E,F,G] \Rightarrow [E,G]$
...
\begin{gather*}
  ...
  \end{tikzpicture}
  \\
  \psi_X(\kw{P}_1) = \psi_X(\kw{P}_2) = \kw{P} \qquad
  \psi_X(\kw{O}) = \kw{O}
  \\
  \psi_X(m) = \begin{cases}
  ...
  \end{cases}
\end{gather*}
We define the internal views
$\psi_1 : [E,F,G] \rightarrow [F,G]$ and
$\psi_2 : [E,F,G] \rightarrow [E,F]$
in similar ways.
Operators of this kind can be extended to
game states, plays, and ultimately strategies
...
$\psi : \vec{\Gamma}_1 \rightarrow \vec{\Gamma}_2$
which maps a path in $\Gamma_1$:
\[
  m_1 m_2 \cdots m_n : u \twoheadrightarrow v
\]
to the path in $\Gamma_2$:
\[
  \psi(m_1) \, \psi(m_2) \cdots \psi(m_n) :
  \psi(u) \twoheadrightarrow \psi(v) \,.
\]
The operator
can be further extended to a function
on well-bracketed expansions
$\psi : \vec{\Gamma_1^*} \rightarrow \vec{\Gamma_2^*}$
which maps each question edge
$m_1 : \vec{k} \rightarrow \vec{k} m$
in a path of $\Gamma_1^*$
to a corresponding subpath
$\psi(m) : \psi(\vec{k}) \rightarrow \psi(\vec{k}m)$
in $\Gamma_2^*$,
and which for each answer $n \in \kw{ar}(m)$
maps the corresponding edge
$n : \vec{k} m \rightarrow \vec{k}$ in $\Gamma_1^*$
to the path
$n^{|\psi(m)|} : \psi(\vec{k} m) \rightarrow \psi(\vec{k})$
of same length
running in the opposite direction.
Since $\psi$ preserves $\kw{O}$,
the plays of $\Gamma_1$ yield plays of $\Gamma_2$,
and the action of $\psi$ on strategies can be defined
as $\kw{FCD}(\psi)$.
\end{definition}

We can then formulate our own definition of composition
as follows.
The ``footprint'' of the plays $s_1 \in P_{[E,F]}$ and $s_2 \in P_{[F,G]}$
can be defined as:
\[
  \psi(s_1, s_2) :=
    \bigsqcup_{s \in P_{[E,F,G]}}
    \{ \psi_1(s) \sqsubseteq s_1 \wedge \psi_2(s) \sqsubseteq s_2 \} ;
    \psi_X(s) \,.
\]
In other words,
the angel chooses a global play $s$
matching $s_1$ and $s_2$
and returns its external view.
The composition operator extends this to strategies
in the usual way:
\[
  \psi(\sigma, \tau) := 
    s_1 \leftarrow \sigma ;
    ...
    \psi(s_1, s_2) \,.
\]
We generalize this approach with the following definition.
...
where $s_i \in P_{\Gamma_i}$
is given by:
\[
  \psi(s_1, \ldots, s_n) :=
    \bigsqcup_{s \in P_{\Gamma_G}}
