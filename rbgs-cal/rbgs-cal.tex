...
we can write $\mathbf{I}^E_m$ simply as $m$.
\end{definition}
...
$(f_m)_{m \in E}$
to provide an interpretation $f_m \in \mathcal{I}_F(\kw{ar}(m))$
of each effect $m \in E$
in terms of another effect signature $F$.
This allows us to transform an interaction specification
$x \in \mathcal{I}_E(A)$ of $E$
into an interaction specification
$x[m := f_m]$ of $F$.

\begin{definition}[Interaction substitution]
Given the effect signatures $E, F$ and the set $A$,
for an interaction specification $x \in \mathcal{I}_E(A)$
and a family $(f_m)_{m \in E}$ with $f_m \in \mathcal{I}_F(\kw{ar}(m))$,
the \emph{interaction substitution} $x[m := f_m] \in \mathcal{I}_F(A)$
is defined by:
\begin{align*}
  \underline{v}[m := f_m] &:= \underline{v} \\
  \underline{m}[m := f_m] &:= f_m ; \bot \\
  \underline{m}ns[m := f_m] &:= r \leftarrow f_m ; \{r = n\} ; s[m := f_m]
\end{align*}
XXX: define assertions $\{P\}$ and the monad notation.
\end{definition}
...
whenever $(f_m)_{m \in F}$ is a family of interactive computations
such that $f_m \in \mathcal{I}_E(\kw{ar}(m))$.
For $x \in \mathcal{I}_F(A)$,
we can write the interaction substitution $x[m := f_m]$
as $x[f]$.
...
so that for a family $(f^i)_{i \in I}$
with $f^i : E \rightarrow F$,
we can define the substitutions
$(\bigsqcup_{i \in I} f^i) : E \rightarrow F$ and
$(\bigsqcap_{i \in I} f^i) : E \rightarrow F$ as:
\begin{gather*}
    \left( \bigsqcup_{i \in I} f^i \right)_m :=
      \bigsqcup_{i \in I} f^i_m \qquad
    \left( \bigsqcap_{i \in I} f^i \right)_m :=
      \bigsqcap_{i \in I} f^i_m \,,
\end{gather*}
and for $f, f' : E \rightarrow F$
define refinement as:
\[
    f \sqsubseteq f' \: \Leftrightarrow \:
    \forall m \in F : f_m \sqsubseteq f'_m \,.
\]
\end{definition}

An interaction substitution $f : E \rightarrow F$
can be interpreted as a well-bracketed strategy for the game
${!E} \multimap F$.
In this game,
...
and the \emph{well-bracketing} requirement
ensures a form of stack discipline
so that at any point,
only the most recent pending question
may be answered.

Our model extends the corresponding traditional notion of strategy
by introducing demonic choice.
The definition of $g \circ f$ given above
otherwise corresponds to the traditional notion of
...
The construction $\otimes$ gives products in the category $\gcat^i$,
as demonstrated below.

\begin{lemma}
The category $\gcat^i$ has all products.
The corresponding objects are given by $\bigotimes_{i \in I} E_i$,
and the projection arrows
are given for each $i \in I$ as
...
as morphisms
$\llbracket L \rrbracket : \gcat^i(\mathbb{E} \otimes A, B)$.
