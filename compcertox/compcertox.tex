...
\begin{example}
  ...
  \]
  The examples of array and counters
  represent the \emph{getter-setter} pattern
  where the implementation
  introduces new state and does not rely on any underlay.
  The example of bounded queue
  represents the \emph{absfun} pattern
  which does not introduce new states
...
In order to compose two layers
on the same level,
we first define
the tensor product
on layer interfaces
...
\begin{example}
  ...
  that the function pointer $f$ points to.
