...
\section{CompCert-Based CAL} \label{sec:cal}

\subsection{limitations of the original framework}

\begin{itemize}
\item ClightX and underlay dependence
\item horizontal composition
\end{itemize}

\subsection{CertiKOS-style strategy layers}

Promote an abstraction relation
to refinement convention
with the $\kw{unchanged\_on}$ constraint

\subsubsection{vertical composition}
Client program and commutation property

\subsubsection{tensor product composition}

\section{Strategy-Based CAL}


\subsection{Abstract Layers}

Briefly go over the LICS'20 model and its limitations.

\subsubsection{Introducing New State}
\subsubsection{Vertical Composition}
\subsubsection{Tensor Product Composition}
\begin{example}
  Ring buffer and bounded queue in abstract layers
\end{example}
...
\[
  \hat{\Theta} := \{ (d_2, d_1, \Theta) \mid d_2 \mathrel{\Theta} d_1 \}
\]
...
\begin{align*}
  (\mathcal{X}@D)@\kw{mem}
  &\cong
  \mathcal{X}@(\kw{mem} \times D)
  \\
  \mathcal{X}@(\kw{mem} \times D)
  &\cong
  \mathcal{X}@(D \times \kw{mem})
\end{align*}
...
\[
  N@D_2 \sqsubseteq_{\hat{R} \rightarrow \hat{R}} N@D_1
\]
