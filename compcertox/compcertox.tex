...
Researchers have been able to interface
certified software projects on a case-by-case basis,
but a systematic approach has not yet emerged.

In principle,
compositional semantics can provide a mathematical foundation
for the interoperability of certified systems.
In practice,
...
with each new layer relying for its correct operation
on the layers below,
such that a defect in any of them
could prevent the overall system from
working as expected.
To address this challenge,
researchers have demonstrated the construction of
\emph{certified} versions of many key components,
including
...
We must the write a specification
which delineates the desired behavior of the program
in terms of the semantic model,
...
we must make sure that the assumption made by one component
match the guarantees provided by the other.
For example,
...
in such a way that the that interact harmoniously
...
the semantics of their assembly languages,
which in turn work very differently from
the high-level languages used to develop application software,
based on the abstractions provided by the operating system.

This suggests important considerations
for future certified systems research.
...
These successes make it possible to envision
the construction of larger certified systems,
obtained by combining existing certified components
and using specifications as interfaces
between their proofs of correctness \cite{deepspec, etc}.
...
\subsection{Making Certified Software Scale}
...
\subsection{Abstraction and Refinement}
...
While it has become possible to 

The successful development over the past decade
of many critical \emph{certified} software components
has prompted researchers to investigate
the ways in which such components could be combined into larger certified systems.


The success of \emph{certified software} over the past decade
has prompted several researchers to investigate 

In the past 10 to 15 years,
researchers have been able to
formally specify and verify the correctness
of many key software components
of computer systems, including
compilers,
operating system kernels,
application software,
and even some hardware components
such as CPU designs.

In the past ten years,
researchers have explored many techniques
for the construction of \emph{certified} software---%
which include formal specifications
and computer-checkable proofs of correctness---%
carrying out such impressive feats as
the formal verification of 

programs which are 


Such software is accompanied by
...
\subsection{Engineering Certified Systems}

Need a semantics,
a way to write specifications,
a way to relate them.
They should be compositional,
ie. give a semantical account of
the ways systems are constructed
...
witnessed by the abstraction relation. In this section, we provide an
interpretation of the layer calculus using the interaction specifications. In
particular, we use Clight programs as the implementations between the layer
interfaces.
...
\]
We will omit the notation $\llbracket - \rrbracket$ when the context is clear.
...
from the language interface $\mathcal{C}$ except the memory.
...
We can define refinement conventions $\gamma_{bq} : E_{bq} \leftrightarrows C$
and $\gamma_{rb} : E_{rb} \leftrightarrows C$ to bridge the gap between $C$ and
the effect signatures $E_{bq}$ and $E_{rb}$ respectively given the relation
$R^V \subseteq \kw{val} \times \kw{V}$.
...
Then we can prove a refinement from the monadic computation $M_{bq}$ to the C code. The
implementation $M_{bq}$ does not modify states so the refinement relation
applies to any abstract state and any relation between the state and the memory.
\[
  \forall S, \forall R \subseteq S \times \kw{mem},
  M_{bq}@S \sqsubseteq (\gamma_{bq} \otimes R)_* \circ
  \llbracket \kw{Clight}(p_{bq}) \rrbracket \circ (\gamma_{bq} \otimes R)^*
\]

By contrast, $M_{rb}$ implements on the state $S_{rb}$ and we have to provide a
relation $R_{rb} \subseteq S_{rb} \times \kw{mem}$ to establish the following
refinement.
\[
  M_{rb}@S_{rb} \sqsubseteq (\gamma_{rb} \otimes R_{rb})_* \circ
  \llbracket \kw{Clight}(p_{rb}) \rrbracket \circ \bot
\]

The refinement order preserves composition. Thus,
\[
  (M_{bq} \circ M_{rb}) @ S_{rb} \sqsubseteq (\gamma_{rb} \otimes R_{rb})_* \circ
  \llbracket \kw{Clight}(p_{rb} + p_{bq}) \rrbracket \circ bot
\]
[TODO: this probably contradicts the principle of certified abstraction layers
since we are supposed to compose layer by layer.]
