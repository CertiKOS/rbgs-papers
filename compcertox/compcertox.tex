...
For example,
the context of the certified compiler CompCert \cite{compcert}
...
the execution of the source C program
...
This involves defining a simulation relation
...
This is possible because in both cases,
...
at different levels of abstraction
\cite{compcerto}.
However, unlike CompCert,
CompCertO does not define a trace semantics
against which the soundness of its open simulations
can be established.

Likewise,
various models of certified abstraction layers
have been proposed
which feature a richer compositional algebra
than in their original presentation
...
\begin{itemize}
  ...
  \item To account for the abstraction gap between
    ...
    allowing us to derive a soundness property for
    CompCertO's open simulations
    (\S\ref{sec:refconv}).
  \item These building blocks can be used to reconstruct
    ...
    is more tractable.
    In particular,
    we were able to formulate a tensor product
    which can be used to compose independent layers.
    Nevertheless,
    it remains formulated at a low level of abstraction,
    and in particular it must deal with the CompCert memory model.
  \item To address this, we extend
    the more abstract model of certified abstraction layers
    proposed in the context of refinement-based game semantics
    to allow layers to introduce new abstract state (\S\ref{sec:rbgs-cal}).
  \item Finally,
    we use refinement conventions
    to connect these two theories of certified abstraction layers
    (\S\ref{sec:correspondence}).
    This allows us to enjoy the best of both worlds:
    high-level reasoning can be performed
    within the mathematically tractable,
    more abstract model,
    then transported to the level of the CompCert-based model
    where it can be connected to concrete code.
\end{itemize}
Our overall approach is summarized in \S\ref{sec:mainideas},
and the interaction specification models of
refinement-based game semantics 
is introduced in \S\ref{sec:rbgs}.
We have formalized our work as well as
the interaction specification model of \citet{rbgs-cal}
...
their semantics will be trivial and cannot be characterized in isolation.
Several solutions to this problem have been proposed (\S\ref{sec:rw:compcert}).
Below we outline the use of certified abstraction layers
in the verification of the operating system kernel CertiKOS,
...
which specifies the behavior of the program's individual functions:
...
$\chi$ is instantiated as the least defined behavior $\bot$.
...
can regard the functions of $\kw{rb.c}$ as \emph{primitives}
whose behavior is given by $L_\kw{rb}$:
\[
  \kw{Clight}_{L_\kw{rb}}[C] \:\le\:
  \kw{Clight}_\bot[C + \kw{rb.c}]
  \,.
\]
This means that the code of $\kw{bq.c}$
can in turn be verified
based on the layer interface defined by $L_\kw{bq}$:
\[
  \forall f \vec{v} m \:\cdot\:
...
certified abstraction layers can be \emph{vertically} composed.
For example:
\[
...
Verifying client code on top of the layer specification $L_\kw{bq}$
...
\[
  ...
  \text{if }\: |\vec{q}| < N
\]
Note that $\kw{enq}$ return the undefined value $\kw{undef}$
and that the concrete memory $m$ is unchanged.
This is because, at this level of abstraction,
the concrete variable $\kw{c1}, \kw{c2}, \kw{buf}$
have been removed from view
and are instead represented by $\vec{q}$.
...
must be related consistently with $R$.
...
We have seen that CompCertX and certified abstraction layers
adapt the whole-program semantics of CompCert
to enable compositional verification techniques.
However,
while these changes allow us to plug in the underlay behavior
as a parameter to CompCert's semantics,
this parameter must be fixed before the semantics is evaluated.
Ultimately,
the semantic model used in CompCert and CompCertX
still describes a closed system
whose behavior is mostly self-contained
between the initial invocation and its termination.

CompCertO is a recent extension to CompCert
which modifies the semantic model
to first describe the behavior of individual components \emph{in isolation}.
Instead of being provided with the behavior of external functions as a parameter,
external calls are recorded as part of
a component's externally observable behavior.
We can then define composition operators
which compute the semantics of a collection of components
by letting them interact with each other.

%and then model their interaction with each other.
%In \S\ref{sec:compcert-cal},
%we use this more flexible model
%to develop a reformulation of certified abstraction layers.
%In the remainder of this section,
%we describe the semantic model used in CompCertO
%and some of the challenges associated with it.

\paragraph{Compositional Semantics} %{{{

While CompCert assigns a close semantics to whole programs,
CompCertO assigns an \emph{open} semantics to individual translation units.
...
carrying a particular register state).
...
The transition systems of CompCertO
are assigned a type
...
\begin{align*}
  ...
  \kw{Asm}(-) :
   ...
   &\twoheadrightarrow
    \mathcal{A}@\kw{mem}
\end{align*}
Transition systems of type $E \twoheadrightarrow F$
accept incoming calls in $F$
and perform outgoing calls in $E$.
...
XXX elaborate:
CompCertO is great but it doesn't have a trace semantics.
Compositional reasoning with transition systems is kind of a PITA.
However, there is an approach which would work well:
game semantics.
Hence in the next section we figure it out.
...
\subsection{XXX CompCert stuff to put in the right spot}
...
\[
  \rho \:\subseteq\:
  \mathop\Sigma {(q^\sharp, q^\flat) \in E^\que \times F^\que} \bdot
  \mathcal{P} \big( E^\ans_{q^\sharp} \times F^\ans_{q^\flat} \big) \,.
\]
...
the low-level question $q^\flat \in F^\que_\flat$.
The subsequent answers must be related by $\rho^\ans$, so that
the high-level answer $r^\sharp \in E^\ans_{q^\sharp}$ can be realized as
the low-level answer $r^\flat \in F^\ans_{q^\flat}$
...
However, to simplify the reasoning
and avoid unintentional leakage of local state,
we mainly consider layers specifications
only transform the abstract state.
In such cases,
...
For example,
a layer interface for the ring buffer
has type
...
After the lifting, the memory is accompanied
with abstract state as in the specifications
and they can be directly composed together.
\[
...
\begin{definition}[Abstraction relation]
  ...
  \]
  satisfying the invariant property that
...
\[
  ...
  @D^\flat \circ L^\flat
\]
The layer correctness implies
the contextual refinement.
\begin{lemma}[Contextual Refinement]
  Given arbitrary client code $N$,
  the following judgment holds
  \[
    \begin{prooftree}
      \hypo{L^\flat \vdash_R^\mathcal{C} M : L^\sharp}
      \infer1{\llbracket \kw{Clight}(N) \rrbracket @ D^\sharp
        \sqsubseteq_{1 \rightarrow \hat{R}}
...
\[
  L^\flat \vdash^\mathcal{A}_{R}
  \mathbb{C}_* \circ \kw{CompCertO}(M) \circ \mathbb{C}^* :
  L^\sharp
\]
%}}}
...
Note that
it is our design choice
that the strategy implementation
do not pass the local state
to its underlay.
On one hand, strategy layers are less expressive
than CompCert layers
because there are cases
where the local states are expected be altered
by external calls.
On the other hand, by forcefully separate
the local states belong to different module,
...
in CompCert environment,
and relate the arguments
and return value correspondingly.
...
with a state.
In particular,
as mentioned in \ref{sec:stcal:state},
the strategies are designed
in a way such that
local states belong to
different module are completely separate.
Therefore,
...
\]
so that
the memory with states of other modules
are ``bounced back'' without being modified.

Second,
we introduce the \emph{memory relation}
to relate the local abstract state
used by the strategy itself
...
As a consequence,
the memory relation is promoted
to a simulation convention
in a way similar to abstraction relation.
We define
...
\begin{example}
  ...
  \begin{align*}
    \lambda_\kw{rb} & = \{ (f(v_1, v_2), \kw{set}(i, x), R_\kw{u}) \mid
                f \leadsto \kw{set},\ v_1 \mathrel{R_\kw{i}}i,\ v_2\mathrel{R_\kw{v}}x\} \\
              & \cup \{ (f(v), \kw{get}(i), R_\kw{v})\mid
                f \leadsto \kw{get},\ v\mathrel{R_\kw{i}}i\}\\
              & \cup \{ (f, \kw{inc_1}, R_\kw{i}) \mid
                f \leadsto \kw{inc_1}\} \cup \{ (f, \kw{inc_2}, R_\kw{i}) \mid
                f \leadsto \kw{inc_2}\}\\
    \lambda_\kw{bq} & = \{ (f(v), \kw{enq}(x), R_\kw{u} \mid
                f \leadsto \kw{enq},\ v\mathrel{R_\kw{v}}x)\} \\
              & \cup \{ (f, \kw{deq}, R_\kw{v})\mid 
                f \leadsto \kw{deq}\}
  \end{align*}
  where $f \leadsto \kw{fun}$ means
  the function pointer $f$ points to
  the function $\kw{fun}$
  according to the CompCert environment.
...
\]
where the abstraction relation between states and
