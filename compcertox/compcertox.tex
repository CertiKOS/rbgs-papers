...
\begin{align*}
  ...
  \mathbb{R}^\que &:= \{ (\omega, q_1, q_2) \mid (q_1, q_2, \omega) \in \rho \} \\&&
  \mathbb{R}^\ans &:= \{ (\omega, r_1, r_2) \mid (r_1, r_2) \in \omega \}
\end{align*}
Once in this form,
simulation conventions
...
\begin{align*}
  \rho^* &:= q_{E^\flat} \mapsto
    \bigsqcup_{q_{E^\sharp}, \rho^\ans
           \mid (q_{E^\sharp}, q_{E^\flat}, \rho^\ans) \in \rho}
    \underline{q}_{E^\sharp} \leadsto r_{E^\sharp} \mapsto
    \bigsqcap_{r_{E^\sharp}, r_{E^\flat} \in \rho^\ans}
    \underline{r}_{E^\flat}
  \\
  \rho_* &:= q_{E^\sharp} \mapsto
    \bigsqcap_{q_{E^\flat}, \rho^\ans
           \mid (q_{E^\sharp}, q_{E^\flat}, \rho^\ans) \in \rho}
    \underline{q}_{E^\flat} \leadsto r_{E^\sharp} \mapsto
    \bigsqcup_{(r_{E^\sharp}, r_{E^\flat}) \in \rho^\ans}
    \underline{r}_{E^\sharp}
%  \\
%  R^* &:= m_F \mapsto
...
\begin{definition}
  ...
  a set of states,
  and $L: 1 \rightarrow \mathcal{C}@(\kw{mem} \times D)$
...
that only transform the abstract data.
In such cases,
...
\begin{example}
  The interface of $L_\kw{bq} : 1 \rightarrow \mathcal{C}@S_\kw{bq}$
\end{example}
...
Promoting Clight(M) to use abstract data
...
\begin{definition}
  ...
  $R \subseteq D^\sharp \times (D^\flat \times \kw{mem})$
  can be promoted to a refinement convention
  ...
  \begin{color}{gray}
    \begin{align*}
      \hat{R} :=
      \big\{
      & f_\sharp(\vec{v}_\sharp)@(d_\sharp, m_\sharp),
        f_\flat(\vec{v}_\flat)@(d_\flat, m_\flat),\\
      & \{ r_\sharp @ (d'_\sharp, m'_\sharp),
        r_\flat @ (d'_\flat, m'_\flat)
        \mid
        d'_\sharp \mathrel{R} (d'_\flat, m'_\flat),
        m'_\sharp \mathrel{\le^\kw{m}} m'_\flat,
        r_\sharp \mathrel{\le^\kw{v}} r_\flat,
        m'_\flat \mathrel{\cong_{B - \kw{glob}(R)}} m_\flat
        \} \mid\\
      & d_\sharp \mathrel{R} (d_\flat, m_\flat),
        f_\sharp \mathrel{\le^\kw{m}} f_\flat,
        \vec{v}_\sharp \mathrel{\le^\kw{\vec{v}}} \vec{v}_\flat,
        m_\sharp \le^\kw{m} m_\flat
        \big\}
    \end{align*}
    where $B \mathrel{=} \kw{no\_perm}(m_\flat)$
    is a set of blocks contains exactly
    those without permissions in the source memory,
    and $B$ is a superset of $\kw{glob}(R)$.
  \end{color}
  [XXX: the above definition looks messy]
  
  \[
    \begin{prooftree}
      \hypo{d_\sharp \mathrel{R} (d_\flat, m_\flat)}
      \hypo{f_\sharp \mathrel{\le^\kw{m}} f_\flat}
      \hypo{\vec{v}_\sharp \mathrel{\le^\kw{\vec{v}}} \vec{v}_\flat}
      \hypo{m_\sharp \le^\kw{m} m_\flat}
      \infer4{f_\sharp(\vec{v}_\sharp)@(d_\sharp, m_\sharp) \mathrel{R^\que}
        f_\flat(\vec{v}_\flat)@(d_\flat, m_\flat)}
    \end{prooftree}
    \quad
    \begin{prooftree}
      \hypo{d_\sharp \mathrel{R} (d_\flat, m_\flat)}
      \hypo{m_\sharp \mathrel{\le^\kw{m}} m_\flat}
      \hypo{v_\sharp \mathrel{\le^\kw{v}} v_\flat}
      \infer3{v_\sharp @ (d_\sharp, m_\sharp) \mathrel{R^\ans}v_\flat @ (d_\flat, m_\flat)}
    \end{prooftree}
  \]
  \[
    \hat{R} :=
    \big\{ q_\sharp, q_\flat,
    \{
    r_\sharp, r_\flat \mid
    r_\sharp \mathrel{R^\ans} r_\flat,
    r_\flat.\kw{m} \mathrel{\cong_{B - \kw{glob}(R)}} q_\flat.\kw{m}
    \}
    \mid q_\sharp \mathrel{R^\que} q_\flat,
    B \mathrel{=} \kw{no\_perm}(q_\flat.\kw{m}),
    \kw{glob}(R) \subseteq B
    \big\}
  \]
  where $q_\flat.\kw{m}$ and $r_\flat.\kw{m}$ projects
  the memory from the question and answer and
  $\kw{no\_perm}(m)$
  is a set of blocks contains exactly
...
The derived refinement convention $\hat{R}$
partitions the memory blocks into three parts.
The blocks with permissions follow the memory extension
between source program and target program.
The blocks correspond to variables in $\kw{glob}(R)$
do not have permissions in the source program and
are related to $D^\sharp$ in the target program.
The rest is those that are not in $\kw{glob}(R)$
and do not have permissions in the source program,
and they are required to stay unchanged
...
\begin{theorem}[Tensor product of CompCert layers]
  \[
    \begin{prooftree}
      \hypo{L_1^\flat \vdash_{R_1} M_1 : L_1^\sharp}
      \hypo{L_2^\flat \vdash_{R_2} M_2 : L_2^\sharp}
      \infer2{L_1^\flat \otimes L_2^\flat \vdash_{R_1 \otimes R_2}
...
\begin{lemma}[Vertical composition of CompCert layers]
  \[
    \begin{prooftree}
      \hypo{L^\flat \vdash_{R} M : L^\natural}
      \hypo{L^\natural \vdash_{S} N : L^\sharp}
      \infer2{L^\flat \vdash_{R,S} M, N : L^\sharp}
    \end{prooftree}
  \]
\end{lemma}
...
\begin{example}
  Bounded queue perfectly fits into this model
\end{example}

Question: what is the relationship to CompCert CAL?

Outline for the rest of the section.
...
\begin{itemize}
  \item A layer \emph{implementation} consists of
    a set of states $U$,
    a~strategy specification $\Sigma : E^\flat \rightarrow E^\sharp @U$
    and a relation $\Theta \subseteq S^\sharp \times (S^\flat \times U)$
    satisfying $\Gamma^\flat \vdash^\kw{st}_\Theta \Sigma : \Gamma^\sharp$.
\end{itemize}
Lifting $\Sigma$ now essentially yields:
\[
  \Sigma@S^\flat : E^\flat @S^\flat \rightarrow E^\sharp@(U \times S^\flat)
\]
The correctness property can again be formulated as:
\[
  \Gamma^\flat \vdash^\kw{st}_\Theta \Sigma : \Gamma^\sharp
  \:\Leftrightarrow\:
  \Gamma^\sharp \sqsubseteq
  E^\sharp@\hat{\Theta}_* \circ \Sigma@S^\flat \circ \Gamma^\flat
  \,
\]
however note that the calls between $E^\sharp@\hat{\Theta}_*$
and $\Sigma@S^\flat$
now use the signature $E^\sharp @(U \times S^\flat)$
rather than simply $E^\sharp @S^\flat$,
allowing $\Sigma$ to use local state of type $U$
...
\begin{example}
  ...
  where
  $\kw{id}\subseteq S_\kw{rb} \times (1 \times S_\kw{rb})$
  lifts the identity relation implicitly
\end{example}
...
\begin{itemize}
\item A strategy $\Sigma: E^\flat \rightarrow E^\sharp @ U$
...
\item A memory relation $\mu \subseteq U \times \kw{mem}$ that explains how the
  ...
  extension
  $\hat{\mu}: \mathcal{C} @ (U \times \kw{mem})
  \Leftrightarrow \mathcal{C} @ \kw{mem}$
\end{itemize}
...
\[
  ...
  \Sigma \sqsubseteq_{(\lambda^\sharp \# \kw{mem})
    \rightarrow (\lambda^\flat \# \kw{mem})@U}
  \hat{\mu}_* \circ \llbracket \kw{Clight}(M) \rrbracket
\]

Explain the how the memory is ``echo''ed.
...
\begin{example}
  ...
  \[
    M_\kw{bq} \le^{\top}_{\lambda_\kw{rb} \rightarrow \lambda_\kw{bq}} \kw{bq.c}
  \]
...
\begin{color}{gray}
  \subsubsection{Tensor Product Composition}
  \begin{example}
    Code verification for array and counter separately
  \end{example}
\end{color}

\subsubsection{Complete Correspondence}
...
we define the following refinement convention
based on a memory relation $rho$,
and memory extension.
\[
  \rho^\kw{ext} :=
  \{
  (m_2, u), m_1,
  \{(m'_2, u), m'_1 \mid m'_1\le_\kw{m} m'_2, (u, m'_1)\in\rho, m_1 \cong_{\neg \kw{glob}(\rho)} m'_1 \}
  \mid m_1\le_\kw{m} m_2, (u, m_1)\in\rho
  \}
\]
The memory relation $\rho_1 \subseteq U \times \kw{mem}$
and $\rho_2 \subseteq V \times \kw{mem}$
can be composed as
\[
  \rho_1 \times \rho_2 \subseteq (U\times V) \times \kw{mem}:=
  \{ (u,v), m \mid (u,m) \in \rho_1, (v,m)\in\rho_2 \}
\]
with
\[
  \kw{glob}(\rho_1\times\rho_2) := \kw{glob}(\rho_1) \cup \kw{glob}(\rho_2)
\]
We can prove the following tensor product composition rule
\[
  \begin{prooftree}
    \hypo{\Sigma_1 \le^{\rho_1}_{\alpha_1 \rightarrow \beta_1} M_1}
    \hypo{\Sigma_2 \le^{\rho_2}_{\alpha_2 \rightarrow \beta_2} M_2}
    \hypo{\kw{glob}(\rho_1) \cap \kw{glob}(\rho_2) = \varnothing}
