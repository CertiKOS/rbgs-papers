...
it provides limited support for external function calls.
Semantics are defined in terms of a parameter $\chi$
which specifies ahead of time the behavior of external functions.
Certified abstraction layers \cite{popl15}
take advantage of this capability to enable compositional verification
...
\]
Since \kw{rb.c} does not rely on any external functions,
we use the least defined behavior $\bot$ as the value for $\chi$.
However,
the property above can be used to derive a form of \emph{contextual refinement}.
Any client code $C$ combined with $\kw{rb.c}$
can regard the functions of $\kw{rb.c}$ as \emph{primitive} external functions
whose behavior is given by $L_\kw{rb}$:
\[
  \kw{Clight}_{L_\kw{rb}}[C] \:\supseteq\:
  \kw{Clight}_\bot[C + \kw{rb.c}]
  \,.
\]
In particular, this means that the code of $\kw{bq.c}$
can in turn be verified in isolation,
