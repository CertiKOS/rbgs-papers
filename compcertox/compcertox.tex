...
is defined as
\[
  E\otimes F := \{ (m,n) : E^\bullet_{m} \times F^\bullet_{n}\ |\ m\in E^\circ,\ n\in F^\circ \}
\]
\end{definition}
...
lifting $R$ with $S$ is defined as
\[
  R@S: E@S  := R \otimes \kw{id}_S
\]
\end{definition}
...
\begin{figure}[t]
  \centering
  \includegraphics{absrel}
  \caption{Abstraction relation with memory extension}
  \label{fig:absrel}
\end{figure}

For example, \autoref{fig:absrel}
shows the structure derived by
combining the $\kw{CKLR}$ $\kw{ext}$
and an abstraction relation $R$.

Memory extension suffices for relating the memory states
between overlay and underlay.
Thus, layer correctness can then be defined as
\[
...
unchanged.
\[
  \iota : 1 \leftrightarrows \kw{mem} := \{ (*, m, \{(*, m)\}) \mid m \in \kw{mem} \}
\]
Thus, we have
\[
...
\begin{example}
  ...
  \begin{align*}
    \gamma_\kw{rb} & = \{ (f(v_1, v_2), \kw{set}(i, x), R_\kw{u}) \mid
              ...
              & \cup \{ (f, \kw{inc_1}, R_\kw{i}) \mid
                ...
                f \leadsto \kw{inc_2}\}\\
    \gamma_\kw{bq} & = \{ (f(v), \kw{enq}(x), R_\kw{u} \mid
...
\begin{lemma}
  ...
  $R$ and $\rho$:
  \[
    R;\rho := \{ (s_2, (m, s_1)) \mid \exists u. (s_2, (u, s_1)) \in R \wedge (u, m) \in \rho\}
  \]
\end{lemma}
...
\item Unsolved problems
  \begin{itemize}
  \item Tensor product composition on code verification,
    in particular how to design
    the memory abstraction relation
  \end{itemize}
\item Narratives in \S\ref{sec:cal}
  \begin{itemize}
  \item Notations for code verification, like $\gamma$, $\rho$, ...
    Need a discipline on when to use Greek letters and
    when to use English letters.
  \item Fill in the missing definitions and explanations
  \item Merge the two parts
  \item Reorganize \S\ref{sec:cal:compare},
    some parts should be moved to introduction
  \end{itemize}
\item Figures to draw
  ...
  \item need a diagram to explain how the abstraction relation
    is extended with a memory extension (or CKLR in general)
