...
The framework described below is mechanized in the Rocq prover and
...
the process pipe operator $P \mathbin| Q$,
where $P$ and $Q$ are executed sequentially---%
...
between components at different levels of abstraction.
Overall,
...
\item Flat composition ($\oplus$)
  ...
  We will not discuss it extensively here.
...
\begin{figure} % fig:hvcomp {{{
  ...
  \begin{array}{c}
    \begin{tikzcd}[sep=0.5ex]
      ...
      & \phi & \\
      E_2 \ar[rr, twoheadrightarrow, "L_2"'] &&
      F_2
    \end{tikzcd}
  \end{array}
  \quad
...
which specifies for each question $m \in E$
...
the environment of a component $L$ can activate it by asking a question $q \in F$,
which the component is expected to eventually answer
...
\\
  ...
  \,.
\end{align*}
Note that the programs in Fig.~\ref{fig:abc} are simple enough that
...
$,
where the signature $\emptysig$ is empty,
and the signature $\mathcal{C} \mathbin@ \kw{mem} \mathbin@ D$
...
This refinement property assumes that incoming
...
which specify how source-level calls are realized
...
\[
  E \mathbin@ U \: := \:
    \{ m @ u : N \times U \mid (m : N) \in E, \, u \in U \}
  \,,
\]
