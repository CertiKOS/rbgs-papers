...
Second,
we will focus on incorporating \emph{specific} effects
into the framework,
...
For example,
even a system involving randomness
will usually consist almost entirely of deterministic code.
The probabilistic behavior of this code only emerges through
...
\[
   ...
   \,,
\]
which for each state and incoming question $m \in E$
...
NB: with $\mathrm{ITree}_E \looparrowright F$
up to tau we recover strategies.
...
to yield a new specification $\sigma[L] : M \looparrowright F$.
To handle an incoming question in $F$,
the layer specification $\sigma[L]$
first executes the strategy $\sigma$.
When $\sigma$ performs an outgoing call in $E$,
