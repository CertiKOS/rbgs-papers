...
The principles outlined above are well-known,
and the use of the transitivity of $\le$
may seem so standard that it is barely worth noting.
However we would like to invite the reader
to take a moment to consider the high-level
algebra of composition that
transition systems and simulations conform to.
In what we have described above:
\begin{itemize}
  ...
  \item Simulations are 1-dimensional objects,
    in the sense that they can be connected
    with each other along 0-dimensional boundaries
    (their endpoints).
    They are similar to edges or paths.
\end{itemize}
Seen in this way,
...
As illustrated in Figure~\ref{fig:pasting},
as we incorporate more composition principles into the framework,
the dimensionality of the systems we consider
and of the correctness proofs we wish to construct
will increase accordingly.
The principle will remain, however,
that objects of dimension $n+1$ can be connected
alongside a boundary of dimension $n$.
...
\begin{figure} % fig:pasting {{{
  \text{(a)}
  \begin{tikzcd}
    \kw{Clight}[p_0] \ar[d, "\rho", Rightarrow] \\
    \kw{RTL}[p_i] \ar[d, "\pi", Rightarrow] \\
    \kw{Asm}[p_n]
  \end{tikzcd}
  \qquad
  \text{(b)}
  \begin{tikzcd}[sep=tiny]
    \bullet \ar[dd, equal] \ar[rrrr, dash, "\Sigma"] &&&&
    \bullet \ar[dd, equal]
    \\
    && \phi
    \\
  ...
  \begin{tikzcd}[sep=tiny]
    ...
    & \rho_1 & & \rho_2
    \\
    \mathcal{A} \ar[rr, equal] \ar[dd, equal] &&
    \mathcal{A} \ar[dd, equal] \ar[rr, "\kw{Asm}(\kw{rb.s})"'] &&
    \mathcal{A} \ar[rr, "\kw{Asm}(\kw{bq.s})"'] &&
    \mathcal{A} \ar[dd, equal]
    \\
    & \kw{id} &&& \pi
    \\
    \mathcal{A} \ar[rr, equal] &&
    \mathcal{A} \ar[rrrr, "\kw{Asm}(\kw{rb+bq.s})"'] && &&
    \mathcal{A}
  \end{tikzcd}
  \caption{%
...
For example,
Figure~\ref{fig:code} shows two C translation units.
...
Transition systems are redefined so that they
characterize the behavior of individual compilation units,
and model function calls across different components:
\begin{itemize}
\item
To initialize a transition system,
we must now specify the name $f \in \kw{ident}$
of the function that we wish to invoke,
values $\vec{v} \in \kw{val}^*$ for its parameters,
and the state $m \in \kw{mem}$ of the global memory
...
a description of the external call,
...
\begin{equation}
 \begin{array}{c}
  ...
  \qquad
  Y^s \subseteq (\kw{val} \times \kw{mem}) \times S
 \end{array}
...
Specifically,
in the transition system $L_1 \oplus L_2$,
whenever $L_1$ makes an external call
to a function provided by $L_2$ (and vice versa),
a new instance of $L_2$ is instantiated to handle the call,
executes until a final state of $L_2$ is reached,
at which point the execution's outcome is used to resume
...
it makes it possible to describe the semantics of programs
...
\]
Note that we use round parentheses to denote
...
An equivalent convention (used in Fig.~\ref{fig:pasting}b)
is to consider them ``loops'' of type ${\bullet} \rightarrow {\bullet}$
over a trivial 0-dimensional endpoint $\bullet$.
Simulations now connect 1-dimensional transition systems,
and therefore they in turn become 2-dimensional objects.
As before,
simulations compose vertically when the target transition system of one
...
of the semantic model of Compositional CompCert:
\[
  \begin{tikzcd}[row sep=tiny, column sep=small]
    & {} \ar[dd, Rightarrow, "\rho_1"] & & {} \ar[dd, Rightarrow, "\rho_2"] \\
  ...
  \begin{tikzcd}[row sep=tiny]
    & {} \ar[dd, Rightarrow, "\rho_1 \oplus \rho_2"] & \\
    \bullet
...
\begin{example}
The example given in Fig.~\ref{fig:pasting}b
illustrates how this compositional algebra can be used.
To capture the operation of the code in Fig.~\ref{fig:code},
we can formulate a specification
for the bounded queue operations $\kw{enq}$ and $\kw{deq}$
as a simple transition system $L_\kw{bq}$,
the show that $\kw{bq.c}$ and $\kw{rb.c}$
together implement $L_\kw{bq}$ using a simulation
\[
  \pi \::\: L_\kw{bq}
...
\]
In other words,
properties such as (\ref{eqn:cex}) induce contextual refinement.
\end{example}

\paragraph{String Diagrams}

[This is a symmetric monoidal category.
Any such admits string diagram notation.]
...
In this section, we revisit our running example,
and retrofit it onto the encapsulation framework.

\subsection{Ad-hoc Verification}

Putting together the scattered pieces in \S\ref{sec:overview},
the final correctness theorem is established as follows.
The proof is present in two-fold:
the first part proves that
the high level specification $L_\kw{bq}$
can be refined by
the program $\kw{bq.c}$
running on top of the ring buffer specification
using the witness relation $R$.
\[
  \begin{prooftree}
    \hypo{L_\kw{bq} \le_{\top \twoheadrightarrow [R]} \Sigma_\kw{bq} @ D_\kw{rb} \odot L_\kw{rb}}
    \infer1{L_\kw{bq} @ \kw{m} \le_{\top \twoheadrightarrow [R]} \Sigma_\kw{bq} @ (\kw{m} \times D_\kw{rb}) \odot L_\kw{rb} @ \kw{m}}
    \hypo{\Sigma_\kw{bq} @ (\kw{m} \times D_\kw{rb}) \le \kw{bq.c} @ D_\kw{rb}}
    \infer2{L_\kw{bq} @ \kw{m} \le_{\top \twoheadrightarrow [R]} \kw{bq.c} @ D_\kw{rb} \odot L_\kw{rb} @ \kw{m}}
  \end{prooftree}
\]
and then the implementation that manipulates $D_\kw{rb}$
is further refined to the programs
that operates on the memory,
witnessed by the relation $S$.
\[
  \begin{prooftree}
    \hypo{L_\kw{bq} @ \kw{m} \le_{\top \twoheadrightarrow [R]} \kw{bq.c} @ D_\kw{rb} \odot L_\kw{rb} @ \kw{m}}
    \hypo{\kw{bq.c} @ D_\kw{rb} \le_{[S] \twoheadrightarrow [S]} \kw{bq.c}}
    \hypo{L_\kw{bq} @ \kw{m} \le_{\top \twoheadrightarrow [S]} \kw{rb.c}}
    \infer3{L_\kw{bq} @ \kw{m} \le_{\top \twoheadrightarrow [R]\cdot[S]} \kw{bq.c} \odot \kw{rb.c}}
  \end{prooftree}
\]

Observe that the explicitly adjoined state becomes overwhelming.
In contrast,
once the abstract states in the specifications
...
which is almost the ideal version in \S\ref{sec:overview:slift}.
\[
  \begin{prooftree}
    \hypo{\Gamma_\kw{bq} @ \kw{m} \sqsubseteq \Sigma_\kw{bq} @ \kw{m} \circ \Gamma_\kw{rb} @ \kw{m}}
    \hypo{\Sigma_\kw{bq} @ \kw{m} \sqsubseteq \kw{bq.cp}}
    \infer2{\Gamma_\kw{bq} @ \kw{m} \sqsubseteq \kw{bq.cp} \circ \Gamma_\kw{rb} @ \kw{m}}
    \hypo{\Gamma_\kw{rb} @ \kw{m} \sqsubseteq \kw{rb.c}}
    \infer2{\Gamma_\kw{bq} @ \kw{m} \sqsubseteq \kw{bq.cp} \circ \kw{rb.cp}}
  \end{prooftree}
\]

\subsection{Certified Abstraction Layers(CAL)}

In theory,
the frameworks, such as CAL,
are capable of handling the boilerplates to produce
a unified interface across different components.
However, CAL, although successful on verify CertiKOS,
suffers from two limitations.
[explain how abstraction relation works].
On one hand, the abstraction relations
are not vertically composible,
because of the consistency issue.
On the other,
there is no horizontal composition.
As a consequence,
the abstract state used for verify CertiKOS
is represented as a huge record
which contains the states used by all the layers.

With the state encapsulation, however,
the abstraction relation can be encapsulated
together with the abstract state.
\[
  L^\flat: \top \rightarrow \mathcal{C} \vdash M : L^\sharp:\top \rightarrow \mathcal{C} :\Leftrightarrow L^\sharp @ \kw{mem} \le \kw{ClightP} \langle M \rangle \circ L^\flat @ \kw{mem}
\]
and the vertical composition can be proved directly.
\[
  \begin{prooftree}
    \hypo{L^\flat \vdash M : L^\natural}
    \hypo{L^\natural \vdash N : L^\sharp}
    \infer2{L^\flat \vdash M + N : L^\sharp}
  \end{prooftree}
\]
Moreover,
given a proper notion of horizontal composition,
a version of horizontal composition can be proved
by exploiting the composition of memory fragments.
\[
  \begin{prooftree}
    \hypo{L_1^\flat \vdash M : L_1^\sharp}
    \hypo{L_2^\flat \vdash N : L_2^\sharp}
    \infer2{L_1^\flat \uplus L_2^\flat  \vdash M + N : L_1^\sharp \uplus L_2^\sharp}
  \end{prooftree}
\]
