...
full-blown state encapsulation of the kind we have presented,
or the more general language interfaces of CompCertO
which underlie our support of data abstraction.
...
However these theories have not been mechanized
or interfaces with CompCert.
...
do not attempt to provide program-level verification facilities,
but rather focus on the more coarse-grained, module-level ``glue''.
Likewise,
...
Nevertheless,
program logics in general and separation logics in particular
are relevant to our work in two different ways.

For practical purposes, it would be beneficial to incorporate
such a program logic into our framework.
For example, the CAL framework of \citet{popl15} provides
a rudimentary Clight program logic which
can be used to establish 
some of the elementary correctness properties
that are used to prove layers correct.
In our case,
it may be useful to investigate whether
the Verified Software Toolchain \cite{vst},
which provides a mechanized separation logic for Clight,
could be interfaced with our model.

Moreover,
spatial compositionality is in fact
the defining feature of separation logic.
Our own treatment of memory separation
draws extensively from
separation algebra \cite{sepalg},
which in turn was defined to build models of separation logic.
Work such as
Conditional Contextual Refinement (CCR) \cite{ccr}
combines refinement and separation logic into
a unified, mechanized framework.
As such it is also a study of the interaction between
spatial and vertical compositionality.
CCR however does not support for state encapsulation
and does not interface with a real-world certified compiler.
...
Explain:
\begin{itemize}
\item In CompComp and CompCertM \ldots
\item In CAL, \ldots
\item
