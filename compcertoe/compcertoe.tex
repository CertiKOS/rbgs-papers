...
In this section,
we give a more detailed characterization
of the ClightP language(\S\ref{sec:application:clightp}),
and put together a complete proof
for our running example (\S\ref{sec:application:ad-hoc}),
...
However, it is noticable that
the simulation convention for $\phi'_3$
is unwieldy because the program $\kw{rb.c}$
implements the state $S_\kw{rb}$ using its memory state.
Moreover, it is requied to prove a relation $R\_kw{rb}$
hold between the abstract state and the memory state
in order to verify the correctness of $\kw{rb.c}$.
As the state grows larger,
the relation becomes more complicated and hard to maintain.
To solve these two issues,
we introduce ClightP.
...
though hidded,
is agnostic to the CompCert memory.
...
The correctness of compilation
from $\kw{ClightP}$ to $\kw{Clight}$
is formulated as
\[
...
\]
The proof can be decomposed as Figure~\ref{fig:clightp-correct},
