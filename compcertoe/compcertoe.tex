...
and we demonstrate multiple use cases.
...
than the one constructed in CompCertO around
...
to a function $f : X \rightarrow Y$ yields the notation
\[
  U \times f + V : U \times X + V \rightarrow U \times Y + V
  \qquad \text{(rather than }
...
\[
  ...
  \,.
\]
Consider also a blank string diagram---%
...
to obtain the property illustrated in Fig.~\ref{fig:3dsd}a:
\[
  \phi_\kw{bq} \: :=
...
is no coincidence.
In fact,
we can use the separation algebra as a relation
$\jr \subseteq (\kw{mem} \times \kw{mem}) \times \kw{mem}$
to describe the frame property for a transition system
...
$\zeta : \epsilon \mathbin{R_\kw{bq}} d_0$
...
\begin{figure} %fig:cal {{{
  ...
  \quad
    ...
    \coordinate (join-l) at (0,1,4);
    \coordinate (join-r) at (8,1,4);
    ...
    [rounded corners] -- (1,0,2)
    [sharp corners] -- (2,0,3)
    [rounded corners] -- (3,0,2) -- (8,0,2);
    % mem
    \draw (join-l) -- (0,0,4)
    [rounded corners] -- (1,0,4)
    [sharp corners] -- (2,0,3)
    [rounded corners] -- (3,0,4) -- (8,0,4);
    % mem join
    \draw (0,6,3.5) [rounded corners] -- (0,1.5,3.5) -- (join-l);
    \draw (0,4,5) [rounded corners] -- (0,1.5,5) -- (join-l);

    % Nodes
    \node[tsn] at (2,0,3) {\Large$\,N\,$};
    \node[scn,bln] at (0,4,5) {};
    ...
    % C
    \draw (0,6,2)
    [rounded corners] -- (1,6,2)
    [sharp corners] -- (2,6,3)
    [rounded corners] -- (3,6,2)
    [sharp corners] -- (8,6,2) -- (8,0,2);
    % mem used by N on top
    \draw (0,6,3.5)
    [rounded corners] -- (1,6,3.5)
    [sharp corners] -- (2,6,3)
    [rounded corners] -- (3,6,3.5) -- (7,6,3.5);
    % mem join
    \draw (8,5,3.5) [rounded corners] -- (8,1.5,3.5) -- (join-r);
    \draw (8,4,5) [rounded corners] -- (8,1.5,5) -- (join-r);
    \draw (join-r) -- (8,0,4);

    % Nodes
    \node[tsn] at (2,6,3) {\Large$\,N\,$};
    \node[scn,bln] at (8,4,5) {};
    \node[tsn,bln] at (7,6,3.5) {};
    \node[scn,bln] at (8,5,3.5) {};
    ...
    % Background
    \fill[tssdbg, opacity=0.6]
      (0,3,0) -- (0,3,6) -- (8,3,6) -- (8,3,0) -- cycle;

    % Strings
    % C
    \draw (0,3,2)
    [rounded corners] -- (1,3,2)
    [sharp corners] -- (2,3,3)
    [rounded corners] -- (3,3,2) -- (8,3,2);
    % mem pass through
    \draw (0,3,5) -- (8,3,5);
    % mem used by N
    \draw (0,3,3.5)
    [rounded corners] -- (1,3,3.5)
    [sharp corners] -- (2,3,3)
    [rounded corners] -- (3,3,3.5) -- (8,3,3.5);

    % Nodes
    \node[tsn] at (2,3,3) {\Large$\,N\,$};

    %% Vertical middle

    % Background
    \fill[tssdbg,opacity=0.6]
      (4,3,0) -- (4,6,0) -- (4,6,6) -- (4,3,6) -- cycle;

    % Strings
    % pass through memory
    \draw (4,4,5) -- (4,3,5);
    % memory used by N
    \draw (4,6,3.5) -- (4,3,3.5);
    % C
    \draw (4,6,2) -- (4,3,2);

    % Nodes
    \node[scn,bln] at (4,4,5) {};
  \end{tikzpicture} }
  \qquad
  \text{(b)}
  \quad
  \raisebox{-0.5\height}{\begin{tikzpicture}[sdp]
    ...
    \coordinate (join) at (0,2,4);
    \coordinate (d2) at (1,6,4.5);
    \coordinate (d1) at (4.2,0,4);
    \coordinate (m0) at (0,3,5);
    \coordinate (R) at (1,4.5,4.5);
    %% Left and bottom faces
    ...
    [rounded corners] -- (1,0,1.5)
    [sharp corners] -- (2,0,3)
    [rounded corners] -- (3,0,1.5) -- (5,0,1.5)
    [sharp corners] -- (6,0,3);
    % mem
    \draw (join) -- (0,0,4)
    [rounded corners] -- (1,0,4)
    [sharp corners] -- (2,0,3)
    [rounded corners] -- (3,0,4) -- (3.5,0,4);
    % d_1
    \draw (d1)
    [rounded corners] -- (5,0,4) -- (6,0,3);
    % Top
    \draw (6,0,3) -- (8,0,3);
    % <mem]^*
    \draw (0,5,3)
    [rounded corners] -- (0,3,3) -- (join);
    % [m_0>_*
    \draw (m0)
    [rounded corners] -- (0,2.5,5) -- (join);

    % Nodes
    \node[tsn] at (2,0,3) {\Large$\,M\,$};
    \node[tsn] at (6,0,3) {\Large$\,L_1\,$};
    \node[tsn,bln] at (3.5,0,4) {};
    \node[tsn,bln] at (d1) {};
    \node[scn,bln] at (0,5,3) {};
    \node[scn,bln] at (m0) {};
    \node[sct] at (join) {$\bullet$};

    %% Internals
    \draw[dashed,thin] (d2)
    [rounded corners] -- (R) -- (m0);
    \draw[dashed,thin] (R) [rounded corners] -- (2,2,3) -- (d1);
    \node[scn] at (R) {\footnotesize$R$};
    ...
    % C
    \draw (0,6,1.5)
    [rounded corners] -- (3,6,1.5) -- (4,6,3);
    % Top
    \draw (4,6,3) -- (8,6,3) -- (8,0,3);
    % d_2
    \draw (d2)
    [rounded corners] -- (3,6,4.5) -- (4,6,3);
    % Nodes
    \node[tsn] at (4,6,3) {\Large$\,L_2\,$};
    \node[tsn, bln] at (d2) {};
  \end{tikzpicture} }
  \caption{Shown here are:
    (a) a property of \kw{Clight} semantics that is used
    to prove composition of abstraction layers.
    The bottom part is the frame rule,
    the top left part is $\phi_1(N) \mathbin@ \phi_2(\kw{m_0})$,
    and the top right part is $\mathcal{C} \mathbin@ \langle \kw{mem}]_\circlearrowleft \mathbin@ \phi_2(\kw{m_0})$,
    where $\phi_1(N) := \kw{Clight}(N)\le\kw{Clight}(N)$
    and $\phi_2(\kw{m_0}) := [\kw{m_0}\rangle \sqsubseteq [\kw{m_0}\rangle $;
    (b) layer correctness for $L_1 \vdash_R M : L_2$ }
  \label{fig:cal}
\end{figure}
%}}}
...
$L_1 \vdash_R M : L_2$,
which says that
the behavior of an overlay specification $L_2: \top \twoheadrightarrow \mathcal{C} @ D_2$
is refined by an implementation $M: \mathcal{C} \mathbin@ \kw{mem} \twoheadrightarrow \mathcal{C} \mathbin@ \kw{mem}$
executed on top of an underlay $L_1: \top \twoheadrightarrow \mathcal{C} @ D_1$
witnessed by the abstraction relation $R \subseteq D_2 \times (\kw{mem} \times D_1)$.

With the techniques from previous sections,
we can interpret the abstraction layers as
\[
  L_1 \vdash_R M : L_2 \: :\Leftrightarrow \:
  \Gamma_2 \le_{\varnothing \twoheadrightarrow \mathcal{C}\mathbin@\langle \kw{mem} ]^* \bullet [\kw{m_0}\rangle_* } \kw{Clight}(M) \odot (\mathcal{C} \mathbin@ \langle \kw{mem} ]) \odot \Gamma_1
\]
which is depicted in Figure~\ref{fig:cal}b, and
\[
  \Gamma_1 := (\mathcal{C} \mathbin@ [ d_1 \rangle ) \odot L_1 : \top \twoheadrightarrow \mathcal{C} \qquad
  \Gamma_2 := (\mathcal{C} \mathbin@ [ d_2 \rangle ) \odot L_2 : \top \twoheadrightarrow \mathcal{C}
\]
are encapsulated layer interfaces,
and the initial states satisfies the relation
$d_2 \mathbin R (m_0, d_1)$.

\begin{example}
The ring buffer and bounded queue
can be viewed as special cases
of certified abstraction layers.
The ring buffer layer is built on top of an empty underlay,
and bounded queue layer does not introduce state.
\end{example}
...
\paragraph{Composotion}
Given another abstraction layer
$L_2 \vdash_S N : L_3$,
the composition of layers relies on
the property shown in Figure~\ref{fig:cal}a.
% \[
%   \kw{Clight}(N) \odot (\mathcal{C} \mathbin@ \langle \kw{mem} ])
%   \le_{\mathcal{C} \mathbin@ \langle\kw{mem}]_* \bullet [ \kw{m_0} \rangle^* \twoheadrightarrow
%     \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{m_0} \rangle^* }
%   \kw{Clight}(N)
% \]
The property can be composed
with the layer correctness of $M$ directly,
% \[
%   \kw{Clight}(N) \odot (\mathcal{C} \mathbin@ [\kw{mem} \rangle) \odot \Gamma_2
%   \le_{ \varnothing \twoheadrightarrow \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{m_0} \rangle^* }
%   \kw{Clight}(N) \odot \kw{Clight}(M) \odot (\mathcal{C} \mathbin@ \langle \kw{mem} ]) \odot \Gamma_1
% \]
and then the correctness of $N$
can be put on top of it:
\[
  \Gamma_3 \le_{\varnothing \twoheadrightarrow \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{n_0} \rangle^*
    ; \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{m_0} \rangle^* }
  \kw{Clight}(N) \odot \kw{Clight}(M) \odot (\mathcal{C} \mathbin@ \langle \kw{mem} ]) \odot \Gamma_1
\]
Finally,
by using refinement between simulation conventions
\[
  \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{n_0 \bullet m_0} \rangle^*
  \sqsubseteq
  \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{n_0} \rangle^*;
  \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{m_0} \rangle^*
\]
we get the composite layer correctness:
\[
  \Gamma_3 \le_{\varnothing \twoheadrightarrow \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{n_0 \bullet m_0} \rangle^* }
  \kw{Clight}(N) \odot \kw{Clight}(M) \odot (\mathcal{C} \mathbin@ \langle \kw{mem} ]) \odot \Gamma_1
\]
where $n_0 \bullet m_0$ denotes
the combined initial memory states,
and by the property of memory separation
we have
$d_3 \mathbin S (n_0 \bullet m_0, d_2)$
and
$d_2 \mathbin R (n_0 \bullet m_0, d_1)$.
...
\subsection{Ad-hoc Verification (To be moved)}
\label{sec:application:ad-hoc} %{{{
...
\[
  ...
  \qquad
  \phi_\kw{rb}' \: : \: \Gamma'_\kw{rb}
    \le_{\varnothing \twoheadrightarrow \mathbf{R}(m_0) }
    \kw{Clight}(\kw{rb.c})
\]
where $\mathbf{R}(m_0) := \mathcal{C} \mathbin@ (\langle \kw{mem} ]^* \mathbin@ [m_0\rangle_*) \mathbin; {\bullet}$.
To put together the individual pieces,
we weaken $\phi_2$ to
\[
  \phi'_2 \: : \: \Sigma_\kw{bq}
  \le_{ \mathbf{R}(m_0) \twoheadrightarrow \mathbf{R}(m_0) }
    \kw{Clight} (\kw{bq.c})
\]
...
\[
\phi_\kw{bq} \: : \:
\Gamma'_\kw{bq} \le_{\varnothing \twoheadrightarrow R(m_0) }
    \kw{Clight}(\kw{bq.c}) \odot \kw{Clight}(\kw{rb.c})
    \: := \:
