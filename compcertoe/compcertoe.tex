...
\begin{figure}
  \[
    (a) \quad \raisebox{-0.5\height}{\includegraphics[scale=0.1]{pr/phi}}
    \qquad \qquad
    (b) \quad \raisebox{-0.5\height}{\includegraphics[scale=0.06]{ccex}}
  \]
  \caption{The general form (a) and a particular example (b)
...
\begin{figure}
  \[
    \begin{prooftree}
      ...
      \hypo{\Gamma_\kw{rb} @ \kw{m} \sqsubseteq \kw{rb.cp}}
      \infer3{\Gamma_\kw{bq} @ \kw{m} \sqsubseteq \kw{bq.cp} \circ \Gamma_\kw{rb} @ \kw{m}}
      \infer1{\Gamma_\kw{bq} @ \kw{m} \sqsubseteq \kw{bq.cp} \circ \kw{rb.cp}}
    \end{prooftree}
    \qquad
    \begin{tikzpicture}[xscale=0.55,yscale=0.45,baseline=1.24cm]
      % Background
...
However, CAL, although successful on verifying CertiKOS,
...
because of the lack of ability to
merge memory fragments.
As a consequence,
the abstract state used for verifying CertiKOS
has to be represented as a huge record
