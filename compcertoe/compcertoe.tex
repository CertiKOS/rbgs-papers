...
and basics of the Unix execution environment.
...
Our model is generic and flexible enough
to account for the CompCertO semantics of C and assembly translation units,
to model the kind of process behavior and communication
required to handle Example~\ref{ex:readwritehello},
and to formally express (\ref{eqn:hellospec}).
...
\end{itemize}
As we will demonstrate with several examples,
these properties can then be combined
along multiple dimensions of compositionality
...
Compositional semantics allow the meaning of a composite program
can be calculated from the meaning of its syntactic parts.
Borrowing from the compositional compiler correctness literature,
we call this \emph{horizontal} compositionality.
At the same time, work on compositional proof techniques
for program and compiler verification
has shown that in these contexts,
compositionality along several other dimensions
can be just as important.

Our goal is to show that \emph{vertical} and \emph{spatial} compositionality
can be given a semantic account as well,
and that the resulting models can be used to tackle verification problems
which have so far been of reach of both
compositional semantics and compositional proof techniques,
because they are more expressive than traditional semantic models,
and unlike specialized proof techniques
they allow horizontal, vertical and spatial composition principles
...
where the term $m(k_i)_{i \in N}$ is a computation
which triggers the effect $m$ with the possible continuations $k_i$.
Our terminology
%of \emph{questions} and \emph{answers}
follows more closely the \emph{game semantics} tradition,
...
The semantic model of CompCertO uses interfaces
of the form $A = \langle A^\circ, A^\bullet \rangle$
as the basis for component interactions.
Language interfaces are similar to effect signatures,
...
the decomposition $\mathcal{C} \mathbin@ \kw{mem}$ below.
\end{example}
%}}}
...
Our model uses effect signatures to assign coarse types
to program modules and other components.
A~component $L : E \rightarrow F$
\emph{uses} the operations of the signature $E$ to
\emph{implement} the operations in~$F$.

As depicted in Fig.~\ref{fig:overview:ts}a,
the environment asks a question $q \in F$ to activate $L$,
which is expected to eventually provide an answer $r \in \kw{ar}(q)$.
In the process,
...
can be translated to our model as well.
For example,
...
Note that this trace is more complicated than the one shown in Example~\ref{ex:decodespec};
...
\subsection{Interaction and Layered Composition} %{{{
...
\subsection{Data Abstraction and Vertical Composition} %{{{
...
\subsection{Combining Effect Signatures} %{{{
...
can be combined into $P \mathbin\texttt{;} Q : \mathcal{C} \rightarrow \mathcal{P}$.
To this end,
