...
% Preamble {{{
Our model uses a standard game semantics approach,
representing strategies as sets of plays %of the form
\[
  q \rightarrowtail
...
At the same time,
our constructions go beyond traditional game semantics.
Refinement squares in particular
cannot be formulated in terms of individual plays.
Therefore we supplement this traditional strategy construction
with a novel (co-)algebraic lens,
explaining the strategy construction in operational terms
and enriching the mathematical toolbox we use
to manipulate strategies.
...
The functor $\hat{E}$ captures the behavior of
a call into the signature $E$.
Conversely,
we can consider the behavior of a \emph{handler} for the call
which itself exhibits a behavior in $T : \mathbf{Sup}\rightarrow\mathbf{Sup}$.
\[
  \hat{E} X \: := \:
    \bigoplus_{q \in E} \Biggl( \bigwith_{r \in E} X \Biggr)_\bot
  \qquad
  \check{E}[T](X) \: := \:
    \bigwith_{q \in E} \, T\Biggl( \bigoplus_{r \in \kw{ar}(q)} X_\bot \Biggr)
\]
Computations in $\check{E}_T$
first await a query $q \in E$,
perform a computation over~$T$,
then produce an answer $r \in \kw{ar}(q)$.
Our strategy model instantiates $T$ with
the free monad over another signature.

\begin{definition}
A \emph{strategy} of type $\sigma : E \twoheadrightarrow F$
is an element of the lattice
$S_{E \twoheadrightarrow F} := \check{F}[\hat{E}^*]^*(\mathbf{0})$.
\end{definition}

[Connect to the play model]
...
The free monad $\hat{E}^* X$
models computations which can invoke
the operations of the signature $E$
an arbitrary number of times,
and can eventually continue as computations of $X$.

\[
  [E \rightarrow F] X :=
    \bigwith_{q \in F} E^* \left(
      \bigoplus_{r \in \kw{ar}(q)} X_\bot \right)
\]
...
then provides the answer $r \in \kw{ar}(q)$
