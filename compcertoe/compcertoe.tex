...
In particular,
semantics given in the \emph{operational} style
are often considered suitable to use as a trustworthy ``ground truth''
of program behavior,
because their definition closely mirrors
...
and can be used to establish properties such as
correctness with respect to Hoare-style specifications,
contextual equivalence, contextual refinement,
and many others.

Among these techniques,
program logics have found
broad adoption.
Modern incarnations can deal with
complex memory layouts,
concurrency,
and other sophisticated language features
while supporting
...
That is,
they satisfy the informal specification:
\begin{equation}
...
can mentally execute them step by step
and convince themself that they will work as expected.
However, this task is complex in its own way
because it mobilises implicit assumptions and knowledge regarding
the C and x86 assembly languages,
the compiler's correctness \emph{with respect to a specific calling convention},
and the basics of Unix execution environments and system calls.

Likewise, any mathematical account of property (\ref{eqn:hellospec})
must involve these aspects of the problem,
...
These limitations of traditional approaches
have long been recognized,
and a fair amount of work has sought to address them.
...
provides a separation logic for CompCert's source language Clight;
its soundness proof interfaces with
the compiler's proof of semantics preservation,
ensuring that the correctness properties obtained for a Clight program
can be formally transferred to the assembly code emitted by CompCert.
Operational semantics \cite{opsem-multi} and
program logics \cite{melocoton}
have been developed for multi-language programs.
Finally,
a network server was verfied by
by incorporating \emph{interaction trees} into VST
to model external interactions \cite{itrees}.

These efforts show that overcoming the various limitations of
the program logic paradigm is possible,
but they only constitute one-off adaptations
to particular setups.
To apply this methodology to Example~\ref{ex:readwritehello}
we would need to investigate and develop a semantics and logic 
tailored to the situation at hand,
which would combine particular features.
...
\begin{figure} % fig:paradigms {{{
  ...
  \caption{
    ...
    By using compositional semantics as a ground truth instead (right),
...
relying on the proof techniques
to achieve compositional reasoning.
...
the behavior of \emph{open} components and
...
allowing us to tackle complex verification problems
in a flexible manner.

%}}}

\subsection{Contributions: Three dimensions of compositionality} %{{{

To demonstrate how various
compositional verification techniques
can be adapted
to the setting of compositional semantics,
we present a semantic model which simultaneously admits
\emph{horizontal}, \emph{vertical} and \emph{spatial}
composition principles,
allowing us to give a semantics-level account of
techniques such as data abstraction and memory separation.

While the model does not focus on a particular language,
our generic notion of component is flexible enough
to account for the CompCertO semantics of C and assembly translation units, and
to model the kind of process behavior and communication
required to handle Example~\ref{ex:readwritehello}.

Moreover,
by extending semantics compositionality
in new directions,
we obtain a model rich enough to capture---%
...
\end{itemize}
As we will demonstrate in the context of several examples,
these properties can then be combined
along the three dimensions %of compositionality
to construct sophisticated refinement proofs,
including a formal statement of (\ref{eqn:hellospec}).

We present a high-level overview of our work in \S\ref{sec:overview},
starting with our analysis of
the horizontal, vertical and spatial dimensions of compositionality
which are found in various program logics and other proof techniques,
but less commonly understood in terms of compositional semantics.
A technical description of our model is given in \S\ref{sec:model}.
We then showcase in \S\ref{sec:eval} several uses of our model
...
\end{itemize}
We discuss of related work in \S\ref{sec:related}
and our conclusions in \S\ref{sec:conclusion}.
Our work has been mechanized in the Coq proof assistant
and we will submit a preliminary artifact alongside this paper.

%}}}

%}}}

\section*{Postpone to Main Ideas}

\begin{example}[Verifying a bounded queue] \label{ex:bq} %{{{
The code shown in Fig.~\ref{fig:code}
implements a bounded queue with capacity $N$.
This is done in two steps.
The translation unit $\kw{rb.c}$
implements a rudimentary ring buffer by
providing access to an array of size $N$
and two counters which wrap around to zero when they reach the value $N$.
...
%Although the code described in Example~\ref{ex:bq} is simple,
...
Our goal in this paper is to present a compositional semantic model
capable of capturing the following ones simultaneously.

\paragraph{Horizontal Composition} %{{{

A proof technique or semantic model is usually deemed
\emph{compositional}
...
\paragraph{Data Abstraction and Vertical Composition} %{{{

The code in Fig.~\ref{fig:code}
stores the queue's data in the array $\kw{buf}$
at indices $\kw{c1} \ldots \kw{c2}-1$.
However,
once the queue has been implemented,
we no longer need to care about the details
of this low-level representation.
A developer using the interface provided by $\kw{bq.c}$
will picture the queue as a sequence of values
which should never exceed length $N$,
and they will understand the behavior of the operations
$\kw{enq}$ and $\kw{deq}$
in those terms.

As shown in Fig.~\ref{fig:spec},
this intuition can be formalized.
There, the specification $\Gamma_\kw{bq}$
expresses the behavior of $\kw{enq}$ and $\kw{deq}$
with respect to an abstract representation $\vec{q} \in \kw{val}^*$.
The specification $\Gamma_\kw{rb}$
plays a similar role for the functions provided by $\kw{rb.c}$,
though its representation of the ring buffer is closer
to the actual memory layout used by the code.

In the context of operational semantics and transition systems,
data abstraction is well-understood:
even when they use different representations for their internal states,
a transition system can be shown to refine another
by exhibiting a \emph{simulation relation} between them.
Moreover,
simulation relations compose,
ensuring transitivity of the associated notion of refinement.

To capture data abstraction in the context of compositional semantics,
CompCertO uses \emph{simulation conventions},
which express relationships between
observable source- and target-level interactions
of the component being refined---%
a formal analogue to the notion of calling convention
used in the context of compiler design.
While program components compose \emph{horizontally} through linking,
simulation conventions compose \emph{vertically}
in the manner of simulation relations,
resulting in a two-dimensional notion of refinement.
We will use a similar approach.

% XXX: to related work
%
%Program logics and other verification techniques
%based on operational semantics
%sometimes support data abstraction on a more granular level;
%examples include the refinement calculus \cite{dataref},
%certified abstraction layers \cite{popl15},
%and various kinds of logical relations.
%There,
%stepwise data abstraction
%exists within the context of other compositional structures.
%
%Unfortunately,
%in the context of compositional semantics,
%a systematic understanding of data abstraction
...
Spatial compositionality is an important feature
of some traditional approaches
which has not been investigated in the context of compositional semantics.
By \emph{spatial} compositionality,
we mean the kind afforded by separation logic \cite{seplog}.

we mean the kind which is offered by the separating conjunction
of separation logic.
...
Processes are represented as semantic objects of type:
\[
  P, Q \::\: \mathcal{F} \with \mathcal{F}
...
with typical shell-like operators:
\[
  \begin{prooftree}
...
\]
The component
$\kw{seq} : \mathcal{P} \with \mathcal{P} \rightarrow \mathcal{P}$
invokes two processes one after the other:
\[
  \kw{seq} \vDash
    \kw{run} \cdot (\kw{run}_1 \cdot n) \cdot (\kw{run}_2 \cdot m) \cdot m
\]
It can then be used to define:
\[
  P \mathbin\texttt{;} Q \: := \:
    \kw{seq} \odot (P \with Q)
             \odot (\mathcal{F} \with \gamma \with \mathcal{F})
             \odot (\Delta \with \Delta)
\]
In addition
$\kw{fifo} : \top \rightarrow \mathcal{F}$
models a buffer so that
we can define:
\[
  P \mathbin\texttt{|} Q \: := \:
    \kw{seq} \odot (P \with Q)
         \odot (\mathcal{F} \with (\Delta \odot \kw{fifo}) \with \mathcal{F})
\]
...
to describe the interfaces between components.
...
\begin{definition}
An \emph{effect signature} is a set $E$
together with an assignment $\kw{ar} : E \rightarrow \mathbf{Set}$
of an \emph{arity set} $\kw{ar}(m)$ to each \emph{operation} $m \in E$.
We will present them together as
$\{ m : N \mid m \in E, \, \kw{ar}(m) = N \}$.
\end{definition}
...
based on entries in their file descriptor tables.
To account for this,
we can describe the interface associated with a file
using the signature
\[
  \mathcal{F} := \{
    \kw{read}[n] \mathbin: \Sigma^* , \,
    \kw{write}[s] \mathbin: \mathbb{N} \, \mid \,
    n \in \mathbb{N}, \,
    s \in \Sigma^*
  \}
  \,,
\]
where the set $\Sigma := \{0,1\}^8$
is the alphabet of possible byte values.
In this formalism,
the program \kw{secret} will invoke
the operation
$\kw{write}[\texttt{"uryyb, jbeyq!\textbackslash{}n"}] \mathbin: \mathbb{N}
 \in \mathcal{F}$
in the context of its \emph{standard output} interface,
and should expect the outcome $14 \in \mathbb{N}$
if the operation succeeds.
\end{example}
%}}}

Note that the signature contains a different operation
for each value of the parameters $n$ and $s$,
whereas in this context the \emph{arity} of an operation
corresponds to the possible \emph{outcomes}.
This terminology comes from the algebraic effects literature,
where a computation is understood as a term
in which function symbols correspond to effects,
and their arguments correspond to
the ways in which the computation
which invoked the effect may be resumed.

Effect signatures can be composed in various ways.
For example, the direct sum \citep{sum-of-effects}
plays the role of a cartesian product in our model.
It combines effects from multiple signatures:
\[
  \bigoplus_{i \in I} E_i \, := \,
    \{ \iota_i(m) \mathbin: N \,\mid\, i \in I,\, (m \mathbin: N) \in E_i \}
\]
In the context of Example~\ref{ex:fdsig},
we will use two copies of the signature $\mathcal{F}$
combined as the binary sum $\mathcal{F} \oplus \mathcal{F}$
 

the binary form $\mathcal{F} \oplus \mathcal{F}$
multiple copies of the elementary signature $\mathcal{F}$
could be combined in this way
to account for the various file descriptors
which a process has access to.
Another operation
annotates every operation and outcome in a signature $E$
with a value taken in a set $S$:
\[
  E \mathbin@ S \, := \,
    \{ m @ s \mathbin: N \times S \,\mid\,
      (m \mathbin: N) \in E, \, s \in S \}
\]
...
We use a simple transition system model
to represent the behavior of program components
and other 
