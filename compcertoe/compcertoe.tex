...
\[
  \kw{Asm}(p_1) \oplus \kw{Asm}(p_2) \le \kw{Asm}(p_1 + p_2)
\]
...
\[
  ...
  \le_{\mathbb{C} \rightarrow \mathbb{C}}
  \llbracket \kw{Asm}(\kw{bq.s}) \rrbracket \odot
  \llbracket \kw{Asm}(\kw{rb.s}) \rrbracket
  \le \llbracket \kw{Asm}(\kw{bq.s} + \kw{rb.s}) \rrbracket
\]
...
\[
  \phi_1 \fatsemi (\phi_\kw{bq} \fatsemi \phi^\kw{cc}_\kw{bq}) \odot
  (\phi_\kw{rb} \fatsemi \phi^\kw{cc}_\kw{rb}) \fatsemi \ell
\]
