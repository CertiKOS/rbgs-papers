...
by giving a mathematical account of program execution.
In particular,
...
also suffer some limitations which
...
\begin{itemize}
  ...
  \item programs such as network servers and clients
    ...
    but conduct complex external interactions,
...
%$ ./secret
%uryyb, jbeyq!
\begin{lstlisting}[title={Use case scenario},language=sh]
...
the programs are almost trivial.
To to verify that property (\ref{eqn:hellospec}) holds,
a reader with the right background
can simply follow along the execution step by step
to convince themself that the scenario outlined in Fig.~\ref{fig:readwritehello}
...
the execution of C and x86 assembly,
...
Any mathematical account of property (\ref{eqn:hellospec})
...
involves \emph{two different} programs simultaneously.
Nonetheless,
all of these concerns apply to real-world systems,
yet to our knowledge
there exists no program logic or verification framework
which can deal with problems of this kind.
\end{example}
%}}}
...
provides a separation logic for CompCert's source language Clight, and
...
Finally,
work based on VST has demonstrated the verification of a network server
by incorporating \emph{interaction trees} into the logic
...
and the result would be unlikely to apply directly
to a different ``exotic'' verification task.
Instead,
we would like to create a tool box of sorts
which would provide reusable building blocks
out of which the semantic models and verification techniques
required to tackle this problem
could be constructed with minimal effort.
...
\subsection{Compositional semantics offer a more flexible
  approach to verification}

The paradigm of operational semantics + proof technique
can be made more flexible by
the use of a \emph{compositional} semantics.
This is illustrated in Fig.~\ref{fig:paradigms}
...
[DimSum does this for multi-language
semantics and verification,
however shows only very simple examples.]

[Compositional semantics is not compositional enough!]

\subsection{Reasoning about programs requires
  multiple dimensions of compositionality}

Typically,
compositional semantics
are only compositional in the horizontal sense.
This limits their power compared to
verification frameworks which also offer:
\begin{itemize}
  \item vertical compositionality
    ie.~transitivity of refinement
    and data abstraction relationships
  \item spatial compositionality
    as in separation logic.
\end{itemize}

% - simple example of secret + rot13

% In response to this challenge, researchers come up with
% approaches that are more open, decentralized, etc
% multi-languages

% Things being tied to a particular language can also sometimes
% get in the way of abstraction.
% cf. queue example.
% if we were to describe wouldn't talk about ring buffer or memory states!


% Things becoming more open-ended but most work focuses on
% particular technical obstacles that show up when you try to do that.
% Ie memory states?

% We don't 

%\subsection{Large-scale engineering requires spatial compositionality
...
\begin{itemize}
  \item work on certified compilers,
    which provides only rudimentary specification and verification
    mechanisms
    \cite{compcompcert,compcertm,compcerto};
  \item work on advanced verification techniques,
    which do not connect with real-world
    language semantics and compilers
...
\subsection{State of the Art} \label{sec:soa} %{{{
...
and provides and end-to-end soundness guarantees.
At the same time,
...
as it can only verify systems whose functionality
...
in the style of Fig.~\ref{fig:spec} and low-level assembly code.
As with VST, the result is closely tied to CompCert language semantics.
CAL also requires a specialized version of CompCert
and the existing implementation is again a closed ecosystem.
...
By nature,
much of this work examines certified compilation
in the context of the techniques outlined in \S\ref{sec:req}.
In particular, we will take advantage of
the powerful abstraction mechanisms introduced by
CompCertO \citep{compcerto}.
We will show that,
by generalizing away some of the CompCert-specific aspects,
and by introducing a novel and compositional treatment of state,
...
\begin{itemize}
  ...
  \item We generalize CompCertO's \emph{simulation conventions} and their (vertical) composition
    so that they can express history-sensitive abstraction relationships.
...
\begin{itemize}
  \item We illustrate the use of our framework
    with the verification of our running example.
