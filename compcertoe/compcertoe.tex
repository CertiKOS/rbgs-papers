...
This paper focuses on the techniques enumerated below;
for illustration purposes we use an informal
refinement ordering ($\sqsubseteq$) and composition operation ($\circ$).
...
There are of course many existing verification projects and tools
which leverage one or more of the techniques above.
Several verification frameworks implemented in the Coq proof assistant
feature some of them,
and interface with the certified compiler CompCert \cite{compcert}
to allow correctness properties proved at the C level
to be transferred to the assembly code.
However,
managing the interaction between these different techniques
can often prove quite challenging.
...
with \emph{certified abstraction layers} (CAL) as our main case study.
...
\begin{equation} \label{eqn:vcomp-sim}
  \begin{prooftree}
    \hypo{\pi : L_1 \le L_2}
    \hypo{\rho : L_2 \le L_3}
...
so that a correctness property for the whole program
may be derived from correctness properties of its constituents:
\begin{equation} \label{eqn:hcomp-sim}
  \begin{prooftree}
    \hypo{\pi : L_1 \le L_1'}
    \hypo{\rho : L_2 \le L_2'}
    \infer2{(\pi \oplus \rho) : L_1 \oplus L_2 \le L_1' \oplus L_2'}
  \end{prooftree}
\end{equation}
Unfortunately,
