...
\footnote{%
  We will gloss over important technical details regarding transition systems
  and simulations.
  ...
  However, compositional extensions are
  largely orthogonal to these aspects of the model.}
...
and this is reflected by the vertical composition principle for simulations:
\[
  \begin{prooftree}
...
only works when the transition systems operate
...
expresses the idea that calls performed according to the convention $S$
...
\[
  \begin{prooftree}
    \hypo{x : R \sqsubseteq R' : A \leftrightarrow B}
    \hypo{y : S \sqsubseteq S' : B \leftrightarrow C}
    \infer2{(x \mathbin; y) : (R \mathbin; S) \sqsubseteq (R' \mathbin; S') : A \leftrightarrow C}
  \end{prooftree}
\]
Together
...
\begin{example}[Limitations of $\oplus$]
...
\subsection{Double Category} \label{sec:overview:lcomp} %{{{

\begin{figure} % fig:compcerto {{{
  \text{(a)}
  \quad
  \raisebox{-0.5\height}{\includegraphics[scale=0.06]{double-sd}}
  \qquad
  \text{(b)}
  \quad
...
%  \end{tikzcd}
  \caption{
    Once it is equipped with with layered composition $\odot$,
    the CompCertO model constitutes a double category.
    In particular,
    simulation convention refinements
    become a special case of simulation,
    and formal pasting/string diagrams become possible [explain more]
    (c)
...
We have seen in \S\ref{sec:compcerto-limitations} that
the semantic model of CompCertO presents certain irregularities.
By extending the model with \emph{layered composition},
we equip it with the uniform structure of a double category.
...
we can use a more fundamental operator
\[
  {\odot}_{A,B,C} :
