...
the type
\[
  L_\kw{bq} : \top \twoheadrightarrow \mathcal{C}
  \,,
\]
...
of some context code $C : \mathcal{C} \twoheadrightarrow \mathcal{C}$
...
The previous section has presented existing CompCert semantics
and attempted to capture their varying degrees of compositionality
in a systematic way.
Building on this background,
we now present an overview of our verification framework.

Our starting point is the semantic model of CompCertO.
However,
to address some of its limitations
we introduce \emph{layered composition} (\S\ref{sec:overview:lcomp})
as a more fundamental form of horizontal composition,
and an additional dimension of \emph{spatial compositionality},
...
\newcommand{\pasv}{%
  \begin{tikzpicture}[baseline=0.1ex]
    \draw[fill=white] (0,0) rectangle (1.5ex,1.5ex);
  \end{tikzpicture}}
\newcommand{\actv}{%
  \begin{tikzpicture}[baseline=0.1ex]
    \draw[fill,pattern=north east lines] (0,0) rectangle (1.5ex,1.5ex);
  \end{tikzpicture}}
