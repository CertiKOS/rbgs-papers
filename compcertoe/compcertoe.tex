...
The original CAL is implemented by CompCertX,
and uses
$\kw{ClightX}_L(M)$
to interpret a program $M$ on top of
a layer interface $L$.
However,
in this approach,
...
are all tightly coupled together.
As a consequence,
the original CAL has to modify the compiler
to support such cases.
By contrast, our approach based on the CompCertO
exploits the open semantics,
and used
The layered composition $\odot$ is more flexible
without having to provide a new compiler.
