...
specialized to the setting of effect signatures.
...
$q \underline{m}_1 n_1
 ...
 q' \cdots$,
where the actions of the component have been underlined
and alternate with the actions of the environment.
...
the strategy $\sigma_{\vec{q}} : \top \twoheadrightarrow E_\kw{bq}$
...
is described by $\sigma_\epsilon : \top \twoheadrightarrow E_\kw{bq}$.
\end{example}
%}}}
...
the play $s \rhd t$ first proceeds as $s$
...
\begin{align*}
  ...
  qs^q \odot t &:= \{ qw \mid w \in s^q \odot t \}
  \\
  ...
  \underline{m} s^{qm} \odot^q t &:=
    \{ w \mid \exists t^m \cdot t = mt^m \wedge w \in s \odot^{qm} t^m \}
  \\
...
%    \{ q w \mid w \in \langle s_1^q, s_2 \rangle^q_1 \} &
%  \langle s_1 , qs_2^q \rangle &:=
%    \{ q w \mid w \in \langle s_1, s_2^q \rangle^q_2 \} \\
%  \langle s_1^q, s_2 \rangle_1^q &:=
%    \{ \}
%\end{align*}
Then
