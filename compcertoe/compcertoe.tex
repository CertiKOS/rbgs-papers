...
which itself exhibits a behavior described by $T : \mathbf{Sup}\rightarrow\mathbf{Sup}$.
\[
  \hat{E} X \: := \:
...
\]
Computations in the lattice $\check{E}_T X$ defined above
first await a query $q \in E$,
execute a strategy over $T$,
then produce answer $r \in \kw{ar}(q)$
before terminating.

\paragraph{Interaction}

Assume $\langle T, \eta_T, \mu_T \rangle$ is a monad equipped with a strength
$s_T^{X,Y} : X \otimes T Y \rightarrow T (X \otimes \check{E}_T Y)$.
We can model interaction using a primitive
$
  \kappa : \hat{E} X \otimes \check{E}_T Y \rightarrow
    T (X \otimes Y)
$.
To this end,
we focus first on the simpler task of
propagating the choice of answer $r \in \kw{ar}(q)$
from the object to the client in the residual computation
