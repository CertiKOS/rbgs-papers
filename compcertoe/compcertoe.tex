...
\begin{itemize}
  \item a \emph{companion} $L_* : A \leftrightarrow B$ when
    $\kw{id}_A \le_{\kw{id}_A \twoheadrightarrow L_*} L$ and
    $L \le_{L_* \twoheadrightarrow \kw{id}_B} \kw{id}_B$;
  \item a \emph{conjoint} $L^* : B \leftrightarrow A$ when
    $\kw{id}_B \le_{L^* \twoheadrightarrow \kw{id}_B} L$ and
    $L \le_{\kw{id}_A \twoheadrightarrow L^*} \kw{id}_A$.
\end{itemize}
\end{definition}
...
we would hope to decompose the correctness proof $\phi$ into
\[
  \begin{array}{c}
    \phi_1 : \Gamma_\kw{bq}
  ...
  \end{array}
  \qquad \qquad
  \begin{tikzpicture}[yscale=0.6,xscale=1.1,baseline=(b)]
    \newcommand{\filltint}{30}
    ...
    \fill[ACMBlue!\filltint] (-2,2) |- (2,1) |- cycle;
    ...
    \begin{scope}[opacity=0.5,outer sep=2pt]
      ...
      \node[below left] at (2,5) {$\top$};
      \node[above] at (0,1) {$\mathcal{C} \mathbin@ \kw{mem}$};
    ...
    \begin{scope}
      \small
      \draw (0,5) node[above] {$\Gamma_\kw{bq}$}
         -- (0,4) [rounded corners] -- (-1,3) [sharp corners] node[left] {$\Sigma_\kw{bq}$}
         -- (-1,1) node[below] {$\kw{Clight}(\kw{bq.c})$};
      \draw (0,4) [rounded corners] -- (1,3) [sharp corners] node[right] {$\Gamma_\kw{rb}$}
         -- (1,1) node[below] {$\kw{Clight}(\kw{rb.c})$};
      \draw (1,2) -- (2,2) node[right] {$\varnothing$};
    \end{scope}
...
Applying the construction ${-} \mathbin@ U$
to the component $\Sigma_\kw{bq}$ of Example~\ref{ex:abspec}
allows us to state
$
...
can be combined.
First,
to interface client code with an abstract specification such as $\Gamma_\kw{rb}$,
we can use $\langle \kw{mem} ] : \mathbbm{1} \leftrightarrows \kw{mem}$
to express the fact that $\Gamma_\kw{rb}$
does not affect the global memory.
For $\kw{bq.c}$ we get:
\[
  \begin{array}{r@{}l}
    L_\kw{bq} :=
...
As a result we can state the overall correctness property as:
\[
  \phi :
  \Gamma_\kw{bq} \le_{\top \twoheadrightarrow \mathbf{R}_\kw{bq}} L_\kw{bq}
  \quad \text{where} \quad
  \mathbf{R}_\kw{bq} :=
  \mathcal{C} \mathbin@ \langle \kw{mem} ]_* \mathbin@ R_\kw{bq} = \:
  \begin{tikzpicture}[xscale=0.5,yscale=0.6,baseline=(R.base)]
    \fill[scsdbg] (0,0) rectangle (6.4,2);
    \fill[pattern=north east lines] (0,0) rectangle (1,2);
...
\[
  \phi_1 \::\:
    \begin{tikzpicture}[
      ...
      % Bottom face
      \fill[tssdbg]
        (-2,-1,-2) -- (-2,-1,+2.5) -- (+2,-1,+2.5) -- (+2,-1,-2) -- cycle;
      \draw[tssdbg] (-2,-1,+2.5) -- (-2,-1,-2);
      \draw (-2,-1,-1)
      ...
      \draw (-2,-1,+1)
        ...
        [rounded corners] -- (+1,-1,+1)
        [sharp corners] -- (+2,-1,+1)
        node[right] {$\mathcal{C}$};
      \fill[pattern=north east lines] (-2,-1,+1)
        [rounded corners] -- (-1,-1,+1)
        [sharp corners] -- (0,-1,0)
        [rounded corners] -- (+1,-1,+1)
        [sharp corners] -- (+2,-1,+1)
        -- (+2,-1,+2.5) -- (-2,-1,+2.5) -- cycle;
      \node[ts] at (0,-1,0) {$\kw{bq.c}$};

      % Left face
      \fill[tssdbg] (-2,-1,+2.5) -- (-2,-1,-2)
        -- (-2,+1,-2) -- (-2,+1,+2.5) -- cycle;
      \draw[thin,dotted] (-2,-1,+2.5) -- (-2,-1,-2);
      \fill[pattern=north east lines] (-2,-1,+1) -- (-2,-1,+2.5)
        -- (-2,+1,+2.5) -- (-2,+1,+1) -- cycle;
      \draw (-2,-1,+1) -- (-2,+1,+1);
      \draw (-2,-1,-1) -- (-2,+1,-1);

      % Top face
      \fill[tssdbg,opacity=0.66]
        (-2,+1,-2) -- (-2,+1,+2.5) -- (+2,+1,+2.5) -- (+2,+1,-2) -- cycle;
      %\draw[thin,dotted] (-2,+1,+2) -- (-2,+1,-2);
      \fill[pattern=north east lines] (-2,+1,+1) -- (+2,+1,+1)
        -- (+2,+1,+2.5) -- (-2,+1,+2.5) -- cycle;
      \draw (-2,+1,-1) node[left] {$\kw{mem}$} -- (+2,+1,-1);
      \draw (-2,+1,+1) node[left] {$\mathcal{C}$} -- (+2,+1,+1);
      \node[ts] at (0,+1,+1) {$\Sigma_\kw{bq}$};

      % Right face
      \fill[tssdbg,opacity=0.66] (+2,-1,+2.5) -- (+2,-1,-2)
        -- (+2,+1,-2) -- (+2,+1,+2.5) -- cycle;
      \draw[thin,dotted] (+2,+1,+2.5) -- (+2,+1,-2);
      %\draw[thin,dotted] (+2,-1,+2.5) -- (+2,-1,-2);
      \fill[pattern=north east lines] (+2,-1,+1) -- (+2,-1,+2.5)
