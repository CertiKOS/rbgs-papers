...
\begin{itemize}
  \item Transition systems are 0-dimensional objects,
    ...
    They do not compose
    but provide ``endpoints'' for simulations types.
...
\begin{figure} % fig:pasting {{{
  ...
  \text{(b)}
  \quad
  \begin{tikzcd}[sep=1ex, row sep=tiny]
    \bullet \ar[dddd, equal] \ar[rr, dash, "C"] &&
    ...
    \\
    \bullet \ar[rr, "C"', dash] &&
    \bullet \ar[rr, "\kw{Asm}(\kw{bq.s})"', dash] && %\ar[dd, equal] &&
    \bullet \ar[rr, "\kw{Asm}(\kw{rb.s})"', dash] &&
    \bullet
    %\\
    %&& \pi
    %\\
...
we used in Fig.~\ref{fig:pasting}.
...
as we examine how the follow-up work on CompCertO
...
owing to
the restrictions of horizontal composition (\ref{eqn:hcomp})
and their workaround
in the form of simulation convention refinements.
As a result,
...
CompCertO semantics and proofs remain available.
In particular,
the compiler correctness proof provides the simulations:
\[
  \pi_\kw{bq} :
    \kw{Clight}(\kw{bq.c})
...
\[
  \begin{prooftree}
    \hypo{\Gamma_\kw{bq} \sqsubseteq \Sigma_\kw{bq} \odot \Gamma_\kw{rb}}
    \hypo{\Sigma_\kw{bq} \sqsubseteq \kw{Clight}(\kw{bq.c})}
    \infer2{\Gamma_\kw{bq} \sqsubseteq \kw{Clight}(\kw{bq.c}) \odot \Gamma_\kw{rb}}
    \hypo{\Gamma_\kw{rb} \sqsubseteq \kw{Clight}(\kw{rb.c})}
    \infer2{\Gamma_\kw{bq} \sqsubseteq \kw{Clight}(\kw{bq.c}) \odot \kw{Clight}(\kw{rb.c})}
  \end{prooftree}
  \qquad \qquad
  \text{(wrong!)}
  \hspace{-4em}
...
used for example
by $(\Sigma_\kw{bq} \mathbin@ D_\kw{rb}) \odot \Gamma_\kw{rb} :
  \top \twoheadrightarrow \mathcal{C} \mathbin@ D_\kw{rb}$
and $\Gamma_\kw{bq} : \top \twoheadrightarrow \mathcal{C} \mathbin@ D_\kw{bq}$
remain an obstacle.
\end{example}
%}}}
...
$
  \pi_\kw{bq} :
  L_\kw{bq}
  \:\le_{\top \twoheadrightarrow \mathcal{C} \otimes [R_\kw{bq}]}\:
...
\paragraph{Composing with Lenses} %{{{
...
\]
For example,
we will sometimes use the trivial lens
$\langle U ] : \mathbbm{1} \leftrightarrows U$
where
$\kw{get}_{\langle U ]}(u) = *$ and
$\kw{set}_{\langle U ]}(u, *) = u$.
Structural isomorphisms and other bijections
define lenses as well.
...
\paragraph{Active and Passive Components} %{{{
...
of sets and lenses.
Lenses and simulation conventions
can simply act in parallel on their own portion of the state.
By contrast,
active components impose their schedule
and conflict with one another.
They can only be spatially composed with passive components.
...
by describing:
\begin{itemize}
  \item language (active) interfaces as objects of type $A : \actv \curvearrowright \pasv$;
...
\begin{itemize}
\item
the trivial ``active/passive tag'' objects
...
%This is illustrated in Fig.~\ref{fig:3dsd},
...
\begin{example}[Spatial composition in string diagrams] %{{{
[XXX need to update and finalize the figure]
Per Example~\ref{ex:abspeclift},
the code specification $\Sigma_\kw{bq}$
can be interfaced with $\Gamma_\kw{rb}$
by extending it into $\Sigma_\kw{bq} \mathbin@ D_\kw{rb}$,
but the outgoing call of its implementation $\kw{Clight}(\kw{bq.c})$
also carry a memory state component.
To express the fact that,
at this level of abstraction,
$\Gamma_\kw{bq}$ leaves that memory state unchanged,
we can compose them in the way described by
Fig.~\ref{fig:3dsd}a:
\[
...
\]
Moreover,
since the overall specification $\Gamma_\kw{bq}$
likewise does not mention the incoming memory state component,
we can use the simulation convention described by Fig.~\ref{fig:3dsd}b
to state correctness as:
\[
  L_\kw{bq}
  \:
  ...
  L_\kw{rb}
\]
The overall refinement property describe above
can be depicted by the diagram in Fig.~\ref{fig:3dsd}c.
\end{example}
%}}}

\hrule
\begin{color}{gray}
Old diagrams to update and use:
\[
  ...
  \quad
    \begin{tikzpicture}[
      ...
      % Bottom face
      \fill[tssdbg,opacity=0.5]
        (-2,-1,-2) -- (-2,-1,+2) -- (+2,-1,+2) -- (+2,-1,-2) -- cycle;
      \draw[tssdbg] (-2,-1,+2) -- (-2,-1,-2);
      \draw (-2,-1,-1)
        [rounded corners] -- (-1,-1,-1)
        [sharp corners] -- (0,-1,0)
        [rounded corners] -- (+1,-1,-1)
        [sharp corners] -- (+2,-1,-1);
      \draw (-2,-1,+1)
        ...
        [rounded corners] -- (+1,-1,+1)
        [sharp corners] -- (+2,-1,+1);
      \fill[pattern=north east lines] (-2,-1,+1)
        [rounded corners] -- (-1,-1,+1)
        [sharp corners] -- (0,-1,0)
        [rounded corners] -- (+1,-1,+1)
        [sharp corners] -- (+2,-1,+1)
        -- (+2,-1,+2) -- (-2,-1,+2) -- cycle;
      \node at (0,-1,0) {$\kw{bq.c}$};

      % Left face
      \fill[tssdbg,opacity=0.5] (-2,-1,+2) -- (-2,-1,-2)
        -- (-2,+1,-2) -- (-2,+1,+2) -- cycle;
      \fill[pattern=north east lines] (-2,-1,+1) -- (-2,-1,+2)
        -- (-2,+1,+2) -- (-2,+1,+1) -- cycle;
      \draw (-2,-1,+1) -- (-2,+1,+1);
      \draw (-2,-1,-1) -- (-2,+1,-1);
      ...
      \fill[tssdbg,opacity=0.5]
        (-2,+1,-2) -- (-2,+1,+2) -- (+2,+1,+2) -- (+2,+1,-2) -- cycle;
      \fill[pattern=north east lines] (-2,+1,+1) -- (+2,+1,+1)
        -- (+2,+1,+2) -- (-2,+1,+2) -- cycle;
      \draw (-2,+1,-1) -- (+2,+1,-1);
      \draw (-2,+1,+1) -- (+2,+1,+1);
      \node at (0,+1,+1) {$\Sigma_\kw{bq}$};
...
$
  L \mathbin@ \kw{mem}
  \le_{A \mathbin@ [\bullet] \twoheadrightarrow B \mathbin@ [\bullet]}
  L
$.
