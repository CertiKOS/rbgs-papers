...
\begin{figure} % fig:pasting {{{
  ...
  \raisebox{-0.5\height}{
    \begin{tikzpicture}[xscale=0.7,yscale=0.35]
      \small
      \fill[color=ACMBlue!20] (-3,+2) rectangle (+2.5,-4);
      \begin{scope}[rounded corners]
...
Then $\phi$ can be constructed from
...
\emph{ie.}~$\multimap$ can slide past $\Sigma_\kw{bq}$.
We~can then in turn vertically compose $\phi_2$
to obtain:
\[
  \phi \: := \:
  \begin{array}{c}
...
\[
  ...
  \,.
\]
However note that at the implementation level,
the memory state passed to $\kw{rb.c}$
will not just contain $\kw{buf}$, $\kw{c1}$ and $\kw{c2}$.
Instead,
these variables will be added into
the memory state use by the client of $\Gamma_\kw{rb}$.
...
\begin{equation} \label{eqn:rbcorrect}
  ...
  \le_{\varnothing \twoheadrightarrow
       \mathcal{C} \mathbin@
       ((R_\kw{rb} \mathbin@ \langle \kw{mem} ]_*) \mathbin; {\bullet})}
  \kw{Clight}(\kw{rb.c})
\end{equation}
Moreover,
...
In \S\ref{sec:encap},
we explain how our model can be extended
...
$d_0 := (\{\kw{undef},\kw{undef},\ldots\}, 0, 0) \in D_\kw{rb}$
we can define
$\Gamma'_\kw{rb} :=
...
Moreover,
since the property $\zeta : \epsilon \mathbin{R_\kw{bq}} d_0$ holds,
...
and used to construct complex proofs in a modular way.
...
which take advantage of their specificities.
Nevertheless,
...
where forcing all components to fit one methodology may be difficult.
...
opens the door to language features
making use of that capability.
As a first step,
...
\section*{Material cut from \S3 which could be useful below} %{{{
