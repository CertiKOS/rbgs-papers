...
In this section, we describe the overall structure of our framework
and provide a variety of example illustrating its use.

\subsection{Overview} %{{{

Our framework consists of four kinds of objects,
each of which may be subject to some of all of four
...
We introduce in \S\ref{sec:esig} the notion of \emph{effect signatures} ($E, F\ldots$),
which model interfaces between the components of a software system.
Signatures serve as
horizontal endpoints for the game-semantic \emph{strategies}
($L : E \rightarrow F$, see \S\ref{sec:strat})
which model the behavior of components, and
vertical endpoints for the \emph{simulation conventions}
($\mathbf{R} : E_1 \leftrightarrow E_2$, see \S\ref{sec:sconv})
...
involve all three kinds of objects,
connecting them in the shape of a square.
...
\item Layered composition ($\odot$) acts horizontally.
  It connects strategies alongside a common endpoint (effect signature)
  over which they are made to interact,
  and connects refinement squares alongside a common
  vertical edge (simulation convention),
  which ensures that the assumptions of one refinement proof
...
The definition of our strategy model
follows the approach used in \citet{rbgs-cal}.
However,
our strategies are much simpler as
we do not attempt to model demonic nondeterminism.

\begin{definition}
The \emph{plays} over an effect signature $E$ with outcomes in $X$
are given by the grammar:
\[
  s \in P_E(X) ::= \underline{x} \mid \underline{m} \mid \underline{m} n s
  \qquad
  (x \in X, m\mathbin:N \in E, n \in N)
\]
Plays are ordered under a prefix relation $\sqsubseteq$.
If $X$ is equipped with an order $\le$, it is taken into account:
\[
  \begin{prooftree}
    \hypo{x \le y}
    \infer1{\underline{x} \sqsubseteq \underline{y}}
  \end{prooftree}
  \qquad
  \begin{prooftree}
    \hypo{s \sqsubseteq t}
...
\paragraph{Coalgebraic view}

The definition given above sits squarely within the game semantics tradition.
However,
following \citet{ags-act}
we can give the model a coalgebraic characterization as well,
which will come handy as a more operational view of the model.
