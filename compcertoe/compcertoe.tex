...
CompCert can compile
the files shown in Fig.~\ref{fig:code},
but since they do not constitute whole programs,
...
In practice,
we will especially use two kinds of lens.
First,
every bijection is a lens,
and this can be used to define structural isomorphisms
such as $\gamma_{A,B} : A \times B \cong B \times A$.
...
$[f] := \kw{id}_{[\mathbbm{1}]} \mathbin@ f : [U] \twoheadrightarrow [V]$.
...
Note that a relation $R \subseteq U \times V$
can be promoted to a simulation convention
$[R] : [U] \leftrightarrow [V]$,
and we can introduce a simpler relationship
$\phi : f \equiv_{R \lensarrow S} g$
which then entails
$[\phi] : [f] \le_{[R] \twoheadrightarrow [S]} [g]$.
However,
in some cases the more powerful machinery of simulation conventions
will be useful in the context of lenses as well as transition systems.
...
Above we have left $[-]$ implicit,
as we often will in the remainder of the paper.
...
The different kinds of objects we have described
are summarized in Table~\ref{tbl:roadmap}.
We call \emph{active} components which are more complex,
and which in the case of transition systems
impose their schedule onto the \emph{passive} components
such as lenses which are adjoined to them.
...
By contrast,
transition systems can only be composed with lenses,
in a way which is more complex to describe.
...
\paragraph{Higher-dimensional Structure} %{{{
...
As shown in \autoref{tbl:roadmap},
by introducing spatial composition,
we have once again increased the dimensionality
of all of our objects by one:
\begin{itemize}
\item
...
it remains to describe the 0-dimensional objects which
serve as endpoints for sets and language interfaces.

Structurally speaking,
$\mathbin@$ is closest to Compositional CompCert's
semantic linking operator (\S\ref{sec:survey:compcomp}).
Recall that in that context,
we considered $\oplus$ as operating on objects of trivial type
${\bullet} \rightarrow {\bullet}$.
The situation with $\mathbin@$ is fairly similar,
but to encode its asymmetric shape
we instead introduce \emph{two} 0-dimensional
``active/passive tag'' objects called
$\actv$ and $\pasv$.
Then we can describe
\begin{itemize}
  \item language (active) interfaces as objects of type $A : \actv \curvearrowright \pasv$, and
...
\[
  ...
  \quad :\Leftrightarrow \quad
    \kw{id}_A \le_{\mathbf{R} \rightarrow \mathbf{S}} \kw{id}_B
\]
