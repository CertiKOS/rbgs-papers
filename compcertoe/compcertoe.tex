...
has a unique fixed point $\mu X \cdot FX \in \mathbf{Sup}$,
with the defining property that
$\mu X \cdot FX \cong F (\mu X \cdot FX)$.
In other words,
a computation in $\mu X \cdot FX$
proceeds as in the expression $FX$,
where any occurrence of $X$
triggers a new iteration of the game.
We discuss this construction extensively
in \S\ref{sec:interactions} below.
...
can be interpreted as a game
which is iterated at will ($\mu S \cdot {-}$).
...
this choices immediately observable.
