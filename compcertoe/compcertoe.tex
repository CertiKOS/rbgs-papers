...
[Add to address process encapsulation per Zhong:]
Mechanized semantics, program logics and certified compilers exist
which can deal with the C and assembly code use in Example~\ref{ex:readwritehello}.
However, the programs \emph{as such} are not the end of the story.
Rather than constituting standalone artifacts,
the programs are part of a larger context where
they are first interpreted as \emph{processes},
then used as building blocks in a larger system.
[communication through function calls, memory states in a shared address space at some level;
vs IPC over file descriptors at another level;
things are encapsulated, etc.]
...
can be constructed as a lattice $\mu \hat{E} \cong \mathcal{D}(\mu \ddot{E})$
...
The plays for $E_\kw{nat}$ take the form
$
  \mu \ddot{E}_\kw{nat} \cong
    \{
...
$\mu_{\hat{E}_\kw{nat}} \cong \mathcal{D}(\mu_{\ddot{E}_\kw{nat}})$
and defined by
\[
...
\[
  ...
  \langle \delta_2 \rangle(*) =
    \{ (\underline{\kw{succ}} \, {*})^n \,\underline{\kw{succ}} \mid n \in \mathbb{N} \}
  \,.
\]
...
to incorporate a generating space.
Given an endofunctor $F : \mathbf{Sup} \rightarrow \mathbf{Sup}$,
...
the $F$-algebra $c_F^X : F (F^* X) \rightarrow F^* X$
defined by:
\[
  F^*S := \mu Y \cdot F Y \oplus X \,,
  \qquad \text{and} \quad
  c_F^X := c_{F(-) \oplus X} \circ \iota_1 \,.
\]
In addition, the adjunction's unit
$\eta_F^X := c_{F(-)\oplus X} \circ \iota_2 : X \rightarrow F^* X$
embeds the generators into $F^*X$.
...
and we will sometimes use the notations
\[
  \underline{x} := \eta_F^X(x)
  \qquad \text{and} \qquad
  \underline{m}(\sigma_n)_n :=
    c_F^X \bigl( \underline{m}\langle \sigma \rangle_n \bigr)
  \,.
\]
...
\subsection{Interactive computations} \label{sec:interactions} %{{{

Consider an interactive computation
which repeatedly invokes operations in an effect signature $E$,
and eventually yields a result in the set $X$.
The corresponding sup-lattice can be written
\begin{equation} \label{eqn:intlat}
  \mathcal{I}_E(X) \: := \:\:
    \mu S \cdot
      \bigoplus_{m \in E} \biggl( \bigwith_{n \in \kw{ar}(m)} S \biggr)_\bot
      \oplus\:
      \bigoplus_{x \in X} \mathbf{1}
  \:\: \in \: \mathbf{Sup}
  \,.
\end{equation}
Computations in $\mathcal{I}_E(X)$
are similar in shape to interaction trees \citep{itrees},
however sup-lattices can model silent divergence as $\bot$
so we do not need to concern ourselves with $\tau$ moves.

Concretely,
the expression (\ref{eqn:intlat}) above
can be interpreted as an iterated game ($\mu S \cdot {-}$).
For each iteration, two choices are possible ($\oplus$):
\begin{itemize}
\item
Per the left-hand side $\bigoplus_m (\cdots)_\bot$
the computation may choose a question $m \in E$.
The lifting operator $(-)_\bot$ makes
this choice immediately observable.
After the environment supplies an answer $n \in \kw{ar}(m)$,
as indicated by the product $\bigwith_n$,
the game will start over ($S$).
\item
Alternatively,
the computation may choose $x \in X$
and terminate ($\mathbf{1}$).
%Note that the elements of $X$ can be understood
%as additional, nullary operations $x \mathbin: \varnothing$,
%and indeed $\mathbf{1} \cong \mathbf{0}_\bot$.
\end{itemize}
Computations in $\mathcal{I}_E(X)$ are terms
generated by the signature $E$
with variables taken in the set $X$.
For an operation $m \mathbin: N \in E$,
arguments $(k_n)_{n \in N} \in \mathcal{I}_E(X)^N$,
and a variable $x \in X$, we would write
\[
  \underline{m}(k_n)_{n \in N} := 
    c \circ \iota_1 \circ \iota_m ( \langle k_n \rangle_{n \in N} )_\bot
  \qquad
  \text{and}
  \qquad
  \underline{x} := \iota_2 \circ \iota_x(*)
  \,.
\]
In addition to these two constructors,
we can use arbitrary joins.
The use of lifting $(-)_\bot$ also allows us
to construct partial terms since $\underline{m}(\bot) \neq \bot$.
Together these constructions can be used to represent infinite behaviors.

\begin{example}
Suppose $m \mathbin: \mathbbm{1}$ is an operation in a signature $E$.
Then the computation which forever invokes $m$ can be described as:
\[
  \bigvee_i
    \underbrace{\underline{m}(\underline{m}(\cdots \underline{m}}_{i \text{ times}}(\bot) \cdots ))
\]
More usefully,
a low-level specification for the function \texttt{deq}
can be given as:
\[
  \Sigma_\kw{deq} \: := \:
    \underline{\kw{inc}}_2 \left(
      \underline{\kw{get}}[n] \bigl( \underline{v} \bigr)_{v \in V}
    \right)_{n \in \mathbb{N}}
    \: \in \: \mathcal{I}_{E_\kw{rb}}(V)
  \,.
\]
\end{example}
