...
Our semantics framework builds on an existing line of work
which has developed
compositional semantics for the certified compiler CompCert.
Therefore,
before we introduce our contributions in the following section,
we begin by a mini-survey of this line of research.
...
\begin{itemize}
  ...
  \item for related states $s_1 \mathrel\rho s_2$,
    if $s_1$ is final in $L_1$ with outcome $x$,
...
As illustrated in Fig.~\ref{fig:pasting},
as we incorporate more composition principles into the framework,
the dimensionality of systems and simulation proofs will increase
(our work will turn simulations into 3-dimensional objects).
...
\begin{figure} % fig:pasting {{{
  \text{(a)}
  \begin{tikzcd}[row sep=3ex]
    \kw{Clight}[p_0] \ar[d, "\pi_1", Rightarrow] \\
    \kw{C\#minor}[p_1] \ar[d, "\pi_2", Rightarrow] \\
...
A limitation of the framework described above
is that semantics only characterize the behavior of whole programs.
...
$L_2$ executes until it reaches a final state,
at which point the outcome of its execution is used to resume
...
We can in fact consider them ``loops'' of type ${\bullet} \rightarrow {\bullet}$
over a trivial 0-dimensional endpoint.
%as in Fig.~\ref{fig:pasting}b.

Because simulations now connect 1-dimensional transition systems,
they become 2-dimensional objects.
As before,
...
is depicted as:
\[
  \includegraphics[scale=0.1]{pr/phi}
\]
The diagram above uses the single node $\phi$
as the entire simulation proof,
but diagrams with more complex internal structures
can be used to denote composite simulation relations:
\begin{itemize}
\item
...
can be used to denote the horizontal composition
of the corresponding simulations.
\end{itemize}
Using these conventions,
the simulation relation (\ref{eqn:ccex}) above can be depicted as:
\[
  \includegraphics[scale=0.07]{ccex}
\]
