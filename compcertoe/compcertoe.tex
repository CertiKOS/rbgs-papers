...
We have demonstrated how a compositional model
based on typical game semantics constructions
can be extended with a vertical dimension,
incorporating notions of data abstraction and strategy refinement
which are useful from a verification perspective.

In this section,
we discuss how the model can be extended further
by introducing a \emph{spatial} dimension to our compositional algebra.
The formal definitions are omitted
...
Our model relies on a slightly richer notion of \emph{stateful} lens,
which carry their own hidden state component $P$
as part of a lens over $V \times P$.
...
These boundaries are specified as
language interfaces $A := \langle A^\que, A^\ans \rangle$,
as illustrated in Example~\ref{ex:compcertosig},
...
This translation allows us
to define
the refinement conventions
$\mathbbm{E}_\kw{bq} : E_\kw{bq} \leftrightarrow
\llbracket \mathcal{C} \rrbracket\:
\text{and}\:
\mathbbm{E}_\kw{rb} : E_\kw{rb} \leftrightarrow
\llbracket \mathcal{C} \rrbracket$,
associating each signature
in $E_\kw{bq}$ and $E_\kw{rb}$
...
from $A$ to $B$ is specified by
a set $W$ of worlds,
a Kripke relation $R^\que \in \mathcal{R}_W\big(A^\que,\, B^\que\big)$
between the language interfaces' questions,
and
a Kripke relation $R^\ans \in \mathcal{R}_W\big(A^\ans,\, B^\ans\big)$
between their answers.
\end{definition}
%}}}
