...
\]
and interpret the abstraction layers:
\[
  \begin{array}{c}
  ...
  \end{tikzpicture} }}
\]
where the initial states satisfies the relation
$d_2 \mathbin R (m_0, d_1)$.

\paragraph{Composotion}
Given another abstraction layer
$L_2 \vdash_S N : L_3$,
abstraction layers can compose
via several auxiliary properties.
\[
  ...
  [\text{diagram}]
\]
The above property can be composed
with the layer correctness of $M$:
\[
...
\]
Then we can put the correctness of $N$ on top of it:
\[
  \Gamma_3 \le_{\varnothing \twoheadrightarrow \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{n_0} \rangle^*
    ; \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{m_0} \rangle^* }
  \kw{Clight}(N) \odot \kw{Clight}(M) \odot (\mathcal{C} \mathbin@ \langle \kw{mem} ]) \odot \Gamma_1
\]
Finally, we apply the refinement between simulation conventions
\[
  ...
  \mathcal{C} \mathbin@ \kw{mem} \bullet [ \kw{m_0} \rangle^*
\]
to obtain the composite layer correctness:
\[
...
the examples of ring buffer and bounded queue
...
The implementation of certified abstraction layers
using CompCertO with state encapsulation
has two outstanding advantages
over the original implementation based on CompCertX.
On one hand,
unlike the CompCertO's open semantics,
the ClightX semantics,
a variant of Clight in CompCertX,
interprets external calls according to underlay primitives.
As a consequence,
overlayers can only be verified
on top of a well-formed underlay.
By contrast, our framework achieves underlay independence.
On the other,
