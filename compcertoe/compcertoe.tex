...
which implements a bounded queue
...
Moreover,
although data abstraction and encapsulation
are not supported by C language features,
...
even though this is not enforced by
the monolithic memory model of C.
In fact it is sufficient for
a user of the code in Fig.~\ref{fig:code}
to picture the queue
as a sequence of the values which have been enqueued
but are still waiting to be dequeued.
...
The specifications in Fig.~\ref{fig:spec},
adapted from \citet{rbgs-cal},
demonstrate one approach.
A top-level specification $\Gamma_\kw{bq}$
...
the intermediate specifications $\Sigma_\kw{bq}$ and $\Gamma_\kw{rb}$
which characterize $\kw{bq.c}$ and $\kw{rb.c}$ in isolation.
$\Gamma_\kw{rb}$ is similar in form to $\Gamma_\kw{bq}$
but describes the ring buffer data structure
rather than the overall queue.
$\Sigma_\kw{bq}$ does not describe a data structure,
but rather the sequence of calls into $\kw{rb.c}$
which are necessary to realize the queue operations
in terms of the ring buffer primitives.
$\Sigma_\kw{bq}$ itself does not carry any state at all,
but the relation $R$ describes how queue state
are realized in terms of the underlying ring buffer states.
\end{example}
...
However, ensuring \emph{end-to-end} correctness
...
mechanized certified systems engineering frameworks
tends to fall into one of two separate categories:
\begin{itemize}
  \item on one hand,
...
\end{itemize}
What makes a synthesis difficult is that
...
illustrates some of the challenges which we have outlined above.
...
which CompCert can correctly compiled to assembly.
It represents an early and valuable achievement which demonstrates
the viability and usefulness of end-to-end correctness techniques.
At the same time,
in essence VST is implemented as an extension to CompCert,
and is tied very closely to CompCert semantics.
As such it cannot be used as the starting point for a larger ecosystem.
It can only be used to verify systems
which are implemented entirely in C
and whose functionality
can be specified completely within the VST logic.

The same kind of restrictions apply to CompCertX,
which was developed to verify the operating system kernel CertiKOS \citep{popl15}
using the methodology of \emph{certified abstraction layers} (CAL).
It provides support for data abstraction;
compared with VST it allows flexible specifications
similar to those shown in Fig.~\ref{fig:spec}
and can support low-level assembly code.
But implementing CAL required invasive changes to CompCert,
and conversely its high-level reasoning techniques
must take into account idiosyncrasies of the CompCert memory model.
...
and by introducing a compositional treatment of state,
...
we begin our exposition with a summary of the main challenges and techniques
used in this line of work (\S\ref{sec:compcert});
we then provide an overview of the structure of our framework (\S\ref{sec:overview}).
The core of our framework's formal definition is given in \S\ref{sec:base}
and our approach to compositional state is detailed in \ref{sec:scomp}.
Finally,
we evaluate the framework by
outline several applications in \S\ref{sec:app},
...
We start by summarizing existing work on CompCert semantics
and outlining some of the difficulties in
interfacing this work with high-level verification techniques.

\subsection{Whole-Program Semantics in CompCert} %{{{
...
and introduce an operator $\oplus$ to model the \emph{linking} process
which must happen before $\kw{rb.c}$
is run as part of a larger program.
This operator should be compatible with simulations,
so that for example we may derived the overall correctness property
\[
...
Unfortunately,
achieving this in the context of CompCert
...
\subsection{Compositional Semantics in CompCertO} %{{{

The problem of achieving horizontal compositionality in CompCert
...
\cite{compcerto},
which is the starting point for our extended model.
...
\begin{figure} % fig:compcerto -- Composition rules in CompCertO {{{
  ...
  \begin{gather*}
    ...
    \begin{prooftree}
      ...
      \infer2[\kw{sc}-$\mathbin;$]{(\mathbf{R} \mathbin; \mathbf{R}') : A \leftrightarrow C}
    \end{prooftree}
    \qquad
    \begin{prooftree}
      \hypo{\phi : L_1 \le_{\mathbf{R} \twoheadrightarrow \mathbf{S}} L_2}
      \hypo{\psi : L_2 \le_{\mathbf{R'} \twoheadrightarrow \mathbf{S'}} L_3}
      \infer2[\kw{sim}-$\mathbin;$]{(\phi \mathbin; \psi) : L_1 \le_{\mathbf{R} \mathbin; \mathbf{R'} \twoheadrightarrow
	\mathbf{S} \mathbin; \mathbf{S'}} L_3}
    \end{prooftree}
  \end{gather*}
  \caption{Composition principles $\oplus$ and $\mathbin;$ in the CompCertO semantic model}
  \label{fig:compcerto}
\end{figure}
%}}}
...
This is used by the vertical composition principle \kw{sim}-$\mathbin;$ for simulations,
...
\subsection{CompCertO Semantics as a Refinement-based Verification Framework} %{{{

In the framework outlined above,
the semantics of the source program serve as a specification
and those of the target program serve as the implementation,
while simulations establish a form of refinement
of the former by the latter.
As such,
this framework can already be used as a rudimentary
refinement-based verification framework,
as illustrated by the example below.

\begin{example} \label{ex:compcomp} %{{{
We can provide a specification for the code in Fig.~\ref{fig:code}
as a simple transition system
$L_\kw{bq} : \mathcal{C}_\kw{m} \twoheadrightarrow \mathcal{C}_\kw{m}$
which spells out the expected behaviors
...
$\pi_\kw{bq} : \kw{Clight}(\kw{bq.c})
  \le_{\mathbb{C} \twoheadrightarrow \mathbb{C}} \kw{Asm}(\kw{bq.s})$ and
$\pi_\kw{rb} : \kw{Clight}(\kw{rb.c})
  \le_{\mathbb{C} \twoheadrightarrow \mathbb{C}} \kw{Asm}(\kw{rb.s})$.
By composing the simulations together,
we can derive
\begin{equation}
  \phi \mathbin; (\pi_\kw{bq} \oplus \pi_\kw{rb}) \::\:
  L_\kw{bq} \:\le_{\mathbb{C} \twoheadrightarrow \mathbb{C}}\:
  \kw{Asm}(\kw{bq.s}) \oplus \kw{Asm}(\kw{rb.s})
  \,.
  \label{eqn:cex}
\end{equation}
...
%  \label{eqn:ccex}
%\end{equation}
[TODO: could add a step with an intermediate $L_\kw{rb}$
and refresh pasting diagram.]
\end{example}
%}}}

To the extent that Compositional CompCert \cite{compcompcert},
CompCertM \cite{compcertm} and CompCertO \cite{compcerto}
constitute verification frameworks,
this is the kind of approach that is referred to.
However,
as we articulate below,
as they stand these frameworks are not
expressive enough to conveniently capture
more advanced forms of reasoning.

[Points to make below:
\begin{itemize}
\item $L_\kw{bq}$ should use abstract state
\item $\Sigma_\kw{rb}$ should use \emph{no} state
\item Even $\vec{q}$ should in fact be hidden from the client
  (state encapsulation)
\item Ad-hoc simulation convention and proof for
  the way $\vec{q}$ is realized as concrete memory
  is not a satisfactory approach.
\item Horizontal composition forces uniform language interface and simulation convention,
  not appropriate for heterogeneous systems
  (incl. het in terms of ab state)]
\end{itemize}

\paragraph{Data Abstraction}

In Example~\ref{ex:compcomp},
the specification $L_\kw{bq}$
fails to hide the queue's implementation details:
because no abstraction occurs,
the specification would describe
the low-level effects of invoking $\kw{enq}$ and $\kw{deq}$
on the variables $\kw{c1}$, $\kw{c2}$ and $\kw{buf}$.

We would like instead to abstract away this low-level state
and replace with a high-level description
such as a sequence $\vec{q} \in \kw{val}^*$
representing the contents of the queue.
The operations $\kw{enq}$ and $\kw{deq}$
would then be 
...
\section*{XXX FROM BEFORE: Compositional Structures in CompCert Semantics} \label{sec:compcert} %{{{
...
\begin{figure} % fig:pasting {{{
  ...
  \end{tikzcd}
  \caption{%
    Pasting diagrams for composite simulations.
    (a) In CompCert,
...
Compositional CompCert
was significant in successfully interfacing
...
used to establish CompCert correctness theorems.
...
slightly generalize the ones in CompCertO.
All original simulations remain valid in our model and
...
% TODO: replace figure with pasting diagram
\[
  L_\kw{bq} : \top \twoheadrightarrow \mathcal{C}_\kw{m}
  \qquad
  \text{and}
...
% TODO: discuss functoriality more by this point
% TODO: the convention described below is used earlier than here
...
Concretely,
these simulation properties mean that for certain simulation statements,
we can choose
...
[TODO: edit to remove string diagrams]
...
\begin{figure} %{{{
  \begin{gather*}
    ...
    \qquad
    \begin{array}{r@{}l}
      (\mathbf{R}_1 \mathbin; \mathbf{R}_2) \mathbin@ (\mathbf{S}_1 \mathbin; \mathbf{S}_2)
      & {} \equiv
      (\mathbf{R}_1 \mathbin@ \mathbf{S}_1) \mathbin; (\mathbf{R}_2 \mathbin@ \mathbf{S}_2)
      \\
      \idsc_A \mathbin@ \kw{id}_U & {} \equiv \idsc_{A \mathbin@ U}
    \end{array}
    \\[1em]
    \begin{prooftree}
...
\citep{popl22}.
In particular a version of the $\mathbin@$ construction
appears in \citet{rbgs-cal}.
However this work has not been mechanized
...
\begin{equation}
  \label{eq:5}
  \psi_{23}\mathbin; (\psi \odot \psi_{12}) :
  L_3
  \le_{\top \twoheadrightarrow \mathcal{C}\mathbin@ (\hat{S} \mathbin; \hat{R})}
...
\[
  \alpha \odot \big(
  \psi_{23} \mathbin; (\psi \odot \psi_{12}) \big) :
  L_3
...
\[
  \begin{array}{c}
    ...
    := (\clightp{M} \mathbin@ \callee{\kw{mem}}^*) \mathbin; \kw{FP'}  \\[1ex]
    \phi_N : \kw{Clight}(N') \le_{\mathbf{S}(m_0) \twoheadrightarrow \mathbf{S}(m_0)} \kw{Clight}(N')
    := (\kw{Clight}(N') \mathbin@ \caller{m_0}_*) \mathbin; \kw{FP'} \\[1ex]
    \alpha : \mathbf{R} \sqsubseteq \mathbf{R} \mathbin; \mathbf{R}
    \qquad
    \beta : \mathbf{S}(n_0) \mathbin; \mathbf{S}(m_0) \sqsubseteq \mathbf{S}(n_0 \bullet m_0)
  \end{array}
  \,.
\]
...
\[
  \beta \odot ((\pi_N \mathbin; \phi_N) \odot (\phi_M \mathbin; \pi_M) ) \odot \alpha :
