...
and define
$\Gamma_\kw{rb} : \top \twoheadrightarrow \mathcal{C}@D_\kw{rb}$
such that:
\begin{align*}
  \Gamma_\kw{rb} &\vDash
...
\begin{example}[Refinement of abstract specifications] %{{{
...
which does not affect the global memory,
...
The simulation $\phi$ can be constructed from the components
...
$(\mathcal{C} \mathbin@ \langle \kw{mem} ]) \odot \Sigma_\kw{bq})
 \equiv
 (\Sigma_\kw{bq} \mathbin@ \kw{mem}) \odot
 (\mathcal{C} \mathbin@ \langle \kw{mem} ])$.
This equivalence corresponds geometrically to
sliding $\multimap$ past $\Sigma_\kw{bq}$,
and makes it possible in turn to vertically compose $\phi_2$
to obtain:
\[
  \big( (\mathcal{C} \mathbin@
   \langle \kw{mem} ]_\circlearrowright \mathbin@
   R_\kw{bq}) \odot \phi_1 \big) \mathbin;
  \big( (\phi_2 \mathbin@ D_\kw{rb}) \odot \Gamma_\kw{rb} \big)
    \quad : \quad
  \vcenter{\hbox{\begin{tikzpicture}[sdp]
