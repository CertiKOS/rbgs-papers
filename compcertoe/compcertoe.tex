...
CompCert
includes a formalization of the source language (Clight)
...
is a complex task.
The key to this achievement was the compositionality of simulation proofs.
...
[NB: could talk about proof-irrelevance here]
...
\begin{figure} % fig:pasting {{{
  ...
  \begin{tikzcd}[sep=0.3ex, row sep=tiny]
    \bullet \ar[dd, equal] \ar[rrrr, dash, "L_\kw{bq}"] &&&&
    \bullet \ar[dd, equal] \ar[rr, dash, "C"] &&
    \bullet \ar[dddd, equal]
    \\
    && \phi
    \\
    ...
    \bullet \ar[dd, equal] &
    \hspace{-1ex} \kw{id} \hspace{-1ex}
    \\
    & \:\:\pi_1\:\: & & \:\:\pi_2\:\:
    \\
    \bullet \ar[rr, "\kw{Asm}(\kw{rb.s})"', dash] && %\ar[dd, equal] &&
    \bullet \ar[rr, "\kw{Asm}(\kw{bq.s})"', dash] &&
    \bullet \ar[rr, "C"', dash] && \bullet
    %\\
    %&& \pi
    %\\
...
characterize the behavior of whole programs.
However, compilers usually act on individual translation units.
CompCert can indeed compile
the files shown in Fig.~\ref{fig:code},
but since they do not constitute a complete program,
...
use the notation $f(\vec{v})@m \in \kw{ident} \times \kw{val}^* \times \kw{mem}$
...
over a single, trivial 0-dimensional endpoint $\bullet$.
...
$\pi_1 : \kw{Clight}(\kw{bq.c}) \le \kw{Asm}(\kw{bq.s})$ and
$\pi_2 : \kw{Clight}(\kw{rb.c}) \le \kw{Asm}(\kw{rb.s})$.
...
to obtain
\begin{equation}
  \kw{id}_C \oplus (\phi \mathbin; (\pi_1 \oplus \pi_2))
  \::\:
  C \oplus L_\kw{bq}
  \:\le\:
...
\begin{figure} % fig:sd {{{
  ...
  \caption{General form (a) and particular example (b)
    of string diagrams representing simulation relations.}
  \label{fig:sd}
\end{figure}
%}}}
...
is depicted by connecting the output wires of the first
to the input wires of the second.
\item
Likewise,
the horizontal juxtaposition of two diagrams
represents the horizontal composition
...
Making those details externally observable
makes it difficult to define a uniform notion of simulation
...
CompCert's correctness proofs and to update them
...
CompCertO introduces a second kind of two-dimensional object.

%Nevertheless,
%CompCertO manages to work around this restriction
%and to derive a uniform compiler correctness theorem
%from heterogeneous pass correctness proofs.
...
Such refinements compose horizontally with each other
and on the left or right of simulations,
transforming their types:
\[
  \begin{prooftree}
  ...
  \end{prooftree}
\]
Simulation conventions refinements
are two-dimensional objects and
compose vertically as well:
\[
  \begin{prooftree}
    \hypo{x : \mathbf{R} \sqsubseteq \mathbf{R'} : A \leftrightarrow B}
    \hypo{y : \mathbf{S} \sqsubseteq \mathbf{S'} : B \leftrightarrow C}
    \infer2{(x \mathbin; y) :
      (\mathbf{R} \mathbin; \mathbf{S}) \sqsubseteq (\mathbf{R'} \mathbin; \mathbf{S'}) :
      A \leftrightarrow C}
  \end{prooftree}
\]
%Together
%$\sqsubseteq$ and $;$ form the basis of a rich
%\emph{typed Kleene algebra} for simulation conventions.
...
\paragraph{Encapsulated State}

CompCertO does not permit
...
\section{Our Framework}\label{sec:overview} %{{{

% Preamble {{{

We now present an overview of our verification framework.
We first
augment the semantic model of CompCertO
with \emph{layered composition} (\S\ref{sec:overview:lcomp})
as a more fundamental form of horizontal composition.
Then we
introduce an additional dimension of \emph{spatial compositionality},
acting on disjoint components of the state.
...
The semantic linking operator
$
  {\oplus}_A : (A \twoheadrightarrow A) \times (A \twoheadrightarrow A)
  \rightarrow (A \twoheadrightarrow A)
$
is limited to \emph{homogeneous} components.
As discussed in \S\ref{sec:compcerto-limitations},
this is at odds with
CompCertO's multiplicity of language interfaces.
By contrast,
the more fundamental \emph{layered composition} operator
...
\[
  ...
  \,,
\]
models only interaction over the language interface $B$.
As depicted in \autoref{fig:overview:ts:comp},
in $L_1 \odot L_2$
...
\begin{color}{gray}
In Example~\ref{ex:oplus-limitations}
we noted that the bounded queue code of Fig.~\ref{fig:code}
could be given a specification of type
$L_\kw{bq} : \top \twoheadrightarrow \mathcal{C}_\kw{m}$
to express 
,
...
Regions of the plane are labeled by language interfaces.
...
When the corresponding boundaries match,
diagrams can be connected horizontally or vertically
in a way that corresponds to (\ref{eqn:hvcomp}).

\begin{example} \label{ex:compcerto-sd}
\autoref{fig:compcerto}b
shows a composite simulation proof in string diagram notation.
[Refer to earlier example and explain]
\end{example}
...
\paragraph{Conjoints and Companions} %{{{
...
However,
we will see that in some circumstances
relationships can be established between
a transition system and
simulation conventions which in some sense
represent equivalent behaviors.
These relationships
can be formalized using the notions of
\emph{companion} and \emph{conjoint}
morphisms in a double category,
which in our setting
concretize as follows.

\begin{definition}
We say that a transition system $L : A \twoheadrightarrow B$ has:
\begin{itemize}
  \item a \emph{companion} $L^* : A \leftrightarrow B$ when
    $\kw{id}_A \le_{\kw{id}_A \twoheadrightarrow L^*} L$ and
    $L \le_{L^* \twoheadrightarrow \kw{id}_B} \kw{id}_B$;
  \item a \emph{conjoint} $L_* : B \leftrightarrow B$ when
    $\kw{id}_B \le_{L_* \twoheadrightarrow \kw{id}_B} L$ and
    $L \le_{\kw{id}_A \twoheadrightarrow L_*} \kw{id}_A$.
\end{itemize}
\end{definition}
...
\begin{figure} % fig:compcerto {{{
  ...
  \raisebox{-0.5\height}{\begin{tikzpicture}[xscale=0.2,yscale=0.16]
    ...
    \begin{scope}[opacity=0.66,outer sep=1pt]
      ...
      % Language interfaces
      \path (R1) |- node[below right] {$A_0$} (L1);
      \path (R1) -- node[right] {$A_1$} (R2);
      \path (Rn) |- node[above right] {$A_k$} (M1);
      \path (M1) -- node[above] {$B'$} (M2);
      \path (Mn) -| node[above left] {$Z_l$} (Sn);
      \path (L1) -- node[below] {$B$} (L2);
      \path (Ln) -| node[below left] {$Z_0$} (S1);
      \path (S1) -- node[left] {$Z_1$} (S2);
    ...
    \end{scope}
    \begin{scope}%[thick]
      \draw (S1) node[right] {$\mathbf{S}_1$}
        [rounded corners] -- (S1c)
        [sharp corners] -- (C);
      \draw (C)
        [rounded corners] -- (S2c)
        [sharp corners] -- (S2) node[right] {$\mathbf{S}_2$};
      \draw (Sn) node[right] {$\mathbf{S}_l$}
        [rounded corners] -- (Snc)
        [sharp corners] -- (C);
      \draw (R1) node[left] {$\mathbf{R}_1$}
        [rounded corners] -- (R1c)
        [sharp corners] -- (C);
      \draw (C)
        [rounded corners] -- (R2c)
        [sharp corners] -- (R2) node[left] {$\mathbf{R}_2$};
      \draw (Rn) node[left] {$\mathbf{R}_k$}
        [rounded corners] -- (Rnc)
        [sharp corners] -- (C);
    \end{scope}
  ...
  \raisebox{-0.5\height}{\begin{tikzpicture}[xscale=0.85,yscale=0.5]
    ...
    \begin{scope}
      ...
      \draw (0,3) node[above] {$L_\kw{bq}$}
         -- (0,2)
         [rounded corners]
         -- (-1, 1) node[left] {\tiny $\kw{Cilght}(\kw{bq.c})$}
      ...
      \draw (0,2)
         [rounded corners]
         -- (1, 1) node[right] {\tiny $\kw{Cilght}(\kw{rb.c})$}
...
so that many of the statements in this derivation are invalid.
...
$,
and it extends to simulations as well:
\[
  \begin{prooftree}
...
$,
and to state the simulation property
$\Sigma_\kw{bq} \mathbin@ \kw{mem} \le \kw{Clight}(\kw{bq.c})$.
Note however that the different data representations
used for example
by $(\Sigma_\kw{bq} \mathbin@ D_\kw{rb}) \odot L_\kw{rb} :
  \top \twoheadrightarrow \mathcal{C} \mathbin@ D_\kw{rb}$
and $L_\kw{bq} : \top \twoheadrightarrow \mathcal{C} \mathbin@ D_\kw{bq}$
remain an obstacle.
\end{example}
%}}}
...
That is,
we would like to generalize ${-} \mathbin@ U$
...
Then a relation $R \subseteq U_1 \times U_2$
...
Like language interfaces and transition systems,
sets and lenses form a category.
In fact,
as summarized in Table~\ref{tbl:roadmap},
parallel to our base model of \emph{active} components,
we build a \emph{passive} version of the framework,
where lenses play the role of horizontal morphisms,
but which otherwise provides the same double category structure and
composition principles.
The $\mathbin@$ operation we have described above
will preserve this structure in both arguments.

Within the passive world,
spatial composition is easy
and coincides with the cartesian product $\times$
of sets, lenses and relations.
Lenses can simply act in parallel on their own portion of the state.
By contrast
active components---%
which impose their schedule---%
conflict with one another.
The can only be spatially composed with passive components,
with the result being itself active.
...
\newcommand{\actv}{%
  \begin{tikzpicture}[baseline=0.1ex,x=1.2ex,y=1.2ex]
    \draw[fill,pattern=north east lines] (0,0) rectangle (1,1);
  \end{tikzpicture}}

While somewhat peculiar,
the distinction between active and passive components
and the associated restrictions on $\mathbin@$-composition
can nevertheless be formalized in a higher-categorical framework
by describing:
\begin{itemize}
  \item language interfaces as objects of type $A : \actv \curvearrowright \pasv$;
  \item sets (passive interfaces) as objects of type $U : \pasv \curvearrowright \pasv$.
\end{itemize}
Composition of interfaces in general can then be given as
$
...
$\mathbin@_{\actv\,,\,\pasv\,,\,\pasv}$ corresponds to the operation we have described, and
$\mathbin@_{\pasv\,,\,\pasv\,,\,\pasv}$ corresponds to the cartesian product of sets.
To sum up,
introducing spatial composition increases
the overall dimensionality of our framework as follows:
\begin{itemize}
\item
the trivial objects
$\actv$ and $\pasv$ are 0-dimensional,
\item
interfaces are now 1-dimensional and compose spatially ($\mathbin@$),
\item
behaviors ($\mathbin@$, $\odot$) and abstraction relationships ($\mathbin@$, $\,\mathbin;\,$) are 2-dimensional,
and \item simulations are 3-dimensional ($\mathbin@$, $\odot$, $\,\mathbin;\,$).
\end{itemize}
The ways in which $\mathbin@$ interacts with layered composition
can be captured with a string diagram notation for (now 2-dimensional) transition systems.
A similar situation holds for simulation conventions.
In these diagrams,
...
Finally,
spatial composition can be incorporated
as \emph{depth}
into simulation string diagrams,
which then become 3-dimensional.
This is illustrated in Fig.~X,
...
\begin{figure}
  \[
    \text{(a)}
    \quad
    \raisebox{-0.5\height}{\includegraphics[scale=0.09]{sd-cal-1}}
    \qquad
    \text{(b)}
    \quad
    \raisebox{-0.5\height}{\includegraphics[scale=0.07]{sd-cal-2}}
    \qquad
    \text{(c)}
    \quad
    \raisebox{-0.5\height}{\includegraphics[scale=0.045]{sd-cal-3}}
  \]
  \caption{%
    String diagrams with spatial composition representing
    (a) a transition system,
    (b) a simulation convention and
    (c) a simulation proof.
    For simplicity,
    in the simulation proof,
    only the boundary is shown.
  }
\end{figure}
...
\begin{example}
XXX we need an example here
showing abstract data being refined into a memory fragment,
being merged using the PCM.
(maybe the overall simulation convention for $L_\kw{bq}$)
\end{example}
...
\begin{figure}
  \[
    \begin{tikzpicture}[xscale=0.55,yscale=0.45,baseline=1.24cm]
      % Background
      \fill[ACMLightBlue!50] (0,0) -- (0,7)
      -- (3.5,7) -- (3.5,6) [rounded corners]
      -- (4.5,5) -- (5,4) [sharp corners] -- (7,4) |- cycle;
      \fill[ACMGreen!30]
      (3.5,7) -- (3.5,6) [rounded corners]
      -- (4.5,5) -- (5,4) [sharp corners] -- (7,4) |- cycle;
      % Region labels
      \begin{scope}[opacity=0.5]
        ...
        \node[below left] at (7,7) {$\top$};
      \end{scope}
      % Strings
      \begin{scope}
        \scriptsize
        \draw (3.5,7) node[above] {$\Gamma_\kw{bq} @ \kw{mem}$}
        -- (3.5,6) [rounded corners]
        -- (4.5,5) node[above right] {$\Gamma_\kw{rb} @ \kw{mem}$}
        -- (5,4) -- (4.5,3) node[right]{$\kw{rb.cp}$} -- (3.5,2);
        \draw (3.5,2) [rounded corners] -- (4.5,1)
        -- (5,0) node[below] (rb) {\footnotesize$\kw{rb.c}$};
        \draw (3.5,6) [rounded corners]
        -- (2.5,5) node[above left] {$\Sigma_\kw{bq} @ \kw{mem}$}
        -- (2,4) -- (2.5,3) node[left]{$\kw{bq.cp}$} -- (3.5,2);
        \draw (3.5, 2) [rounded corners] -- (2.5,1)
        -- (2,0) node[below] (bq) {\footnotesize$\kw{bq.c}$};
        \draw (5,4) -- (7,4) node[right] {$\varnothing$};
        \draw (0,2) node[left] {$\mathcal{C} \mathbin@ \kw{mem} \otimes \langle m_0 \rangle^\ans_*$} --
        (7,2) node[right] {$\mathcal{C} \mathbin@ \kw{mem} \otimes [\kw{mem}]^\ans_*$};
        \path (bq) -- (rb) node[midway] {\footnotesize$\circ$};
        \end{scope}
      % Nodes
      \begin{scope}[every node/.style={circle,draw,fill=white},inner sep=1pt]
        \scriptsize
        \node at (3.5,6) {$\pi_\kw{bq}$};
        \node at (2,4) {$\pi_\kw{bq}'$};
        \node at (5,4) {$\pi_\kw{rb}$};
      \end{scope}
      \begin{scope}
        \scriptsize
        \node[draw,fill=white,rounded corners] at (3.5,2) {$\kw{UnP}$};
      \end{scope}
    \end{tikzpicture}
  \]
