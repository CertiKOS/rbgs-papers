...
Now that we have recalled
relevant constructions in $\mathbf{Sup}$
and examined them through the lens of
game semantics, algebraic effect and universal coalgebra,
we are ready to use these constructions
to formalize the game model outlined in \S\ref{sec:overview},
starting with our notion of strategy.
...
A play in $E \twoheadrightarrow F$
...
Our strategy spaces are equipped with an inherent notion of refinement.
...
\subsection{Duality in $\mathbf{Sup}$}

\subsection{Definition}

The dual construction $\lnot S$ introduced above
allows to define refinement conventions as follows.

\begin{definition}
A \emph{refinement convention} $R : E \leftrightarrow F$
%between the effect signatures $E$ and $F$
is an element of the sup-lattice
\[
  S_{E \leftrightarrow F} \: := \:
    \mu W \cdot
    \bigoplus_{(m_1, m_2) \in E \times F}
    \biggl(
      \lnot
      \bigoplus_{(n_1, n_2) \in \kw{ar}(m_1) \times \kw{ar}(m_2)}
      \bigl( \lnot W \bigr)_\bot
    \biggr)_\bot
  \,.
\]
\end{definition}

Our use of $\lnot$ in the definition above
allows us to alternate between angelic and demonic nondeterminism
as we switch between relating questions and answers.
Remarkably,
$S_{E \leftrightarrow F}$
can still be constructed as a free sup-lattice
$\mathcal{D}(P_{E \leftrightarrow F})$,
though its ``plays'' are subject
to an unusual order:
\[
  s \in P_{E \leftrightarrow F} \: ::= \:
    m_1 m_2 \: \le \:
    m_1 m_2 n_1 n_2 s \: \le \:
    m_1 m_2 n_1 n_2
  \qquad
  \left(
  \begin{array}{cc}
  ...
  \right)
\]
These plays, interpreted as follows,
allow more simulations to succeed as
more and larger plays are added to the convention:
\begin{itemize}
  \item The play $m_1m_2$ allows the questions $m_1$ and $m_2$
  ...
  \item The play $m_1m_2n_1n_2s$
    ...
    However,
  \item the play $m_1m_2n_1n_2$ \emph{disallows}
...
\paragraph{Coinductive Definition}
...
which classify interactions
in a coarse-grained form.
In particular,
unlike effect signatures,
the type of answers
is not dependent on the questions.
Thus,
...
of a CompCertO transition system.
Notably, the transitions
hide the internal steps.
\[
  ...
  \end{prooftree}
\]
Using the well-known adjunction
...
CompCertO comes with a notion of
horizontal composition,
which is implemented by the \emph{linking}
...
\[
  \mathit{z}: \llbracket L_1 \rrbracket \odot \llbracket L_2 \rrbracket
...
\[
  \theta_{p_1, p_2} :=
  \mathit{z} \fatsemi (\phi^\kw{cc}_{p_1} \odot \phi^\kw{cc}_{p_2}) \fatsemi \ell
  \: : \:
  \llbracket \kw{Clight}(p_1) \rrbracket \odot
  \llbracket \kw{Clight}(p_2) \rrbracket
...
We prove the individual correctness that
\[
  \phi_\kw{bq} : \Sigma_\kw{bq} \le_{\mathcal{C}@\langle m_{0} \rangle \twoheadrightarrow \mathcal{C}@\langle m_{0} \rangle} \llbracket \kw{Clight}(\kw{bq.c}) \rrbracket
  \qquad
  \phi_\kw{rb} : \Gamma_\kw{rb} \le_{\varnothing \twoheadrightarrow \mathcal{C}@\langle m_{0} \rangle} \llbracket \kw{Clight}(\kw{rb.c}) \rrbracket
\]

[TODO: memory relation]
...
Example~\ref{ex:bqcorrect} proves at high abstraction level.
We can give the complete proof as:
\[
  \phi_1 \fatsemi (\phi_\kw{bq} \odot \phi_\kw{rb}) \fatsemi \theta_{\kw{bq}, \kw{rb}}
\]
...
Similarly, we define a loader for Clight programs
with the following definition
and property:

\[
  \begin{prooftree}
    \hypo{\kw{load}_\mathcal{C}(L_1)
  \::=\: \kw{entry}_\mathcal{C} \odot \llbracket L_1 \rrbracket
  \odot \kw{runtime}_\mathcal{C}}
    \infer[no rule]1{\kw{load}_\mathcal{A}(L_2)
  \::=\: \kw{entry}_\mathcal{A} \odot \llbracket L_2 \rrbracket
...
and CompCertO semantics.
The strategy- and
program-level specification
for the secret module:
{\scriptsize
\begin{align*}
  ...
  & \rightarrowtail
    (\kw{write}(1, b, 14)@m [b \mapsto \texttt{"uryyb, jbeyq!\textbackslash{}n"}] \leadsto 14@m[b \mapsto \texttt{"uryyb, jbeyq!\textbackslash{}n"}]) \\
...
\[
  ...
  \begin{array}{c}
    \begin{tikzcd}[row sep=0.5ex, column sep=1ex]
      \mathcal{S} \ar[dd, leftrightarrow, "\kw{runtime}^*"']
	  \ar[rr, "\Gamma_\kw{secret}"] &&
      \mathcal{P} \ar[dd, leftrightarrow, "\kw{entry}_*"] \\
      & \phi_\kw{secret} & \\
...
\begin{align*}
  ...
  \kw{main()}@m 
  \rightarrowtail (\kw{read}(0, b, 100)@m[b \mapsto unspecified] \leadsto \kw{len}(s)@m[b \mapsto s]) \\
  & \rightarrowtail (\kw{write}(1, b, \kw{len}(s))@m[b\mapsto \kw{rot13}(s)] \leadsto n@m[b\mapsto \kw{rot13}(s)]) \\
  & \rightarrowtail 0@m[b \mapsto deallocated]
\end{align*}}

The loader takes care of
the connection between strategies
and CompCertO semantics.
The rest of the proof
can be split into two parts.

On the end of program semantics,
we prove the CompCertO programs
indeed meet their specifications
\begin{gather*}
...
we define the overall behavior:
\[
  \Gamma \:\vDash\:
