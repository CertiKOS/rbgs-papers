...
and describe the kind of process interactions %behavior and communication
...
these properties are completely independent of one another.
...
but for now we will focus on effect signatures as elementary games.
...
using explicit proof terms will allows us to explicitly construct
sophisticated refinement proofs from more elementary properties,
as an application of our model's compositional structure.
...
defined in terms of a sequence $\vec{q} \in V^*$.
However,
the refinement
...
In addition,
we will show that \emph{linking} implements layered composition in the following sense:
\[
  \ell :
  \kw{Asm}(p_1) \odot \kw{Asm}(p_2) \le
  \kw{Asm}(p_1 + p_2)
\]
Note that here the refinement convention $\idsc_\mathcal{A}$
has been left implicit.
Importantly,
the properties $\phi^\kw{cc}$ and $\ell$ can be combined,
resulting in a property using the refinement convention
$\mathbb{C} \fatsemi \idsc_\mathcal{A} = \mathbb{C}$.
When two Clight programs $p_1$ and $p_2$
compile into the assembly programs $p_1'$ and $p_2'$,
we can characterize the behavior of the linked assembly program
$p_1' + p_2'$ using the refinement
\[
  (\phi^\kw{cc}_{p_1} \odot \phi^\kw{cc}_{p_2}) \fatsemi \ell
  \: : \:
  \kw{Clight}(p_1) \odot \kw{Clight}(p_2)
...
Having given
an overview of the overall compositional structure of our framework, and
a high-level description of the semantic objects involved,
...
%%{{{
%Our formalism uses standard game semantics constructions.
%Strategies are sets of plays of the form
%\[
%  q \rightarrowtail
...
%matches
%the outgoing moves
%($\underline{m}_1 n_1, \underline{m}_2 n_2, \ldots$) of $\sigma$
%in the signature $F$
%with the incoming moves
%($\underline{q}r, \underline{q}'r', \ldots$) of $\tau$
%and hides them from the composite
%$\sigma \odot \tau : E \twoheadrightarrow G$.
%
%At the same time,
%our model goes beyond traditional game semantics.
%Refinement squares in particular
%cannot be formulated in terms of individual plays.
%Therefore we supplement this traditional approach
%by deploying with a novel (co-)algebraic lens,
%which explains the strategy model in operational terms
%and provide new ways to manipulate strategies.
%
%
%%}}}

\subsection{Algebraic Game Semantics} %{{{

Our formalism employs a novel approach which we dub
...
to construct appropriate models.
Types are interpreted as domains and
terms of a given type
are interpreted as elements of the corresponding domain---%
often input--output functions of some kind.
...
terms of a given type are interpreted as 
\emph{strategies} for the corresponding games.
For example, the game associated with an effect signature $E$
...
the set of plays that the component will generate as 
we range over the possible choices of the environment.
...
edges correspond to the possible questions of the client.
Nodes are labeled with the answers given by the component,
or $\bot$ if the strategy is undefined.
The corresponding set of plays
traces the possible paths from the root of the tree.
Note that in each play,
...
the structures required to interpret the constructions of a given language
...
which are no longer directly manipulated \emph{as such}.

At the same time,
constructions on domains and constructions on games
remain closely related.
For example, games formed as
environment choices $\bigwith_{i \in I} G_i$ and
system choices $\bigoplus_{i \in I} G_i$
are precisely the products and coproducts
...
can be obtained using domain-theoretic constructions.
Specifically,
a domain of strategies
represented as prefix-closed sets of plays
can be derived as a free sup-lattice
...
\[
  ...
  \biggr)_\bot
\]
Note that the lifting operator ensures that
every decision of the component becomes immediately visible.
% XXX formalism (\bot in wrong place)
Writing
$\bigl\langle M[v] \mathrel{\big|} N \bigr\rangle_{v \in V} \in
 \bigwith_{q \in E_\kw{bq}} S_\bot$
for the computation which responds to the question $\kw{enq}[v]$
by continuing as $M[v]_\bot \in S_\bot$
and to $\kw{deq}$ by continuing as $N_\bot \in S_\bot$,
...
The tensor product $S_1 \otimes S_2$ can be generated from
pairs of the form $\sigma_1 \otimes \sigma_2$
which are then
...
\[
  ...
  \end{prooftree}
\]
The trivial lattices can be described as the units
...
is generated by taking formal joins over the poset $\langle P, {\le} \rangle$.
Concretely,
...
\[
  \mathcal{D}(P) :=
    \{ x \subseteq P \mid
      \forall p q \cdot p \le q \wedge q \in x \Rightarrow p \in x \}
\]
...
sup-preserving maps out of $\mathcal{D}(P)$
are defined by their actions on the generators:
\[
  f : \mathcal{D}(P) \rightarrow S \in \mathbf{Sup}
  \qquad \Longleftrightarrow \qquad
  g : P \rightarrow |S| \in \mathbf{Pos}
\]
...
is closed under the combinators above:
\[
%  \begin{array}{c}
...
categorical algebra uses endofunctors as signatures.
For example,
in $\mathbf{Sup}$
the effect signature $E$ can be represented by the functor
$\hat{E} : \mathbf{Sup} \rightarrow \mathbf{Sup}$
defined as:
\[
  \hat{E} S := \bigoplus_{m \in E} \biggl(
    \bigwith_{n \in \kw{ar}(m)} S
  \biggr)_\bot
\]
The lattice $\hat{E}S$ contains
...
triggers an external operation $m \in E$,
...
\[
  ...
  \,.
\]
Under a computational interpretation,
these lattices describe behaviors which can either
terminate immediately ($\kw{zero}\langle \rangle$)
or trigger a trivial output
and continue with a similar computation ($\kw{succ}\langle \sigma \rangle$).
The interpretation $\alpha$
uses arbitrary angelic choices
among computations of the form
$\kw{succ} \langle \cdots \kw{succ} \langle
\kw{zero} \langle \rangle \rangle \cdots
\rangle$
which trigger $\kw{succ}$
a finite number of times before terminating with $\kw{zero}$.
By contrast,
$\beta$ interprets $\kw{zero}$ as the undefined behavior $\bot$.
The elements of $\bar{\mathbb{N}}$
correspond to partial and total observations of
the infinite computation
$\kw{succ}\langle \kw{succ}\langle \cdots \rangle \rangle$.
\end{example}
%}}}

\paragraph{Categories of algebras} %{{{

The definitions above work for an arbitrary endofunctor $F$,
and $F$-algebras form a category $\mathbf{Alg}(F)$.
An algebra homomorphism $f : (S, \alpha) \rightarrow (T, \beta) \in \mathbf{Alg}(F)$
is a morphism $f : S \rightarrow T$ in the underlying category
such that $f \circ \alpha = \beta \circ F f$.
In the context of effect signatures,
this means an $\hat{E}$-algebra homomorphism
preserves the various operations of $E$.
...
The categorical dual of an algebra is
an $F$-\emph{coalgebra} $\delta : S \rightarrow F S$.
Coalgebras generalize transition systems
by associating possible actions captured by $F$
to states in the space $S$.
...
for an enriched endofunctor $F : \mathbf{Sup} \rightarrow \mathbf{Sup}$
always coincide:
...
The free monad $\hat{E}^*$
models computations which can invoke
the operations of the signature $E$
...
\[
  ...
  S_{E \rightarrow F} \: := \:
    ...
    \: \in \: \mathbf{Sup}
\]
awaits a question in the signature $F$
to provides the answer $r \in \kw{ar}(q)$.
In the process, the strategy
carries out a computation in $\hat{E}^*$.
Afterwards,
the game repeats and
the strategy awaits the next question.
...
In this case,
to define a strategy of type $E \rightarrow F$,
it is enough to provide for each question $(q \mathbin: R) \in F$ 
a computation in $E^*(\mathcal{P}(R))$.

\begin{definition}
A \emph{regular strategy} of type $E \rightarrow F$ is a computation
$\sigma \in \bigwith_{q \in F} \hat{E}^*\mathcal{P}(\kw{ar}(q))$.
It can be promoted to the strategy $\hat{\sigma} : E \rightarrow F$ defined by:
\[
  \hat{\sigma} := \langle
    {*} \mapsto E^* (\{r\} \mapsto \iota_r({*}_\bot) )\circ \sigma
  \rangle
\]
\end{definition}
...
\subsection{Game semantics formulation} %{{{
...
In~\ref{sec:application:compcerto}
and~\ref{sec:application:loader},
we show the CompCertO semantics and the simulation
can be interpreted
into strategies and the refinement order,
thus complete the verification
of the running examples.
In addition to ad-hoc verification
on individual programs,
we further implement the theory of
certified abstraction layers,
which is a generic verification framework,
using our strategy model in~\ref{sec:application:cal}.
Inspired by the stateful strategies,
we develop the ClightP language in~\ref{sec:application:clightp},
a variant of the Clight in CompCertO,
enabling language features such as state encapsulation
for better modularity in verification.
...
Given a CompCertO's transition system
$L: A \twoheadrightarrow B = \langle S, \rightarrow, I, X, Y, F \rangle$,
we define the following transitions
to simulate its observable behaviors
by hiding all the internal steps,
where
\[
  \delta : S \rightarrow \mathcal{P}(\ddot{A}(S) + B^\ans) \qquad
  \begin{prooftree}
    \hypo{s \rightarrow^* s' \: X \: m \leadsto n \: Y^{s'} \: s''}
    \infer1{\iota_1(\underline{m}n\underline{s''}) \in \delta(s)}
  \end{prooftree}
  \qquad
  \begin{prooftree}
    \hypo{s \rightarrow^* s' \: F \: r}
    \infer1{\iota_2(r) \in \delta(s)}
  \end{prooftree}
\]
Since there is as adjunction
$(\epsilon, \eta) : \mathcal{P} \dashv |{-}| : \mathbf{Set} \rightarrow \mathbf{Pos}$,
the above state transition generates
a coalgebra $(\mathcal{P}(S), \delta')$ where
\[
  \delta' : \mathcal{P}(S) \rightarrow \hat{A}(\mathcal{P}(S)) \oplus \mathcal{P}(B^\ans) \qquad
  \delta' :=  \mathcal{P}(\delta) \circ \epsilon_{\mathcal{P}(\ddot{A}(S) + B^\ans)}
\]
The CompCertO transition systems are stateless,
so we define the strategy
$\llbracket {L} \rrbracket := \hat{\sigma}$
by promoting the following regular strategy
\[
  \sigma(q) := \delta'(\{ s | q \: I\: s \})
\]
...
\paragraph{Linking Property}

To compose transition systems
with compatible language interfaces,
CompCertO defines a notion of
horizontal composition
$L_1 \oplus L_2$ with the property
\[
  \kw{Asm}(p_1) \odot \kw{Asm}(p_2) \le \kw{Asm}(p_1 + p_2)
\]
[Move \ref{ex:bq-proof} to here]

The embedding is compatible
with CompCertO's horizontal composition
in the sense that
\[
...
\emph{under-approximates}
the horizontal composition $\oplus$.

\paragraph{Example}
The property above allows us to prove
\[
...
\]
Thus, combining with~\ref{ex:bq},
we conclude that
specification $\Gamma_\kw{bq}$
is implemented by the assembly programs
$\kw{bq.s}$ and $\kw{rb.s}$.
[$\Gamma_\kw{bq}$ refined by $\Sigma_\kw{bq} \odot \Sigma_\kw{rb}$ seems unexplained]
...
On the other end,
the component
$\kw{runtime} : \mathcal{E} \rightarrow \mathcal{A}$
...
\begin{align*}
  ...
  \rightarrowtail \vec{rs'}[\kw{RAX} \mapsto n']@m[b \mapsto deallocated]
\end{align*}
...
loads a sequence of bytes from the file descriptor 0,
...
writes the first $n$ bytes starting from
where the pointer value $b$ points to
...
is defined as
\[
  \kw{load}_\mathcal{A}(L_\mathcal{A}) : \mathcal{S} \rightarrow \mathcal{P}
  \::=\: \kw{entry}_\mathcal{A} \odot L_\mathcal{A}
  \odot \kw{runtime}_\mathcal{A}
\]

\begin{example}
  The program $\kw{secret.s}$ is itself written is assembly.
  We compile the $\kw{rot13.c}$ to assembly,
  and load the programs:
  \[
    \kw{load}_\mathcal{A}(\kw{secret.s} + \kw{rot13.s}) \:\vDash\:
