...
To ground our framework in a more solid algebraic foundation,
...
By formalizing the underlying categorical structures
...
as illustrated in Example~\ref{ex:nicdriver}.
...
to obtain a proper symmetric monoidal structure
...
CompCertO adapts forward simulations
to the compositional setting
but does not establish
a theorem analogous to trace containment.
To do so,
we must incorporate component interactions
into event traces,
which become typed by language interfaces.
Simulation conventions must then be used to relate
...
and transpose the compositional structures uncovered by Tasks 1a and 1b.
...
we will also improve CompCertO from an engineering perspective.
This will include an improved management of symbol tables,
addressing the issues which prevented us from
supporting the \textsf{Unusedglob} compilation pass,
...
frameworks.
But because our changes are extremely minimal,
