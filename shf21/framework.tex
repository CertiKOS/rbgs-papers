...
Beyond correctness properties,
simulation conventions can be used to formulate \emph{as simulations}
properties of language semantics such as
stability under memory injections.
CompCertO uses such properties as glue
for establishing the final correctness theorem.
...
not addressed by the state of the art.

\vspace*{-2ex}
\paragraph{Ad-hoc Specifications} %{{{
...
In other cases,
static analyses can be used to establish specific properties.
The overarching challenge we seek to tackle
is to determine how to express such specifications
within our refinement algebra,
so that they can be combined with other correctness results.
...
or between threads.
State is partitioned into components
...
treats memory as a global and uniform store
whose entire state is continuously passed between components.
This makes
state encapsulation
...
To interface high-level verification tools with
low-level compiler correctness results,
providing a model flexible enough
to accomodate them both is not sufficient.
We must elucidate the correspondence between
high-level views of a program
which rely on a compositional and abstract treatment of state,
and their concrete, C-level representation
...
Our prior work on the certified operating system kernel CertiKOS \cite{dscal15}
%produced one of few large certified artifacts
%whose proof of correctness incorporates a certified compiler
%to provide assembly-level guarantees.
combines CompCert
with a verification technique
known as \emph{certified abstraction layers} (CAL).
%
The idea behind CAL is to verify code layer by layer,
...
data representations and primitive operations
formulated at an appropriate level of abstraction.
Unfortunately,
interfacing CAL and CompCert is non-trivial.
It required modifying CompCert
language semantics to treat certain external calls
as abstract primitive invocations,
and to keep track of the system's abstract state,
which affected the entire compiler correctness proof.
Conversely,
our implementation of CAL
had to take into account
many CompCert-specific details.

We seek to make CompCertO's semantic model
...
should not require changes to the compiler
or force the verification framework
to be overly specialized.

%}}}

%}}}

\subsection{Overview of Our Solution} \label{sec:mainideas:solution}
...
we can also use $\otimes$ to incorporate abstract state.
...
simulations. For example, the following constructions can be defined:
\[
...
For example,
in order to support certified abstraction layers,
...
When $\Sigma$ is implemented as a program $N$,
this abstract state component will be replaced by
a concrete memory state of type $\kw{mem}$
and the context $M$ will take on the type
$\mathcal{\tilde{C}} \otimes [\kw{mem}] \otimes [\kw{mem}]$.
...
Spatial composition allows us to incorporate
tools from separation logic into our framework
and use them to address this question.
...
The component $[u\rangle$ maintains an internal state of type $U$.
Upon its first activation with a trivial question ${*} \in I^\que$,
...
we can hide from the environment
...
transition system, simulation convention and simulation proof,
...
Although CompCert uses transition systems and simulations
to define language semantics and establish correctness proofs,
they are not necessarily a good choice
as a ``ground truth'' definition for
the notions of component behavior and refinement,
as they involve details such as internal states and branching
which are not externally observable.
Therefore,
it may be useful to provide a version of our model
which retains its compositional structure
but uses a more canonical representation of behaviors,
such as a trace semantics of strategy model.
However,
simulation conventions present a particular challenge
to adapt the this kind of setting.
...
current, we must update it with each new release of CompCert.
...
frameworks.
However, because our changes are extremely minimal,
...
in CompCertO, once closed using this operator, are identical to
the original CompCert semantics.
}
